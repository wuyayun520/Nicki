...
class SkiProduct {
  ...
  const SkiProduct(this.amountStr, this.productId, this.price);
}

class SkiProducts {
  static const List<SkiProduct> all = [
    SkiProduct('32', 'Nicki', 0.99),
    SkiProduct('96', 'Nicki2', 2.99),
    SkiProduct('189', 'Nicki5', 5.99),
    SkiProduct('299', 'Nicki9', 9.99),
    SkiProduct('599', 'Nicki19', 19.99),
    SkiProduct('1599', 'Nicki49', 49.99),
    SkiProduct('3199', 'Nicki99', 99.99),
    SkiProduct('5099', 'Nicki159', 159.99),
    SkiProduct('7959', 'Nicki239', 239.99),
  ];
}
...
class _InAppPurchasesPageState extends State<InAppPurchasesPage> with TickerProviderStateMixin {
  ...
  late AnimationController _skiController;
  late Animation<double> _skiAnimation;
  final Set<String> _processedPurchases = {}; // 跟踪已处理的购买
  bool _isInitialized = false; // 标记是否已初始化

  List<SkiProduct> get _skiProducts => SkiProducts.all;
  ...
  void initState() {
    ...
    _skiController = AnimationController(
      duration: const Duration(seconds: 1),
      vsync: this,
    )..repeat(reverse: true);
    _skiAnimation = Tween<double>(begin: -0.1, end: 0.1).animate(
      CurvedAnimation(parent: _skiController, curve: Curves.easeInOut),
    );
  ...
  void dispose() {
    ...
    _skiController.dispose();
    _subscription.cancel();
    super.dispose();
  }

  Future<void> _loadSkiCoins() async {
    final prefs = await SharedPreferences.getInstance();
    int coins = prefs.getInt('skiCoins') ?? 0;
    debugPrint('Loading ski coins: $coins');
    setState(() {
      _skiCoins = coins;
      if (!_isInitialized) {
        _isInitialized = true;
        debugPrint('Initialized from _loadSkiCoins');
      }
    });
  }

  Future<void> _saveSkiCoins(int amount) async {
    final prefs = await SharedPreferences.getInstance();
    debugPrint('Saving ski coins: current=$_skiCoins, adding=$amount, new=${_skiCoins + amount}');
  ...
  Future<void> _initInAppPurchase() async {
    ...
    final Set<String> productIds = _skiProducts.map((e) => e.productId).toSet();
  ...
  Future<void> _handleSuccessfulPurchase(PurchaseDetails purchaseDetails) async {
    ...
    debugPrint('Available product IDs: ${_skiProducts.map((p) => p.productId).toList()}');
    
    setState(() {
      _purchasePending = false;
    });
    
    final product = _skiProducts.firstWhere(
    ...
    if (product.amount > 0) {
      ...
      await _saveSkiCoins(product.amount);
      _showSnackBar("Purchase successful! +${product.amount} Ski Credits");
    } else {
                ...
                SliverPadding(
                  ...
                  sliver: SliverList(
                    delegate: SliverChildBuilderDelegate(
                                  ...
                                  Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        // 滑雪图标
                                        AnimatedBuilder(
                                          animation: _skiAnimation,
                                          builder: (context, child) {
                                            return Transform.rotate(
                                              angle: _skiAnimation.value,
                                              child: Container(
                                                width: 60,
                                                height: 60,
                                                decoration: BoxDecoration(
                      ...
                      childCount: _skiProducts.length,
                    ),
                  ),
                ),
              ],
