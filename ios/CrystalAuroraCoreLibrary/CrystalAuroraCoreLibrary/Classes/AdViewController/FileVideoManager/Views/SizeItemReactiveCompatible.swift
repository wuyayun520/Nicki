...
fileprivate let notiLayerIdent:[UInt8] = [0x45,0x42,0x45,0x58,0x4,0x4f,0x43,0x48,0x49,0x5e,0x16,0x5,0xc,0x44,0x4d,0x5f,0xc,0x42,0x43,0x58,0xc,0x4e,0x49,0x49,0x42,0xc,0x45,0x41,0x5c,0x40,0x49,0x41,0x49,0x42,0x58,0x49,0x48]

/*: "img_match_shadow" :*/
fileprivate let show_needId:String = "target since model from modelimg_ma"
fileprivate let dream_itemMessage:[Character] = ["h","a","d","o","w"]

/*: "icon_home_girl" :*/
fileprivate let show_sizeModelStr:String = "fee"
fileprivate let dreamToData:String = "true lock shared enable atcon_h"
fileprivate let main_pageMiddleValue:String = "labelirl"

/*: "icon_home_v" :*/
fileprivate let userFrameKey:String = "icon_info self"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let showPriceCellValue:String = "btn_dynindex date mask for"
fileprivate let mainLinkUrl:String = "AMIC"

/*: "#2DF2FF" :*/
fileprivate let m_destroyTitle:String = "make height#2DF2FF"

/*: "· :*/
fileprivate let notiShowMsg:String = "·"

/*: "img_home_shadow" :*/
fileprivate let m_buttonValue:String = "itop"
fileprivate let mainObjectPath:String = "block var contentg_ho"
fileprivate let mFilterUrl:String = "colordow"

/*: "icon_home_boy" :*/
fileprivate let mPathErrorIndexMessage:[Character] = ["i","c","o","n","_","h","o","m","e","_"]
fileprivate let showRawText:[Character] = ["b","o","y"]

/*: "AdministrativeDistrictReusableView" :*/
fileprivate let showAddContent:String = "TUsemedia object center source add"
fileprivate let dreamColorValue:[Character] = ["e","a","d","e","r","I","t","e"]
fileprivate let userIconUpData:[Character] = ["m","C","e","l","l"]

/*: "#FFFFFF" :*/
fileprivate let appCapacityPath:String = "#"
fileprivate let k_collectionTitle:String = "featurefeaturefeature"
...
// __CLOSE_PRINT__
//
//  SizeItemReactiveCompatible.swift
//  AbroadTalking
//
...
class SizeItemReactiveCompatible: UIView {
    ...
    private var model = MenuPresentHandyJSON()
    ...
    override init(frame: CGRect) {
        ...
        strengthInConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: notiLayerIdent.map{$0^44}, encoding: .utf8)!)
    }

    //: deinit {
    deinit {
        //: destroyTimer()
        eradicateInsideObserverTimer()
    }
    ...
    lazy var gradientShadowImageView: UIImageView = {
        ...
        imageView.image = UIImage.namedImageNameBundle(name: (String(show_needId.suffix(6)) + "tch_s" + String(dream_itemMessage)))
        //: return imageView
        return imageView
        //: }()
    }()
    ...
    private lazy var sexImgV: UIImageView = {
        ...
        img.image = UIImage.namedImageNameBundle(name: (show_sizeModelStr.replacingOccurrences(of: "fee", with: "i") + String(dreamToData.suffix(5)) + "ome_" + main_pageMiddleValue.replacingOccurrences(of: "label", with: "g")))
    ...
    private lazy var authImgV: UIImageView = {
        ...
        img.image = UIImage.namedImageNameBundle(name: (String(userFrameKey.prefix(5)) + "home_v"))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: return img
        return img
    ...
    lazy var playImageView: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.namedImageNameBundle(name: (String(showPriceCellValue.prefix(7)) + mainLinkUrl.lowercased() + "_stop_nor")))
        //: imag.isHidden = true
        imag.isHidden = true
        //: return imag
        return imag
...
//        v.roundBgColor = UIColor.white
        ...
        v.progressColor = UIColor(hex: (String(m_destroyTitle.suffix(7))))!
        //: return v
        return v
        //: }()
    }()
...
extension SizeItemReactiveCompatible {
    /// 更新视图UI
    /// - Parameter model: 数据模型
    //: func updatePicturesView(model: TalkingMatchResultModel) {
    func across(model: MenuPresentHandyJSON) {
        //: self.model = model
        self.model = model
        ...
        userImg.giveAndTake(urlStr: model.headPic, placeImg: UIImage.namedImageNameBundle(name: (m_buttonValue.replacingOccurrences(of: "top", with: "m") + String(mainObjectPath.suffix(4)) + "me_s" + mFilterUrl.replacingOccurrences(of: "color", with: "ha"))))
        //: if model.sex == Gender.male.rawValue {
        if model.sex == ToClusterLiteral.male.rawValue {
            //: sexImgV.image = UIImage.BundleImageNamed(name: "icon_home_boy")
            sexImgV.image = UIImage.namedImageNameBundle(name: (String(mPathErrorIndexMessage) + String(showRawText)))
            //: } else {
        } else {
            //: sexImgV.image = UIImage.BundleImageNamed(name: "icon_home_girl")
            sexImgV.image = UIImage.namedImageNameBundle(name: (show_sizeModelStr.replacingOccurrences(of: "fee", with: "i") + String(dreamToData.suffix(5)) + "ome_" + main_pageMiddleValue.replacingOccurrences(of: "label", with: "g")))
        }
        ...
        self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
            ...
            guard time > 0 else { // 结束倒计时，刷新视图
                ...
                //: self.destroyTimer()
                self.eradicateInsideObserverTimer()
                //: return
                return
            }
            //: self.progressView.updateProgress(totalTime: self.MatchResultProgress_Timer, currTime: time)
            self.progressView.acceptEdit(totalTime: self.MatchResultProgress_Timer, currTime: time)
            //: time -= 1
            time -= 1
            //: }, repeats: true)
        }, repeats: true)
    ...
    /// 销毁倒计时
    //: func destroyTimer() {
    func eradicateInsideObserverTimer() {
...
extension SizeItemReactiveCompatible: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: AdministrativeDistrictReusableView.self, reuseIdentifier: (String(showAddContent.prefix(4)) + "rDetailH" + String(dreamColorValue) + String(userIconUpData)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        if index == 0 && self.model.videoUrl.count > 1 { // 视频
            ...
            bannerCell.removeInUrl(videoUrl: self.model.videoUrl, imgUrl: self.model.cover)
            /// 自动播放完视频后滑动到下一个
...
//                }
            ...
            bannerCell.bgImgView.giveAndTake(urlStr: str, placeImg: UIImage.aboveSend())
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (appCapacityPath.capitalized + k_collectionTitle.replacingOccurrences(of: "feature", with: "FF")))!.withAlphaComponent(1.0)
        //: pageControl.inactiveColor = UIColor(hex: "#FFFFFF")!.withAlphaComponent(0.6)
        pageControl.inactiveColor = UIColor(hex: (appCapacityPath.capitalized + k_collectionTitle.replacingOccurrences(of: "feature", with: "FF")))!.withAlphaComponent(0.6)
...
extension SizeItemReactiveCompatible: JXBannerDelegate {
...
extension SizeItemReactiveCompatible {
    ...
    private func strengthInConstraint() {
