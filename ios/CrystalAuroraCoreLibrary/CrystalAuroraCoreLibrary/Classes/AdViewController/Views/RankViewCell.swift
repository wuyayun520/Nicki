...
fileprivate let m_faceMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_rank_coin" :*/
fileprivate let user_okStr:[UInt8] = [0x89,0x83,0x8f,0x8e,0xbf,0x92,0x81,0x8e,0x8b,0xbf,0x83,0x8f,0x89,0x8e]

private func elseName(action num: UInt8) -> UInt8 {
    return num ^ 224
}

/*: "icon_lounge" :*/
fileprivate let mainTextViewEqualData:[Character] = ["i","c","o","n","_","l","o","u","n","g"]
fileprivate let main_frameNeedText:[Character] = ["e"]

/*: "icon_rank_coin2" :*/
fileprivate let notiLabelIdent:[Character] = ["i","c","o","n","_","r","a","n","k","_","c","o","i"]
fileprivate let notiRequestValue:[Character] = ["n","2"]
...
class RankViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.blockConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: m_faceMessage.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.totalervertebralDisc()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .fontFile(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = RowView().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.namedImageNameBundle(name: String(bytes: user_okStr.map{elseName(action: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .fontFile(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.versionSize(colors: UIColor.errorSuccess(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.namedImageNameBundle(name: (String(mainTextViewEqualData) + String(main_frameNeedText)))
    }
}
...
extension RankViewCell {
    ...
    func administrator(model: BoundaryLineItemModel, index: Int) {
        ...
        headPic.placeOf(urlStr: model.headPic)
    ...
    func medietyType(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: user_okStr.map{elseName(action: $0)}, encoding: .utf8)! : (String(notiLabelIdent) + String(notiRequestValue))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.namedImageNameBundle(name: imageStr), for: .normal)
    }
}
