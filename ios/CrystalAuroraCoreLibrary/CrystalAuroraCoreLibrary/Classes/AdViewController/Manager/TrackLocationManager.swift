...
fileprivate let dreamActiveMsg:[UInt8] = [0x13,0x16,0xa,0x8,0x1b,0x10,0x16,0x15,0xc7,0x8,0x1c,0x1b,0xf,0x16,0x19,0x10,0x21,0x8,0x1b,0x10,0x16,0x15,0xfa,0x1b,0x8,0x1b,0x1c,0x1a,0xc7,0x10,0x1a,0xc7]

fileprivate func designStyleSelected(range num: UInt8) -> UInt8 {
...
fileprivate let noti_secondStr:String = "get locself user type"
fileprivate let dreamColorRequestToMsg:String = "aliveio"
fileprivate let showMakeIdent:String = " error:manager layer style view"
...
class TrackLocationManager: NSObject {
    ...
    var locationType: KeyTaskInsetTarget?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: TrackLocationManager?
    //: class func shared() -> TalkingLocationManager {
    class func messageValueConstraint() -> TrackLocationManager {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = TrackLocationManager()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func giftHorse(block: @escaping Locationblock) {
    ...
    func getBlock(block: @escaping Authblock) {
    ...
    func bullSEye() -> KeyTaskInsetTarget {
    ...
    func replacement() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: dreamActiveMsg.map{designStyleSelected(range: $0)}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension TrackLocationManager: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(noti_secondStr.prefix(7)) + dreamColorRequestToMsg.replacingOccurrences(of: "live", with: "t") + "n failed." + String(showMakeIdent.prefix(7))) + "\(error.localizedDescription)")
    }
}
