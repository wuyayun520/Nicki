...
fileprivate let user_listIdent:[UInt8] = [0xe3,0xe4,0xe3,0xfe,0xa2,0xe9,0xe5,0xee,0xef,0xf8,0xb0,0xa3,0xaa,0xe2,0xeb,0xf9,0xaa,0xe4,0xe5,0xfe,0xaa,0xe8,0xef,0xef,0xe4,0xaa,0xe3,0xe7,0xfa,0xe6,0xef,0xe7,0xef,0xe4,0xfe,0xef,0xee]

/*: "icon_rank_left" :*/
fileprivate let app_tabData:[Character] = ["i","c","o","n","_","r","a","n"]
fileprivate let k_cellValue:String = "k_leftimage deadline camera return var"

/*: "icon_rank_right" :*/
fileprivate let appDateUrl:String = "icon_ranself title app item"
fileprivate let user_sizeFilePath:String = "k_rightstatus return make content equal"

/*: "Yesterday" :*/
fileprivate let dreamPinAddPopularMessage:String = "rate name return imageYeste"

/*: "Today" :*/
fileprivate let dreamViewFormat:[Character] = ["T","o","d","a","y"]
...
class AdViewController: StateViewController {
    // MARK: params

    //: var titles = [String]()
    var titles = [String]()
    //: var rankPeriodModel = SocialPeriodModel()
    var rankPeriodModel = ModelType()
    //: var type = 0
    var type = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_listIdent.map{$0^138}, encoding: .utf8)!)
    }
    ...
    let leftImgView = UIImageView().then {
        ...
        $0.image = UIImage.namedImageNameBundle(name: (String(app_tabData) + String(k_cellValue.prefix(6))))
    }

    //: let rightImgView = UIImageView().then {
    let rightImgView = UIImageView().then {
        //: $0.image = UIImage.BundleImageNamed(name: "icon_rank_right")
        $0.image = UIImage.namedImageNameBundle(name: (String(appDateUrl.prefix(8)) + String(user_sizeFilePath.prefix(7))))
    }
...
extension AdViewController {
    ...
    private func fullPhaseOfTheMoonName(subTitle: String) -> [BoundaryLineItemModel] {
        //: if subTitle == "Yesterday".localized {
        if subTitle == (String(dreamPinAddPopularMessage.suffix(5)) + "rday").localized {
            //: return addEmptyModel(data: rankPeriodModel.yesterday?.res ?? [])
            return enterThumb(data: rankPeriodModel.yesterday?.res ?? [])
            //: } else if subTitle == "Today".localized {
        } else if subTitle == (String(dreamViewFormat)).localized {
            //: return addEmptyModel(data: rankPeriodModel.today?.res ?? [])
            return enterThumb(data: rankPeriodModel.today?.res ?? [])
            //: } else {
        } else {
            //: return addEmptyModel(data: rankPeriodModel.weekly?.res ?? [])
            return enterThumb(data: rankPeriodModel.weekly?.res ?? [])
        }
    }
    ...
    private func enterThumb(data: [BoundaryLineItemModel]) -> [BoundaryLineItemModel] {
        //: var mData = data
        var mData = data
        //: if mData.count == 0 {
        if mData.count == 0 {
            ...
            for _ in mData.count ... 9 {
                ...
                var model = BoundaryLineItemModel()
                //: model.isEmpty = true
                model.isEmpty = true
                //: mData.append(model)
                mData.append(model)
...
extension AdViewController: JXSegmentedListContainerViewListDelegate {
...
extension AdViewController: JXSegmentedListContainerViewDataSource {
    ...
    func listContainerView(_: JXSegmentedListContainerView, initListAt index: Int) -> JXSegmentedListContainerViewListDelegate {
        ...
        let listView = ModeListDelegate()
        //: listView.type = self.type
        listView.type = self.type
        //: listView.dataArray = self.getModelArray(subTitle: titles[index])
        listView.dataArray = self.fullPhaseOfTheMoonName(subTitle: titles[index])
        //: return listView
        return listView
    }
}
...
extension AdViewController {
    ...
    private func destroyFinish() {
        ...
        segmentedDataSource.titleSelectedColor = UIColor.my(207, 72, 255, 1)
        //: segmentedDataSource.titleNormalFont = .pingfangFont(type: .Medium, fontSize: 15)
        segmentedDataSource.titleNormalFont = .fontFile(type: .Medium, fontSize: 15)
        //: segmentedDataSource.titleSelectedFont = .pingfangFont(type: .Medium, fontSize: 15)
        segmentedDataSource.titleSelectedFont = .fontFile(type: .Medium, fontSize: 15)
        ...
        segmentedView.backgroundColor = UIColor.my(212, 131, 255, 1)
        //: segmentedView.defaultSelectedIndex = 1
        segmentedView.defaultSelectedIndex = 1

        //: segmentedView.frame = CGRect(x: (ScreenWidth-270)/2, y: 10, width: totalWidth, height: 36)
        segmentedView.frame = CGRect(x: (kBlockRateValue - 270) / 2, y: 10, width: totalWidth, height: 36)
