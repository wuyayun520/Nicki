...
/*: "id" :*/
fileprivate let noti_equalData:String = "indexd"
...
struct DetailFileHandyJSON: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct PicTransformable: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct HrModelType: HandyJSON {
    ...
    var status: ToCustomReflectable?
}

///
//: enum MomentVideoStatus: String {
enum ToCustomReflectable: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension DetailFileHandyJSON {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func query(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(UserSizeView.demonstrateDismiss(text: content!, size: CGSize(width: kBlockRateValue - 54, height: CGFloat(MAXFLOAT)), font: UIFont.fontFile(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == StateConvertible.Photo.rawValue || momentType == StateConvertible.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
