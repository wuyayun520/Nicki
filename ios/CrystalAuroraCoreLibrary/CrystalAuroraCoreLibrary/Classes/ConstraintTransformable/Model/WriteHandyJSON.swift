...
/*: "id" :*/
fileprivate let mWhenCharmValue:String = "ibottom"

/*: "%@ reply %@：%@" :*/
fileprivate let dream_touchStr:[Character] = ["%","@"," ","r","e","p"]
fileprivate let m_layerKey:[Character] = ["l","y"," ","%","@","：","%","@"]

/*: " %@：%@" :*/
fileprivate let kLoadIdent:String = " "
fileprivate let k_sendUrl:String = "%@：%@"
...
struct WriteHandyJSON: HandyJSON {
    //: var cid: String?
...
extension WriteHandyJSON {
    ...
    mutating func area() {
        ...
        if replyUser.count > 0 {
            ...
            tempContent = String(format: (String(dream_touchStr) + String(m_layerKey)), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(UserSizeView.demonstrateDismiss(text: tempContent, size: CGSize(width: kBlockRateValue - 82, height: CGFloat(MAXFLOAT)), font: UIFont.fontFile(type: .Regular, fontSize: 14), lineNumber: 0).height)
