...
/*: "id" :*/
fileprivate let appNameId:[Character] = ["i","d"]

/*: "reply" :*/
fileprivate let showStackStr:[Character] = ["r","e","p","l","y"]

/*: :  :*/
fileprivate let showListText:[Character] = [":"," "]

/*:  : :*/
fileprivate let dreamUserNorMessage:String = " :"
    ...
    mutating func wait() {
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if ManagerThen.shared.direction == .leftToRight {
                ...
                tempContent = (String(showStackStr)).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (String(showStackStr)).localized
            }
        ...
        let sizeheight = Int(UserSizeView.demonstrateDismiss(text: tempContent, size: CGSize(width: kBlockRateValue - 70, height: CGFloat(MAXFLOAT)), font: UIFont.fontFile(type: .Regular, fontSize: 16), lineNumber: 0).height)
