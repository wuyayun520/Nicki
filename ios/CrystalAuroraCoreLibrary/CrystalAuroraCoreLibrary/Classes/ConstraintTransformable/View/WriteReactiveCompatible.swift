...
fileprivate let user_imageId:[UInt8] = [0x15,0x12,0x15,0x8,0x54,0x1f,0x13,0x18,0x19,0xe,0x46,0x55,0x5c,0x14,0x1d,0xf,0x5c,0x12,0x13,0x8,0x5c,0x1e,0x19,0x19,0x12,0x5c,0x15,0x11,0xc,0x10,0x19,0x11,0x19,0x12,0x8,0x19,0x18]

/*: "icon_home_v" :*/
fileprivate let dreamCornerFormat:String = "icon_homreturn range data button image"
fileprivate let noti_showId:String = "e_vindex not class"

/*: "icon_lounge" :*/
fileprivate let showKitData:String = "more then imageicon_l"

/*: "#AAAAAA" :*/
fileprivate let show_errorTitle:String = "#AAAAAAmodel make manager first"

/*: "icon_moment_nor_report" :*/
fileprivate let showByKey:String = "icon_return effect text add right"
fileprivate let showBlockMessage:String = "label i selfnt_nor_r"
fileprivate let userStyleUrl:[Character] = ["e","p","o","r","t"]

/*: "img_me_edit_photo_default" :*/
fileprivate let showMessageData:[Character] = ["i","m","g","_","m","e","_","e","d","i","t","_","p","h","o","t","o","_"]
fileprivate let appErrorMessage:String = "DEFAULT"

/*: "icon_home_boy" :*/
fileprivate let kViewName:[Character] = ["i","c","o","n"]
fileprivate let main_applicationIdent:String = "_home_boypath height"

/*: "icon_home_girl" :*/
fileprivate let mainOfFormat:[Character] = ["i","c","o","n","_"]
fileprivate let userBottomFormat:[Character] = ["h","o","m","e","_","g","i","r","l"]

/*: "   " :*/
fileprivate let mAppKey:String = "pathpathpath"

/*: "level_" :*/
fileprivate let showCustomTitle:String = "LEVEL"
fileprivate let app_showFormat:String = "data"
...
class WriteReactiveCompatible: UITableViewCell {
    //: var cid = ""
    var cid = ""
    //: var uid = ""
    var uid = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_imageId.map{$0^124}, encoding: .utf8)!)
    }
    ...
    lazy var cardImgView: UIImageView = {
        ...
        img.image = UIImage.namedImageNameBundle(name: (String(dreamCornerFormat.prefix(8)) + String(noti_showId.prefix(3))))
    ...
    lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.namedImageNameBundle(name: (String(showKitData.suffix(6)) + "ounge"))
        //: self.addSubview(img)
        self.addSubview(img)
        //: return img
        return img
    ...
    lazy var timeLB: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(show_errorTitle.prefix(7))))
        //: self.addSubview(label)
        self.addSubview(label)
        //: return label
        return label
    ...
    lazy var BlockBtn: UIButton = {
        ...
        btn.setImage(UIImage.namedImageNameBundle(name: (String(showByKey.prefix(5)) + "mome" + String(showBlockMessage.suffix(8)) + String(userStyleUrl))), for: .normal)
        //: btn.imageView?.contentMode = .scaleAspectFill
        btn.imageView?.contentMode = .scaleAspectFill
        //: btn.addTarget(self, action: #selector(BlockBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(commentAwakeCamera), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
        //: return btn
        return btn
...
extension WriteReactiveCompatible {
    ...
    @objc func before(gestureRecongnizer: UIGestureRecognizer) {
    ...
    @objc func commentAwakeCamera() {
        //: let reportView = TalkingReportAlertView.init(frame: UIScreen.main.bounds, type: .reportCommentType, rId: cid)
        let reportView = VideoUserDataSource(frame: UIScreen.main.bounds, type: .reportCommentType, rId: cid)
        //: reportView.showReportViewIn(view: MacroReactiveCompatible.getWindow())
        reportView.wallop(view: MacroReactiveCompatible.colorForDestroy())
    }
}
...
extension WriteReactiveCompatible {
    ...
    func configViewStochasticProcess(model: ConstraintTransformable) {
        //: cid = model.cid ?? ""
        cid = model.cid ?? ""
        //: uid = model.uid ?? ""
        uid = model.uid ?? ""
        //: iconBtn.setUrlImage(urlStr: model.headPic ?? "", placeImg: UIImage.BundleImageNamed(name: "img_me_edit_photo_default"))
        iconBtn.frank(urlStr: model.headPic ?? "", placeImg: UIImage.namedImageNameBundle(name: (String(showMessageData) + appErrorMessage.lowercased())))
        ...
        nameLabel.textColor = model.loungePlus ? .career() : .eraseColor()
        ...
        case ToClusterLiteral.male.rawValue:
            //: sexBtn.setBackgroundImage(UIImage.BundleImageNamed(name: "icon_home_boy"), for: .normal)
            sexBtn.setBackgroundImage(UIImage.namedImageNameBundle(name: (String(kViewName) + String(main_applicationIdent.prefix(9)))), for: .normal)
        //: break
        //: case Gender.female.rawValue:
        case ToClusterLiteral.female.rawValue:
            //: sexBtn.setBackgroundImage(UIImage.BundleImageNamed(name: "icon_home_girl"), for: .normal)
            sexBtn.setBackgroundImage(UIImage.namedImageNameBundle(name: (String(mainOfFormat) + String(userBottomFormat))), for: .normal)
        //: break
        //: default:
        default:
            //: break
        ...
        levelBtn.setBackgroundImage(UIImage.namedImageNameBundle(name: (showCustomTitle.lowercased() + app_showFormat.replacingOccurrences(of: "data", with: "_")) + String(model.level ?? 1)), for: .normal)
        ...
        contentLB.text = model.content
        //: BlockBtn.isHidden = ModeStageReactiveCompatible.share.loginUserMode.userID == model.uid
        BlockBtn.isHidden = ModeStageReactiveCompatible.share.loginUserMode.userID == model.uid
    }
}
...
extension WriteReactiveCompatible {
    ...
    private func setupVideo() {
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(before(gestureRecongnizer:)))
