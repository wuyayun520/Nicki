...
fileprivate let mHiddenUrl:[UInt8] = [0x3d,0x3a,0x3d,0x20,0x7c,0x37,0x3b,0x30,0x31,0x26,0x6e,0x7d,0x74,0x3c,0x35,0x27,0x74,0x3a,0x3b,0x20,0x74,0x36,0x31,0x31,0x3a,0x74,0x3d,0x39,0x24,0x38,0x31,0x39,0x31,0x3a,0x20,0x31,0x30]

private func halfWill(length num: UInt8) -> UInt8 {
    return num ^ 84
}
...
class ToPreviewCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mHiddenUrl.map{halfWill(length: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func holder() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(guide))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(sinceEmptyTap))
...
extension ToPreviewCell: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func guide(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func sinceEmptyTap(tap: UIGestureRecognizer) {
    ...
    func array(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.placeOf(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func statusImage(image: UIImage) {
