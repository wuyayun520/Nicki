...
fileprivate let dreamTitlePathMessage:[UInt8] = [0xdf,0xd8,0xdf,0xc2,0x9e,0xd5,0xd9,0xd2,0xd3,0xc4,0x8c,0x9f,0x96,0xde,0xd7,0xc5,0x96,0xd8,0xd9,0xc2,0x96,0xd4,0xd3,0xd3,0xd8,0x96,0xdf,0xdb,0xc6,0xda,0xd3,0xdb,0xd3,0xd8,0xc2,0xd3,0xd2]

private func makeView(color num: UInt8) -> UInt8 {
    return num ^ 182
}
...
// __CLOSE_PRINT__
//
//  ResourceFrameReactiveCompatible.swift
//  AbroadTalking
//
...
class ResourceFrameReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dreamTitlePathMessage.map{makeView(color: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: ProductProgressView = {
        ...
        let v = ProductProgressView(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(renderGesture(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(beforeDesign(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension ResourceFrameReactiveCompatible {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func renderGesture(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func beforeDesign(gesture: UIPanGestureRecognizer) {
