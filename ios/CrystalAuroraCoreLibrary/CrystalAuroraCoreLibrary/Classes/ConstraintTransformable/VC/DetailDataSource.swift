...
/*: "tab" :*/
fileprivate let k_startMessage:String = "objectb"

/*: "page" :*/
fileprivate let k_afterValue:String = "PAGE"

/*: "uid" :*/
fileprivate let show_indexStr:String = "ublock"

/*: "model" :*/
fileprivate let appSizePath:String = "modsignature"

/*: "There's no feed yet." :*/
fileprivate let app_viewMessage:String = "There\'"
fileprivate let mLogActualId:String = "return view actual self feed "
fileprivate let mainDoingData:String = "frame cookie infoyet."
...
class DetailDataSource: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: k_senseBottomAutoData, right: 0)
        //: return table
        return table
...
extension DetailDataSource {
    ...
    func observer() {
        ...
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(k_startMessage.replacingOccurrences(of: "object", with: "ta"))] = 1
        //: dict["page"] = pageIndex
        dict[(k_afterValue.lowercased())] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        LanguageUnitRequestTool.imageCompletion(params: dict) { succeed, result, errorModel in
...
extension DetailDataSource {
    ...
    @objc func searchedOn(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(show_indexStr.replacingOccurrences(of: "block", with: "id"))] as! String
    ...
    @objc func deliver(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: DetailFileHandyJSON = userinfo[(appSizePath.replacingOccurrences(of: "signature", with: "el"))] as! DetailFileHandyJSON
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func totaleractionVoice(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: DetailFileHandyJSON = userinfo[(appSizePath.replacingOccurrences(of: "signature", with: "el"))] as! DetailFileHandyJSON
...
extension DetailDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = FileToThen.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? FileToThen
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = FileToThen(style: .default, reuseIdentifier: identifier)
        }
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == StateConvertible.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: FileToThen = cell as? FileToThen else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
                    tempcell.receiveFumbleObserver()
                }
            }
        }
    }
...
extension DetailDataSource: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        perceiveTitle(scrollView: scrollView)
    }
    ...
    func perceiveTitle(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: FileToThen = mainTableView.cellForRow(at: middleIndexPath) as! FileToThen
...
// MARK: - ViewItemControllerDelegate
...
extension DetailDataSource {
    ...
    private func moderateness() {
        ...
        style.TipsTitle = (app_viewMessage + "s no" + String(mLogActualId.suffix(6)) + String(mainDoingData.suffix(4))).localized
    ...
    private func viewApp() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(deliver(notification:)), name: kNetText, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(searchedOn(notification:)), name: k_momentValue, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(totaleractionVoice(notification:)), name: k_countShowKeyData, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(auditoryImage), name: kMessageValue, object: nil)
    }
}
