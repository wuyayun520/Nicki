...
fileprivate let notiManagerIdent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Save successfully" :*/
fileprivate let notiEnableValue:String = "listen aspect succeedSave s"
fileprivate let mainQuantityVisualIdent:[Character] = ["u","c","c","e","s","s","f","u","l","l","y"]

/*: / :*/
fileprivate let noti_makeObserverIdent:String = "with"
...
class CloudViewDelegate: ToKeyViewController {
    ...
    init(momentModel: DetailFileHandyJSON, index: Int, type: ModelPinpointPhotoType) {
        ...
        if ManagerThen.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(kBlockRateValue), y: 0)
    ...
    init(images: [UIImage], index: Int, type: ModelPinpointPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiManagerIdent.reversed(), encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        crushToUpModle(modle: currenModel)
//        if myType == .normal {
//            setDownloadView()
//        }
    }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: kBlockRateValue, height: kHalfText)
        ...
        view.frame = CGRect(x: 0, y: 0, width: kBlockRateValue, height: kHalfText)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(kBlockRateValue), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(kBlockRateValue), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(kBlockRateValue), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(ToPreviewCell.self, forCellWithReuseIdentifier: ToPreviewCell.className())
...
//    lazy var downloadBtn: RowView = {
//        let btn = RowView.init()
...
//        PounderReactiveCompatible.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: PicTransformable = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func memoryImage(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.logArea(showMsg: (String(notiEnableValue.suffix(6)) + String(mainQuantityVisualIdent)).localized)
                }
                //: } else {
            } else {
...
extension CloudViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func targetLoad() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func successView() {
        //: if  currenModel.sex==ModeStageReactiveCompatible.share.loginUserMode.sex {
        if currenModel.sex == ModeStageReactiveCompatible.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: ToPreviewCell = collectionView.dequeueReusableCell(withReuseIdentifier: ToPreviewCell.className(), for: indexPath) as! ToPreviewCell
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: PicTransformable = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.array(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.statusImage(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.statusImage(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: ToPreviewCell = cell as! ToPreviewCell
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / kBlockRateValue)
        //: if myType == .normal {
        if myType == .normal {
