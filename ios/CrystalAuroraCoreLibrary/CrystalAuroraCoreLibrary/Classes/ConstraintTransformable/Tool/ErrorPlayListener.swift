...
/*: "%@%@" :*/
fileprivate let user_halfMsg:String = "manager@manager@"

/*: "/TXCache" :*/
fileprivate let app_gestureContent:String = "normal component action crush info/T"
fileprivate let k_detailMessage:String = "player let view inXCache"
...
protocol ItemPlayerDelegate: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func subOnStatus(player: ErrorPlayListener, status: LibraryToPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func nonremittal(player: ErrorPlayListener, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func reload(player: ErrorPlayListener, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class ErrorPlayListener: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: ItemPlayerDelegate?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: LibraryToPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = ErrorPlayListener()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(app_gestureContent.suffix(2)) + String(k_detailMessage.suffix(6)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(dismissName), userInfo: nil, repeats: true)
...
extension ErrorPlayListener {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func addFresh(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: kVoiceName.format(), with: k_screenRecordData)
    ...
    @objc func dismissName() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.reload(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func settleItemWayMode(renderMode: ModelRowRenderMode) {
    ...
    func nationalService(time: Float) {
        //: self.player.seek(time)
        self.player.seek(time)
    }

    //: func height() ->Int {
    func tooBlockHeight() -> Int {
    ...
    func empower(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isWork == true {
        if EffectSocketDelegate.shared.isWork == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func freebie() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.subOnStatus(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func viewQuantityeraction() -> Bool {
...
extension ErrorPlayListener: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.nonremittal(player: self, duration: Int(duration), currentTime: Int(currentTime))
