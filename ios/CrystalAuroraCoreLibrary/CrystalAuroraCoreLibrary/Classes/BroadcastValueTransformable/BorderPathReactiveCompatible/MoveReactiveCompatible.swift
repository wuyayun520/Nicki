...
/*: "position.x" :*/
fileprivate let dream_blockName:String = "clearosi"

/*: "slideIn" :*/
fileprivate let appContentMsg:String = "slideInsize self return super share"

/*: "slideOut" :*/
fileprivate let user_inviteIdent:[Character] = ["s","l"]
fileprivate let user_reportKey:String = "ideOutadd let"
...
class MoveReactiveCompatible: NSObject {
    ...
    private var giftArr = [BroadcastValueTransformable]()
    ...
    private lazy var giftView: BorderPathReactiveCompatible = {
        //: let v = GiftFloatView()
        let v = BorderPathReactiveCompatible()
        //: MacroReactiveCompatible.getWindow().addSubview(v)
        MacroReactiveCompatible.colorForDestroy().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension MoveReactiveCompatible {
    ...
    private func lockButylate(gift: BroadcastValueTransformable) {
        //: self.synchronized(self) {
        self.forePath(self) {
            //: self.giftArr.append(gift)
            self.giftArr.append(gift)
            //: self.showNextGift(index: 0)
            self.logIndex(index: 0)
        }
    }

    //: public func synchronized<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    public func forePath<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    ...
    private func deleteDown(_ model: BroadcastValueTransformable) {
        ...
        let isRTL = ManagerThen.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : kBlockRateValue + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = kBlockRateValue / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? kBlockRateValue + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (dream_blockName.replacingOccurrences(of: "clear", with: "p") + "tion.x"))
        ...
        self.giftView.layer.add(slideIn, forKey: (String(appContentMsg.prefix(7))))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (dream_blockName.replacingOccurrences(of: "clear", with: "p") + "tion.x"))
            ...
            self.giftView.layer.add(slideOut, forKey: (String(user_inviteIdent) + String(user_reportKey.prefix(6))))
...
extension MoveReactiveCompatible {
    ...
    func userTitle() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.makeNoti(noti:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.partyBackground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.priceSend),
    ...
    @objc private func makeNoti(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<BroadcastValueTransformable>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != BroadcastHashable.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = ModeStageReactiveCompatible.share.appUserConfigMode.userType
        let currUserType = ModeStageReactiveCompatible.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != ModeStageReactiveCompatible.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != ModeStageReactiveCompatible.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
        ...
        self.lockButylate(gift: gift)
    }

    /// 应用退到后台
    //: @objc private func enterBackground() {
    @objc private func partyBackground() {
