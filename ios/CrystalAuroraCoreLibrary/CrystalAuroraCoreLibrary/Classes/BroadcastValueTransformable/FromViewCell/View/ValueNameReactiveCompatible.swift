...
fileprivate let m_managerName:[UInt8] = [0x83,0x84,0x83,0x9e,0xc2,0x89,0x85,0x8e,0x8f,0x98,0xd0,0xc3,0xca,0x82,0x8b,0x99,0xca,0x84,0x85,0x9e,0xca,0x88,0x8f,0x8f,0x84,0xca,0x83,0x87,0x9a,0x86,0x8f,0x87,0x8f,0x84,0x9e,0x8f,0x8e]

/*: "img_home_shadow" :*/
fileprivate let m_ofValue:[Character] = ["i","m","g","_","h"]
fileprivate let noti_playerValue:String = "return show name jump viewome_"

/*: "#9610FF" :*/
fileprivate let showErrorViewFormat:String = "#9610"
fileprivate let kNumberUseData:String = "size"

/*: "#8566FF" :*/
fileprivate let notiBetweenData:String = "#8566FFstring price"

/*: "Random Video" :*/
fileprivate let notiZoneIdent:String = "in target control titleRand"
fileprivate let show_arrayStr:String = "om Vtoday model"

/*: "icon_match_call_free" :*/
fileprivate let appIndexStr:[Character] = ["i","c","o","n","_","m","a","t","c","h","_","c","a","l","l","_","f","r","e"]
fileprivate let mainTitleMakeId:String = "status"

/*: "Free" :*/
fileprivate let dreamTotalBackgroundIdent:String = "return behavior live the cookieFree"

/*: "Scroll_upto_discove" :*/
fileprivate let mainTailData:String = "make stream guard in regionScroll_"
fileprivate let userRangeFormalKey:String = "year mini guard trueupto_"

/*: "icon_VectorStroke" :*/
fileprivate let mainFatalText:String = "time false statusicon_V"
fileprivate let mainAtPath:String = "action size varStro"
fileprivate let appModeId:[Character] = ["k","e"]

/*: "get json error" :*/
fileprivate let mModelMessage:String = "match viewget json"
fileprivate let mainProUrl:[Character] = [" "]
fileprivate let dreamKindPath:[Character] = ["e","r","r","o","r"]
...
// __CLOSE_PRINT__
//
//  ValueNameReactiveCompatible.swift
//  AbroadTalking
//
...
protocol ResourceDataSource: NSObjectProtocol {
    ///
    //: func numberOfCountInCardView(cardView: TalkingClubCardView) -> Int
    func quantityerchangeView(cardView: ValueNameReactiveCompatible) -> Int
    //: func cardViewCellForRowAtIndex(cardView: TalkingClubCardView, index: Int) -> TalkingClubCardViewCell
    func willFile(cardView: ValueNameReactiveCompatible, index: Int) -> LabelRecognizerDelegate
    //: func CellForRowAtIndexModel(index: Int) -> TalkingMatchResultModel
    func systemBy(index: Int) -> MenuPresentHandyJSON
}

//: @objc protocol TalkingClubCardViewDelegate: NSObjectProtocol {
@objc protocol PushAdViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func didRemoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func targetSize(cardView: ValueNameReactiveCompatible, cardViewCell: LabelRecognizerDelegate, forRowAtIndex: Int)

    //: @objc optional func didRemoveLastCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func dataCompartment(cardViewCell: LabelRecognizerDelegate, forRowAtIndex: Int)

    //: @objc optional func didDisplayCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func postIndex(cardViewCell: LabelRecognizerDelegate, forRowAtIndex: Int)

    //: @objc optional func didMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell, forMovePoint: CGPoint)
    @objc optional func removeFollow(cardView: ValueNameReactiveCompatible, cardViewCell: LabelRecognizerDelegate, forMovePoint: CGPoint)
    /// 随机匹配视频
    //: @objc optional func clubCardRandowBtnClick()
    @objc optional func subChangeFeature()
}

//: class TalkingClubCardView: UIView {
class ValueNameReactiveCompatible: UIView {
    ...
    var maxRemoveDistance = kBlockRateValue / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [LabelRecognizerDelegate]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: ResourceDataSource? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: PushAdViewDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: m_managerName.map{$0^234}, encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.image = UIImage.namedImageNameBundle(name: (String(m_ofValue) + String(noti_playerValue.suffix(4)) + "shadow"))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.versionSize(colors: [UIColor(hex: (showErrorViewFormat.capitalized + kNumberUseData.replacingOccurrences(of: "size", with: "FF")))!.cgColor, UIColor(hex: (String(notiBetweenData.prefix(7))))!.cgColor], size: CGSize(width: kBlockRateValue - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(notiZoneIdent.suffix(4)) + String(show_arrayStr.prefix(4)) + "ideo").localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(moreSearched), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        ...
        img.setBackgroundImage(UIImage.namedImageNameBundle(name: (String(appIndexStr) + mainTitleMakeId.replacingOccurrences(of: "status", with: "e"))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(dreamTotalBackgroundIdent.suffix(4))).localized, for: .normal)
...
extension ValueNameReactiveCompatible {
    //: func dismissGuide() {
    func beacon() {
        //: if guideView != nil {
        if guideView != nil {
    ...
    private func include() {
        ...
        image.image = UIImage.namedImageNameBundle(name: (String(mainTailData.suffix(7)) + String(userRangeFormalKey.suffix(5)) + "discove"))
        ...
        image2.image = UIImage.namedImageNameBundle(name: (String(mainFatalText.suffix(6)) + "ector" + String(mainAtPath.suffix(4)) + String(appModeId)))
    ...
    private func toPlayer(player: SVGAPlayer) {
        ...
        do {
            ...
            let url = LineReactiveCompatible.default.failure(type: .Club_guidePage)
            ...
            printLog(message: (String(mModelMessage.suffix(8)) + String(mainProUrl) + String(dreamKindPath)))
        }
    }

    //: @objc private func randomBtnClick() {
    @objc private func moreSearched() {
        //: self.delegate?.clubCardRandowBtnClick?()
        self.delegate?.subChangeFeature?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func changeAnim(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.quantityerchangeView(cardView: self) ?? 0
    ...
    func equalIndex(index: Int) {
        ...
        let cell = dataSource?.willFile(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? LabelRecognizerDelegate(), at: 0)
    ...
    func controlImage(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: LabelRecognizerDelegate = self.visibleCells[i] as! LabelRecognizerDelegate
            ...
            if isVisbleLast {
                ...
                self.delegate?.postIndex?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                raw(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func raw(cell: LabelRecognizerDelegate, transform: CGAffineTransform) {
    ...
    func datePrice(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func dequeueLikeIdentifier(identifier: String) -> LabelRecognizerDelegate? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! FromViewCell.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: FromViewCell = cellclass.init(reuseIdentifier: identifier)
    ...
    func lagniappeButton(index: Int) -> LabelRecognizerDelegate {
        //: let visibleIndex = visibleIndex(inde: index)
        let visibleIndex = server(inde: index)
        //: let cell = self.visibleCells[visibleIndex]
        let cell = self.visibleCells[visibleIndex]
        //: return cell as! TalkingClubCardViewCell
        return cell as! LabelRecognizerDelegate
    }

    /// 获取cell对应的index
    //: func indexForCell(cell: TalkingClubCardViewCell) -> Int {
    func objectCell(cell: LabelRecognizerDelegate) -> Int {
        //: let arr = self.visibleCells as? [TalkingClubCardViewCell]
        let arr = self.visibleCells as? [LabelRecognizerDelegate]
...
extension ValueNameReactiveCompatible: StateCellDelegete {
    ...
    func light(cell: LabelRecognizerDelegate) {
        ...
        self.delegate?.targetSize?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.quantityerchangeView(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.dataCompartment?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            equalIndex(index: self.currentIndex + 1)
        }
        // 更新布局
        //: updateLayoutVisibleCells(anim: true)
        controlImage(anim: true)
    }
    ...
    func instrumentation(cell: LabelRecognizerDelegate, forMovePoint: CGPoint) {
        ...
        self.delegate?.removeFollow?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
...
extension ValueNameReactiveCompatible {
    ...
    func infoName() {
        ...
        let index = RenameEraseWidthTotal.randow
