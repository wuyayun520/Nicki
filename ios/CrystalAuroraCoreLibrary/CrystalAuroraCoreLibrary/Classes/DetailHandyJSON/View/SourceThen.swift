...
fileprivate let m_aboveSucceedPath:[UInt8] = [0xe6,0xe1,0xe6,0xfb,0xa7,0xec,0xe0,0xeb,0xea,0xfd,0xb5,0xa6,0xaf,0xe7,0xee,0xfc,0xaf,0xe1,0xe0,0xfb,0xaf,0xed,0xea,0xea,0xe1,0xaf,0xe6,0xe2,0xff,0xe3,0xea,0xe2,0xea,0xe1,0xfb,0xea,0xeb]

private func miniPath(info num: UInt8) -> UInt8 {
    return num ^ 143
}

/*: "Edit Title" :*/
fileprivate let notiFileId:[Character] = ["E","d","i","t"," ","T","i"]
fileprivate let notiSourceValue:String = "TLE"

/*: "0/ :*/
fileprivate let noti_transformPath:String = "0/"

/*: "Finish" :*/
fileprivate let dreamSharedValue:String = "value false name contentFinish"

/*: "btn_delete" :*/
fileprivate let kColorStr:String = "btn_delerow in to self make"
fileprivate let main_userIdent:[Character] = ["t","e"]

/*: / :*/
fileprivate let appItemStr:String = "deadline"

/*: "\n" :*/
fileprivate let dream_wordModelStr:String = "\n"
...
class SourceThen: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(nowadays))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: m_aboveSucceedPath.map{miniPath(info: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(notiFileId) + notiSourceValue.lowercased()).localized
    ...
    lazy var inputTView: ContextView = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(dreamSharedValue.suffix(6))).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.versionSize(colors: UIColor.errorSuccess(), size: CGSize(width: 267, height: 50)), for: .normal)
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.namedImageNameBundle(name: (String(kColorStr.prefix(8)) + String(main_userIdent))), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(languageUnit), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension SourceThen: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = targetName(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func targetName(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
