...
fileprivate let showEqualViewUrl:[UInt8] = [0xeb,0xec,0xeb,0xf6,0xaa,0xe1,0xed,0xe6,0xe7,0xf0,0xb8,0xab,0xa2,0xea,0xe3,0xf1,0xa2,0xec,0xed,0xf6,0xa2,0xe0,0xe7,0xe7,0xec,0xa2,0xeb,0xef,0xf2,0xee,0xe7,0xef,0xe7,0xec,0xf6,0xe7,0xe6]

private func modelBar(user num: UInt8) -> UInt8 {
    return num ^ 130
}

/*: "icon_male_default" :*/
fileprivate let user_directionTitlePath:[UInt8] = [0xd6,0xdc,0xd0,0xd1,0xe0,0xd2,0xde,0xd3,0xda,0xe0,0xdb,0xda,0xd9,0xde,0xca,0xd3,0xcb]

private func makeCorner(request num: UInt8) -> UInt8 {
    return num ^ 191
}

/*: "icon_female_default" :*/
fileprivate let m_fatalFormat:[Character] = ["i","c","o","n","_","f","e","m","a","l","e","_","d","e","f","a","u","l","t"]

/*: "level_%@" :*/
fileprivate let kToKey:[Character] = ["l","e","v","e","l"]
fileprivate let noti_wrapMsg:String = "_%@"
...
class EqualTopicView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showEqualViewUrl.map{modelBar(user: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .fontFile(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .fontFile(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension EqualTopicView {
    //: func updateContentWithModel(model: TPreviewInfoModel) {
    func streetSmart(model: DetailHandyJSON) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.betise())
        ...
        let imgStr = model.sex == ToClusterLiteral.male.rawValue ? String(bytes: user_directionTitlePath.map{makeCorner(request: $0)}, encoding: .utf8)! : (String(m_fatalFormat))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(kToKey) + noti_wrapMsg.capitalized), model.level)
