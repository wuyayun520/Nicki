...
fileprivate let mainIndexId:[UInt8] = [0xb3,0xb4,0xb3,0xae,0xf2,0xb9,0xb5,0xbe,0xbf,0xa8,0xe0,0xf3,0xfa,0xb2,0xbb,0xa9,0xfa,0xb4,0xb5,0xae,0xfa,0xb8,0xbf,0xbf,0xb4,0xfa,0xb3,0xb7,0xaa,0xb6,0xbf,0xb7,0xbf,0xb4,0xae,0xbf,0xbe]

private func valueHiddenResume(top num: UInt8) -> UInt8 {
    return num ^ 218
}

/*: "Gifts received" :*/
fileprivate let dream_titleKey:[Character] = ["G","i","f","t","s"," ","r","e"]
fileprivate let user_barValue:String = "ceiveframe"
...
class NameFlowLayout: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainIndexId.map{valueHiddenResume(top: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var titleLab: UILabel = {
        ...
        lb.text = (String(dream_titleKey) + user_barValue.replacingOccurrences(of: "frame", with: "d")).localized
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collect.register(FrameAtReusableView.self, forCellWithReuseIdentifier: FrameAtReusableView.className())
        //: return collect
        return collect
        //: }()
    }()
...
extension NameFlowLayout: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: FrameAtReusableView.className(), for: indexPath) as! FrameAtReusableView
        //: let model: TalkingUserReceivedGiftModel = self.giftsDataArray[indexPath.row]
        let model: UserStateHandyJSON = self.giftsDataArray[indexPath.row]
        //: cell.configGiftItemCellWithData(giftModel: model, isNewProfile)
        cell.through(giftModel: model, isNewProfile)
        //: return cell
        return cell
    }
...
extension NameFlowLayout {
    //: private func setupSubviews() {
    private func poringOverWith() {
