...
/*: "Game Center" :*/
fileprivate let userShareFormat:[Character] = ["G","a","m","e"," ","C"]
fileprivate let showStatUrl:[Character] = ["e","n","t","e","r"]

/*: "You've got no list yet." :*/
fileprivate let user_sumTitle:String = "You\'v"
fileprivate let dreamPriceMessage:String = "hidden manager active no "
fileprivate let kFilePath:[Character] = ["."]

/*: "list" :*/
fileprivate let mErrorTableMsg:[Character] = ["l","i","s","t"]
...
class ConversationViewController: StateViewController {
    //: override func viewDidLoad() {
    override func viewDidLoad() {
        //: super.viewDidLoad()
        super.viewDidLoad()
        //: self.title = "Game Center".localized
        self.title = (String(userShareFormat) + String(showStatUrl)).localized
        //: setupSubviews()
        makeAll()
    ...
    private lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (user_sumTitle + "e got" + String(dreamPriceMessage.suffix(4)) + "list yet" + String(kFilePath)).localized
...
extension ConversationViewController {
    ...
    private func component() {
        ...
        InfoDetailRequestTool.modeCompletion(category: 1) { [weak self] _, result, _ in
            //: ProgressHUD.dismiss()
            ...
            json[(String(mErrorTableMsg))].arrayObject?.forEach { dict in
...
extension ConversationViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.close(model: dataArr[indexPath.row], nameColor: .startMake())
        //: return cell
        return cell
    }
