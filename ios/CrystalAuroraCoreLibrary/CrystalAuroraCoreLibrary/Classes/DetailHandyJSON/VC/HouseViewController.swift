...
/*: "Blacklist" :*/
fileprivate let appAspectMsg:String = "Blamanager collection"
fileprivate let mainShowImageFormat:String = "cktimeist"

/*: "page" :*/
fileprivate let dream_pointPath:[Character] = ["p","a","g","e"]

/*: "limit" :*/
fileprivate let noti_contentMsg:[Character] = ["l","i","m","i","t"]

/*: "20" :*/
fileprivate let dreamTextContent:String = "2center"

/*: "You've got Blacklist yet." :*/
fileprivate let app_timeStopName:String = "You\'ve"
fileprivate let appMakeStr:[Character] = ["a","c","k","l","i","s","t"," ","y","e","t","."]
...
class HouseViewController: StateViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var DataSource: [TalkingBlacklistModel] = []
    var DataSource: [LibraryBlacklistModel] = []
    ...
    override func viewDidLoad() {
        ...
        self.title = (String(appAspectMsg.prefix(3)) + mainShowImageFormat.replacingOccurrences(of: "time", with: "l")).localized
        //: designView()
        toWithView()
        //: reqData()
        premiere()
...
extension HouseViewController {
    ...
    func premiere() {
        ...
        var dict = [String: Any]()
        //: dict["page"] = pageIndex
        dict[(String(dream_pointPath))] = pageIndex
        //: dict["limit"] = "20"
        dict[(String(noti_contentMsg))] = "20"
        //: TalkingMeRequestTool.req_BlackList(params: dict) { succeed, result, errorModel in
        InfoDetailRequestTool.month(params: dict) { succeed, result, _ in
            ...
            if succeed {
                ...
                var dataArr: [LibraryBlacklistModel] = []

                //: if let datas = Array<TalkingBlacklistModel>.deserialize(from: array as? Array) {
                if let datas = Array<LibraryBlacklistModel>.deserialize(from: array as? Array) {
                    //: dataArr.append(contentsOf: (datas as? [TalkingBlacklistModel])!)
                    dataArr.append(contentsOf: (datas as? [LibraryBlacklistModel])!)
                }
                //: if self.pageIndex == 0 {
                if self.pageIndex == 0 {
...
extension HouseViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = StageReactiveCompatible.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingBlacklistCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? StageReactiveCompatible
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingBlacklistCell(style: .default, reuseIdentifier: identifier)
            cell = StageReactiveCompatible(style: .default, reuseIdentifier: identifier)
        }
        //: let model: TalkingBlacklistModel = self.DataSource[indexPath.row]
        let model: LibraryBlacklistModel = self.DataSource[indexPath.row]
        //: cell?.setCell(model: model, index: indexPath.row)
        cell?.compartment(model: model, index: indexPath.row)
...
extension HouseViewController {
    ...
    private func toWithView() {
        //: var style = EmptyStyle()
        var style = AtAdEmptyStyle()
        //: style.TipsTitle = "You've got Blacklist yet.".localized
        style.TipsTitle = (app_timeStopName + " got Bl" + String(appMakeStr)).localized
        ...
        MainTable.register(StageReactiveCompatible.self, forCellReuseIdentifier: StageReactiveCompatible.className())
