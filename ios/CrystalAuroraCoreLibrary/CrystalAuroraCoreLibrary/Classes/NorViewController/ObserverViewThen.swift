...
/*: "&sr=6" :*/
fileprivate let m_lineStr:[Character] = ["&","s","r","=","6"]
...
enum CreateConstraintCaseIterable: String, CaseIterable {
...
enum ToOutputStreamable: String {
...
class ObserverViewThen: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func cameraWith(urlType: ToOutputStreamable) -> String {
        //: var url = ModeStageReactiveCompatible.share.appConfigMode.urlH5Domain
        var url = ModeStageReactiveCompatible.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(ModeStageReactiveCompatible.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : ModeStageReactiveCompatible.share.appUserConfigMode.fullRechargePage)
            url.append(ModeStageReactiveCompatible.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : ModeStageReactiveCompatible.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(ModeStageReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : ModeStageReactiveCompatible.share.appUserConfigMode.halfRechargePage)
            url.append(ModeStageReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : ModeStageReactiveCompatible.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = ModeStageReactiveCompatible.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = ModeStageReactiveCompatible.share.appUserConfigMode.halfRechargePage.appending((String(m_lineStr)))
            //: url.append(ModeStageReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(ModeStageReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
