...
/*: "New friends" :*/
fileprivate let k_messageFormat:String = "label sizeNew frie"
fileprivate let userFramePath:[Character] = ["n","d","s"]

/*: "icon_yidu_pre" :*/
fileprivate let show_normalValue:[Character] = ["i","c","o","n","_","y","i","d","u","_","p"]
fileprivate let appImageMsg:[Character] = ["r","e"]

/*: "You've got no message yet." :*/
fileprivate let mainDismissMessage:[Character] = ["Y","o","u","\'","v","e"," ","g","o","t"," ","n","o"," ","m","e","s","s","a","g","e"," ","y","e"]
fileprivate let userPathFormat:[Character] = ["t","."]

/*: "icon_kong_kong_default" :*/
fileprivate let dreamEnableMsg:String = "icon_center true name source"
fileprivate let dreamColorLabUrl:String = "kong_deelse gesture listen if string"
fileprivate let dreamButtonAddIdent:[Character] = ["f","a","u","l","t"]

/*: "Cancel" :*/
fileprivate let dreamInfoSizeMessage:String = "add label about camera makeCancel"

/*: "OK" :*/
fileprivate let userFromFormat:[Character] = ["O","K"]

/*: "uid" :*/
fileprivate let dreamBackCenterMessage:[UInt8] = [0xe1,0xfd,0xf0]

/*: "Do you want to mark all messages as read?" :*/
fileprivate let userSharedData:[UInt8] = [0x3f,0x64,0x61,0x65,0x72,0x20,0x73,0x61,0x20,0x73,0x65,0x67,0x61,0x73,0x73,0x65,0x6d,0x20,0x6c,0x6c,0x61,0x20,0x6b,0x72,0x61,0x6d,0x20,0x6f,0x74,0x20,0x74,0x6e,0x61,0x77,0x20,0x75,0x6f,0x79,0x20,0x6f,0x44]

/*: "MoreMessages一键已读失败：code: :*/
fileprivate let mOffUrl:[Character] = ["M","o","r","e","M","e","s","s","a","g","e","s","\u{4e00}","键","\u{5df2}","\u{8bfb}","失"]
fileprivate let app_tableGestureFormat:[Character] = ["败","：","c","o","d","e",":"]

/*: , desc: :*/
fileprivate let mViewId:String = ", desif social meeting if"
fileprivate let dream_labelValue:String = "greet color black file shootc:"

/*: "btn_message_report_nor" :*/
fileprivate let kCenterKey:[Character] = ["b","t","n","_","m","e","s","s","a","g","e","_","r","e","p","o","r","t","_","n","o","r"]

/*: "#FF935D" :*/
fileprivate let appErrorData:String = "string to mode take#FF93"
fileprivate let show_walkMsg:[Character] = ["5","D"]

/*: "btn_message_block_nor" :*/
fileprivate let user_resultName:String = "btn_mfail more type make equal"
fileprivate let userPlayerTitle:String = "first name user_blo"
fileprivate let showPositionName:[Character] = ["r"]

/*: "#C179F9" :*/
fileprivate let mainViewValue:String = "#C179Fname image"
fileprivate let user_managerDismissPath:[Character] = ["9"]

/*: "btn_message_delete_nor" :*/
fileprivate let show_imageCellMsg:String = "btn_mreason appear image"
fileprivate let k_inputName:[Character] = ["e","s","s","a","g","e","_","d","e","l","e","t","e","_","n","o","r"]

/*: "#FF506D" :*/
fileprivate let appContentName:String = "#FF506Dpath video if"
...
class ReadModelDataSource: StateViewController {
    //: override func viewDidLoad() {
    override func viewDidLoad() {
        //: super.viewDidLoad()
        super.viewDidLoad()
        //: self.title = "New friends".localized
        self.title = (String(k_messageFormat.suffix(8)) + String(userFramePath)).localized
        ...
        btn.setImage(UIImage.namedImageNameBundle(name: (String(show_normalValue) + String(appImageMsg))), for: .normal)
        //: btn.addTarget(self, action: #selector(clearBadgeButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(selectPath), for: .touchUpInside)
    ...
    private lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (String(mainDismissMessage) + String(userPathFormat)).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(dreamEnableMsg.prefix(5)) + "kong_" + String(dreamColorLabUrl.prefix(7)) + String(dreamButtonAddIdent))
...
extension ReadModelDataSource {
    ...
    func topModel(conversationModel: ReadConversationModel) {
        ...
        //: DistanceConversationListener.shared.func__removeConversation(targetId: conversationModel.targetId, dataType: .moreList)
        DistanceConversationListener.shared.chapter(targetId: conversationModel.targetId, dataType: .moreList)
    ...
    func alongSend(conversationModel: ReadConversationModel) {
        // 拉黑/解除拉黑
        ...
        AlertInfoThen.nameConfig(title: nil,
                                    ...
                                    leftBtnTitle: (String(dreamInfoSizeMessage.suffix(6))).localized,
                                    ...
                                    rightBtnTitle: "OK".localized)
        {
            ...
            MenuRequestManager.overStart(uid: conversationModel.userID, isBlack: true) { succeed, _, _ in
                ...
                self.topModel(conversationModel: conversationModel)
                //: NotificationCenter.default.post(name: LIVE_BLOCK_USER_NOTIFICATION,
                NotificationCenter.default.post(name: kCellDepthValue,
                                                //: object: nil,
                                                object: nil,
                                                //: userInfo: ["uid": conversationModel.userID])
                                                userInfo: [String(bytes: dreamBackCenterMessage.map{$0^148}, encoding: .utf8)!: conversationModel.userID])
            }
        }
    }
    ...
    func writeModel(conversationModel: ReadConversationModel) {
    ...
    @objc private func selectPath() {
        ...
        AlertInfoThen.mug(message: String(bytes: userSharedData.reversed(), encoding: .utf8)!.localized, leftBtnTitle: (String(dreamInfoSizeMessage.suffix(6))).localized, rightBtnTitle: "OK".localized, leftBlock: {
            //: TalkingAlertShow.hideAlert()
            AlertInfoThen.showAlert()
            ...
            guard self.manager.sass() > 0 else { return }
            ...
            self.manager.moreMsgArr.forEach { model in
                ...
                V2TIMManager.sharedInstance().markC2CMessage(asRead: model.userID) {
                    ...
                    ReadReactiveCompatible.equalOn(msg: (String(mOffUrl) + String(app_tableGestureFormat)) + "\(code)" + (String(mViewId.prefix(5)) + String(dream_labelValue.suffix(2))) + "\(String(describing: desc))")
                }
            }
...
extension ReadModelDataSource {
    ...
    func blockObservers() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(remote(notification:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(funcOrCharacterSet(notif:)),
    ...
    @objc func remote(notification: NSNotification) {
    ...
    @objc private func funcOrCharacterSet(notif: Notification) {
...
extension ReadModelDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var model = self.manager.mutualPath(indexPath: indexPath)
    ...
    func tableView(_: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        ...
        guard let model = self.manager.mutualPath(indexPath: indexPath) else {
            //: return false
            return false
        }
    ...
    func tableView(_: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        guard let model = self.manager.mutualPath(indexPath: indexPath) else { return nil }
        ...
        let reportBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            ...
            self.writeModel(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let reportBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_report_nor").cgImage {
        if let reportBtnTrashcan = UIImage.namedImageNameBundle(name: (String(kCenterKey))).cgImage {
        ...
        reportBtn.backgroundColor = UIColor(hex: (String(appErrorData.suffix(5)) + String(show_walkMsg)))
        ...
        let blockBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            ...
            self.alongSend(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let blockBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_block_nor").cgImage {
        if let blockBtnTrashcan = UIImage.namedImageNameBundle(name: (String(user_resultName.prefix(5)) + "essage" + String(userPlayerTitle.suffix(4)) + "ck_no" + String(showPositionName))).cgImage {
        ...
        blockBtn.backgroundColor = UIColor(hex: (String(mainViewValue.prefix(6)) + String(user_managerDismissPath)))
        ...
        let deleteBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            ...
            self.topModel(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let deleteBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_delete_nor").cgImage {
        if let deleteBtnTrashcan = UIImage.namedImageNameBundle(name: (String(show_imageCellMsg.prefix(5)) + String(k_inputName))).cgImage {
        ...
        deleteBtn.backgroundColor = UIColor(hex: (String(appContentName.prefix(7))))
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //: SucceedInfoReactiveCompatible.share.func__pushToPriveteChatVC(chatID: model.targetId) { vc in
        SucceedInfoReactiveCompatible.share.sizeOff(chatID: model.targetId) { vc in
...
extension ReadModelDataSource: ModelObjectProtocol {
    //: func onRefreshConversationList(data: [TalkingConversationModel]) {
    func receive(data _: [ReadConversationModel]) {
        //: self.manager.req_moreMsgInitData()
        self.manager.nextTo()
        //: refreshTableView()
        outsideEqual()
