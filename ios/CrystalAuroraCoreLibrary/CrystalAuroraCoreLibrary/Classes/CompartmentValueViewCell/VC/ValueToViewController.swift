...
fileprivate let noti_interactionMessage:[UInt8] = [0x33,0x34,0x33,0x2e,0x72,0x39,0x35,0x3e,0x3f,0x28,0x60,0x73,0x7a,0x32,0x3b,0x29,0x7a,0x34,0x35,0x2e,0x7a,0x38,0x3f,0x3f,0x34,0x7a,0x33,0x37,0x2a,0x36,0x3f,0x37,0x3f,0x34,0x2e,0x3f,0x3e]

private func addValue(gift num: UInt8) -> UInt8 {
    return num ^ 90
}

/*: "icon_photo_giftbg" :*/
fileprivate let userFinishIdent:[Character] = ["i","c","o","n","_","p","h"]
fileprivate let main_contentValue:[Character] = ["o","t","o","_","g","i","f","t","b","g"]

/*: "Send" :*/
fileprivate let dreamCoverUrl:[Character] = ["S","e","n","d"]

/*: "nav_back_black_nor" :*/
fileprivate let notiRefreshText:String = "nav_banormal size path"
fileprivate let kListData:String = "lacontentk"

/*: "toUid" :*/
fileprivate let userTransformKey:String = "view share var share imagetoUid"

/*: "giftId" :*/
fileprivate let dream_centerValue:String = "action"
fileprivate let showSizeSendMsg:[Character] = ["i","f","t","I","d"]

/*: "giftNum" :*/
fileprivate let userModelName:String = "model err cover vargiftNum"

/*: "unlockMsgId" :*/
fileprivate let kSharedEngineId:String = "unlvalueck"
fileprivate let notiBetweenRawUrl:String = "MsgIdme bottom true if"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let dream_fileData:[UInt8] = [0x34,0x1f,0x1f,0x1e,0x5a,0x1b,0x5a,0x5f,0x3a,0x5a,0x52,0x5f,0x3a,0x5a,0x3d,0x15,0x16,0x1e,0x5a,0x19,0x15,0x13,0x14,0x9,0x53,0x5a,0xe,0x15,0x5a,0xf,0x14,0x16,0x15,0x19,0x11]

private func tempFrame(style num: UInt8) -> UInt8 {
    return num ^ 122
}

/*: "icon_photo_lock" :*/
fileprivate let m_sectionSizeTitle:String = "icon_pempty frame video error"
fileprivate let k_remindPath:String = "octhen"

/*: "Click on the screen to view 15s" :*/
fileprivate let show_viewTableStr:[UInt8] = [0xbb,0x94,0x91,0x9b,0x93,0xd8,0x97,0x96,0xd8,0x8c,0x90,0x9d,0xd8,0x8b,0x9b,0x8a,0x9d,0x9d,0x96,0xd8,0x8c,0x97,0xd8,0x8e,0x91,0x9d,0x8f,0xd8,0xc9,0xcd,0x8b]

private func repeatMakeHidden(priority num: UInt8) -> UInt8 {
    return num ^ 248
}

/*: "icon_photo_burn" :*/
fileprivate let noti_topName:String = "succeed let user pageicon_p"
fileprivate let appViewFormat:String = "hdeadlineo"

/*: "Already burned" :*/
fileprivate let dream_cellName:String = "model tip voice bottom hiddenAlrea"
fileprivate let m_screenFormat:String = "center"
fileprivate let kGroupUrl:[Character] = ["y"," ","b","u","r","n","e","d"]
...
typealias IntimatePhotoBlock = (_ model: RestrictionModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class ValueToViewController: StateViewController {
    ...
    //: private var imageTotalData = WriteCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = WriteCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = RestrictionModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: WriteCellData, toUid: String) {
    init(imageCellData: WriteCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(ModeStageReactiveCompatible.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(ModeStageReactiveCompatible.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_interactionMessage.map{addValue(gift: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.namedImageNameBundle(name: (String(userFinishIdent) + String(main_contentValue)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.placeOf(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(dreamCoverUrl)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(startTarget), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.namedImageNameBundle(name: (String(notiRefreshText.prefix(6)) + "ck_b" + kListData.replacingOccurrences(of: "content", with: "c") + "_nor")).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(ofConstraint), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ValueToViewController {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func extraNum(giftId: String, giftNum: String) {
        ...
        params[(String(userTransformKey.suffix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(dream_centerValue.replacingOccurrences(of: "action", with: "g") + String(showSizeSendMsg))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(userModelName.suffix(7)))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(kSharedEngineId.replacingOccurrences(of: "value", with: "o") + String(notiBetweenRawUrl.prefix(5)))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension ValueToViewController {
    ...
    @objc private func ofConstraint() {
    ...
    @objc private func startTarget() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        extraNum(giftId: photoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnPhotoTapGesture() {
    @objc private func itemAdd() {
        ...
        self.loadBlock(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.acceptEdit(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension ValueToViewController {
    ...
    private func sizeIn() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: dream_fileData.map{tempFrame(style: $0)}, encoding: .utf8)!.localizedDataArguments(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.namedImageNameBundle(name: (String(m_sectionSizeTitle.prefix(6)) + "hoto_l" + k_remindPath.replacingOccurrences(of: "then", with: "k")))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: show_viewTableStr.map{repeatMakeHidden(priority: $0)}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(itemAdd))
                ...
                self.intimateStatusImgV.image = UIImage.namedImageNameBundle(name: (String(noti_topName.suffix(6)) + appViewFormat.replacingOccurrences(of: "deadline", with: "ot") + "_burn"))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(dream_cellName.suffix(5)) + m_screenFormat.replacingOccurrences(of: "center", with: "d") + String(kGroupUrl)).localized
            }
        ...
        self.loadBlock(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func loadBlock(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
