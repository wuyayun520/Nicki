...
fileprivate let user_faceMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Reply" :*/
fileprivate let dreamLeadingName:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let app_hiddenMessageTitle:String = "#F95151count search player color equal"
...
// __CLOSE_PRINT__
//
//  MessageContextViewModelReactiveCompatible.swift
//  AbroadTalking
//
...
let kTabTitle = (kPositionName + 80.0)
//: class TalkingMessageNotificationView: UIView {
class MessageContextViewModelReactiveCompatible: UIView {
    //: var startDismissBlock: ((_ model: TalkingConversationModel) -> Void)?
    var startDismissBlock: ((_ model: ReadConversationModel) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: MessageContextViewModelReactiveCompatible) -> Void)? // 完成移除视图Block
    //: private var currModel = TalkingConversationModel()
    private var currModel = ReadConversationModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_faceMsg.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .fontFile(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .fontFile(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(dreamLeadingName)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(app_hiddenMessageTitle.prefix(7)))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension MessageContextViewModelReactiveCompatible {
    /// 展示视图，5s后自动移除
    //: func show(_ model: TalkingConversationModel?) {
    func disappear(_ model: ReadConversationModel?) {
    ...
    func reply(needAlpha: Bool = true) {
    ...
    @objc private func field() {
        ...
        if currModel.chatType == .service {
            ...
            //: SucceedInfoReactiveCompatible.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            SucceedInfoReactiveCompatible.share.sizeOff(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func quantityimateGesture(gesture: UISwipeGestureRecognizer) {
...
extension MessageContextViewModelReactiveCompatible {
    ...
    private func totalerval() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(field))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(quantityimateGesture))
        //: swipe.direction = .up
        swipe.direction = .up
