...
/*: "gifts" :*/
fileprivate let showLeadingMomentFormat:String = "giftbehind"

/*: "vipGifts" :*/
fileprivate let mMakeText:String = "player else border card hiddenvipG"

/*: "packGifts" :*/
fileprivate let noti_requestTitle:String = "packGview rank"

/*: "giftSendItem" :*/
fileprivate let noti_equalData:[Character] = ["g","i","f","t","S","e","n","d","I","t","e"]
fileprivate let appManagerPath:[Character] = ["m"]
...
class TrivialityThen: NSObject {
    ...
    private var cacheDict = [String: [PrixFixeSubscriptType: [Any]]]()
    // 数据锁
    ...
    private var giftType = ConstraintValueConvertible.normal

    //: static let share = TalkingChatGiftManager()
    static let share = TrivialityThen()
...
extension TrivialityThen {
    ...
    func soapResume(type: ConstraintValueConvertible = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        TrivialityThen.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[PrixFixeSubscriptType.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            TrivialityThen.share.cubicleRecord(completion: { isSucceed in
            ...
            TrivialityThen.share.statusObject(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            TrivialityThen.share.compartmentMake(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            TrivialityThen.share.windowCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func windowCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = TrivialityThen.share.giftType
        //: PushModelReactiveCompatible.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        PushModelReactiveCompatible.pushDownStack(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [PrixFixeSubscriptType: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(showLeadingMomentFormat.replacingOccurrences(of: "behind", with: "s"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<NnwTransformable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[PrixFixeSubscriptType.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(mMakeText.suffix(4)) + "ifts")] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<NnwTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[PrixFixeSubscriptType.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(noti_requestTitle.prefix(5)) + "ifts")] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<NnwTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[PrixFixeSubscriptType.Bags] = bagsArr
                }
                ...
                let sendItem: Array = data[(String(noti_equalData) + String(appManagerPath))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<ValueHandyJSON>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func cubicleRecord(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = TrivialityThen.share.giftType
        //: PushModelReactiveCompatible.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        PushModelReactiveCompatible.titleGift(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func tintCompletion(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: PushModelReactiveCompatible.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        PushModelReactiveCompatible.safetyMargin(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<WriteMemberModel>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func statusObject(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = ConstraintValueConvertible.live
        //: PushModelReactiveCompatible.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        PushModelReactiveCompatible.activityMake(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func compartmentMake(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = ConstraintValueConvertible.party
        //: PushModelReactiveCompatible.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        PushModelReactiveCompatible.upgrade(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func viewTop(perPageCount: Int, dataType: PrixFixeSubscriptType) -> Int {
    ...
    func voiceView(dataType: PrixFixeSubscriptType) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func typeAdd(dataType: PrixFixeSubscriptType, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension TrivialityThen {
    ...
    private func noblesse() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(equalAward),
    ...
    @objc private func equalAward() {
        //: gift_clearMemoryCache(showRed: true)
        mentalRepresentation(showRed: true)
    }
}
...
extension TrivialityThen {
    ...
    func mentalRepresentation(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        TrivialityThen.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
