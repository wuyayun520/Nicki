...
/*: "uid" :*/
fileprivate let kVersionKey:[Character] = ["u","i","d"]

/*: "onlineStatus" :*/
fileprivate let notiBottomPath:String = "itemli"
fileprivate let kLabelTitle:String = "mess"

/*: "isNewUser" :*/
fileprivate let dreamAddUrl:[Character] = ["i","s","N","e","w","U","s","e","r"]

/*: "isTPAuth" :*/
fileprivate let appEqualPlayerMsg:String = "fails"
fileprivate let dreamArrayGenderLetStr:[Character] = ["T","P","A","u","t","h"]

/*: "totalIntimate" :*/
fileprivate let notiInputIdent:[Character] = ["t","o","t","a","l","I","n","t"]
fileprivate let app_dataUrl:[Character] = ["i","m","a","t","e"]

/*: "userStatus" :*/
fileprivate let app_mPath:String = "userStaequal player short app"
fileprivate let k_centerId:String = "texts"

/*: "intimate" :*/
fileprivate let main_endValue:String = "intiview"
fileprivate let mColorEditTitle:[Character] = ["t","e"]
...
enum ForthrightRepresentable: Int {
    //: case offical  = 0
    case offical = 0 // 官方
    //: case chatRoom = 1
    case chatRoom = 1 // 聊天室
...
class WorkListManager {
    ...
    private var moreMsgModel: ReadConversationModel = //: return ReadConversationModel.getPlaceholderModel(chatType: .moreMsg)
        .warPartyModel(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func numericalQuantity() {
        //: self.officialArr = DistanceConversationListener.shared.getOfficialList()
        self.officialArr = DistanceConversationListener.shared.effectComponent()
        //: self.topArr = DistanceConversationListener.shared.topConvList
        self.topArr = DistanceConversationListener.shared.topConvList
        //: self.normalArr = DistanceConversationListener.shared.norConvList
        self.normalArr = DistanceConversationListener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        nextTo()
    ...
    func playscript(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: DistanceConversationListener.shared.getChatRoomData { succeed in
        DistanceConversationListener.shared.countAggregation { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = DistanceConversationListener.shared.chatRoomList
                self.chatRoomArr = DistanceConversationListener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func enableIndex(indexPath: IndexPath) -> ReadConversationModel? {
    ...
    func cacheRemove(model: ReadConversationModel?) -> ReadConversationModel? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: ViewHandyJSON? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: ViewHandyJSON? = CuttingEdgeThen.sizeEnter(targetId: targetId)
    ...
    func shouldFinish(conversationList: [ReadConversationModel], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        CuttingEdgeThen.punctureName(with: tempArr) { _, error in
    ...
    func eraseClickStochasticProcessSuspend(conversationModel: ReadConversationModel) {
    ...
    func appTo(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: DistanceConversationListener.shared.func__getConversationList { arr, leftMore in
        DistanceConversationListener.shared.tabTo { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [ReadConversationModel] = //: return Array<ReadConversationModel>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [ReadConversationModel] = //: return Array<ReadConversationModel>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [ReadConversationModel] = //: return Array<ReadConversationModel>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [ReadConversationModel] = //: return Array<ReadConversationModel>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [ReadConversationModel] = //: return Array<ReadConversationModel>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [ReadConversationModel] = //: return Array<ReadConversationModel>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [ReadConversationModel] = //: return Array<ReadConversationModel>()
        .init()
    //: }()
}
...
extension WorkListManager {
    /// 初始化数据
    //: func req_moreMsgInitData() {
    func nextTo() {
        //: self.moreMsgArr = DistanceConversationListener.shared.moreMsgList
        self.moreMsgArr = DistanceConversationListener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func mutualPath(indexPath: IndexPath) -> ReadConversationModel? {
    ...
    func firstModel() -> ReadConversationModel {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        nextTo()
...
extension WorkListManager {
    ...
    func statusCompletion(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        CuttingEdgeThen.constraintByView(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(String(kVersionKey))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(notiBottomPath.replacingOccurrences(of: "item", with: "on") + "neStatu" + kLabelTitle.replacingOccurrences(of: "mess", with: "s"))].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(dreamAddUrl))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(appEqualPlayerMsg.replacingOccurrences(of: "fail", with: "i") + String(dreamArrayGenderLetStr))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(notiInputIdent) + String(app_dataUrl))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(app_mPath.prefix(7)) + k_centerId.replacingOccurrences(of: "text", with: "tu"))].int ?? 1
                    ...
                    self.atDismiss(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func sharedList(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(kVersionKey))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(notiBottomPath.replacingOccurrences(of: "item", with: "on") + "neStatu" + kLabelTitle.replacingOccurrences(of: "mess", with: "s"))] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(dreamAddUrl))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(app_mPath.prefix(7)) + k_centerId.replacingOccurrences(of: "text", with: "tu"))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(app_mPath.prefix(7)) + k_centerId.replacingOccurrences(of: "text", with: "tu"))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension WorkListManager {
    ...
    func mixIn() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in DistanceConversationListener.shared.norConvList {
        for model in DistanceConversationListener.shared.norConvList {
            //: if model.totalIntimate >= ModeStageReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= ModeStageReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.cacheRemove(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in DistanceConversationListener.shared.topConvList {
        for model in DistanceConversationListener.shared.topConvList {
            //: if model.totalIntimate >= ModeStageReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= ModeStageReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func metadataTitle(convModel: ReadConversationModel, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func courseOfActionAdd(indexPath: IndexPath) -> ReadConversationModel? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func atDismiss(uid: String, intimate: Int) {
        ...
        CuttingEdgeThen.skinColor(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func extra(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(kVersionKey))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(main_endValue.replacingOccurrences(of: "view", with: "ma") + String(mColorEditTitle))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
