...
fileprivate let notiGiftUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "ReadMenuCell" :*/
fileprivate let noti_maxName:String = "Dropsex sign effect"
fileprivate let k_modelId:String = "erMenumake make return suspend image"
fileprivate let appFileFormat:String = "Cellequal message on remove"

/*: "roomId" :*/
fileprivate let mCoverPath:String = "if info makeroomId"

/*: "type" :*/
fileprivate let showFillTooStr:String = "tfeaturepe"

/*: "page" :*/
fileprivate let k_actionKey:String = "viewage"

/*: "icon_chats_pcr" :*/
fileprivate let main_adjustUrl:String = "ICON"
fileprivate let mManagerTitle:[Character] = ["s","_","p","c","r"]

/*: "All Numbers" :*/
fileprivate let app_targetMsg:String = "All Nstill view medium of talk"
fileprivate let user_listIdent:[Character] = ["u","m","b","e","r","s"]

/*: "all" :*/
fileprivate let dream_infoId:[Character] = ["a","l","l"]

/*: "Recent Gifts" :*/
fileprivate let k_sendFormat:String = "Recento block content in result"
...
protocol ModelDetailViewDelegate {
...
class OfDataSource: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    //: var menuDelegate: DropDownMemberMenuViewDelegate?
    var menuDelegate: ModelDetailViewDelegate?
    ...
    fileprivate lazy var contentView: UIView = self.accumulationAddGift()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.analyze()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = OfDataSource(frame: .zero)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiGiftUrl.reversed(), encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func accumulationAddGift() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func analyze() -> UITableView {
    ...
    private func coverList() {
        ...
        mTable.register(ReadMenuCell.self, forCellReuseIdentifier: (String(noti_maxName.prefix(4)) + "Memb" + String(k_modelId.prefix(6)) + String(appFileFormat.prefix(4))))
    ...
    func stage() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(mCoverPath.suffix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(showFillTooStr.replacingOccurrences(of: "feature", with: "y"))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(k_actionKey.replacingOccurrences(of: "view", with: "p"))] = String(pageIndex)
        ...
        TrivialityThen.share.tintCompletion(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = WriteMemberModel()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (main_adjustUrl.lowercased() + "_chat" + String(mManagerTitle))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(app_targetMsg.prefix(5)) + String(user_listIdent)).localized
                //: model.uid = "all"
                model.uid = (String(dream_infoId))
...
extension OfDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(noti_maxName.prefix(4)) + "Memb" + String(k_modelId.prefix(6)) + String(appFileFormat.prefix(4))), for: indexPath) as! ReadMenuCell
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! WriteMemberModel
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! WriteMemberModel
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.member(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.terrace(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: kBlockRateValue, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.subTarget()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .fontFile(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .totalervertebralDisc()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(k_sendFormat.prefix(5)) + "t Gifts").localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(app_targetMsg.prefix(5)) + String(user_listIdent)).localized
        }
