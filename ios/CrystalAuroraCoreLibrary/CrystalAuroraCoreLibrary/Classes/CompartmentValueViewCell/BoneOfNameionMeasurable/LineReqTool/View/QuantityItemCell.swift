...
fileprivate let show_nextMsg:[UInt8] = [0xa1,0xa6,0xa1,0xac,0x60,0x9b,0xa7,0x9c,0x9d,0xaa,0x72,0x61,0x58,0xa0,0x99,0xab,0x58,0xa6,0xa7,0xac,0x58,0x9a,0x9d,0x9d,0xa6,0x58,0xa1,0xa5,0xa8,0xa4,0x9d,0xa5,0x9d,0xa6,0xac,0x9d,0x9c]

fileprivate func textExtra(sex num: UInt8) -> UInt8 {
...
/*: "#FA9D33" :*/
fileprivate let k_keyIdent:[Character] = ["#","F","A","9","D","3"]
fileprivate let userMagnitudeData:[Character] = ["3"]

/*: "#343338" :*/
fileprivate let userShareToName:String = "#3image3"
fileprivate let k_lengthKitStr:String = "38"
...
class QuantityItemCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_nextMsg.map{textExtra(sex: $0)}, encoding: .utf8)!)
    }
...
extension QuantityItemCell {
    /// 刷新cell
    /// - Parameters:
    ///   - position: 麦位
    ///   - model: 麦位数据
    //: func refreshIcon(position: Int, model: TalkingMikeListItemModel) {
    func permit(position: Int, model: ResultMeasurable) {
        //: guard model.uid > 0, String(model.uid) != ModeStageReactiveCompatible.share.loginUid else {
        guard model.uid > 0, String(model.uid) != ModeStageReactiveCompatible.share.loginUid else {
        ...
        let color = model.needGift ? UIColor(hex: (String(k_keyIdent) + String(userMagnitudeData))) : UIColor(hex: (userShareToName.replacingOccurrences(of: "image", with: "43") + k_lengthKitStr.capitalized))
