...
fileprivate let showTitlePicFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Online Users" :*/
fileprivate let main_voiceIdent:String = "Onlineshow new self"
fileprivate let dream_makeLabelKey:String = "tops"
...
class PushDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showTitlePicFormat.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        v.attributeAt(width: kBlockRateValue,
                      //: height: contentV_H,
                      height: contentV_H,
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(main_voiceIdent.prefix(6)) + " Use" + dream_makeLabelKey.replacingOccurrences(of: "top", with: "r")).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(PathCountThen.self, forCellReuseIdentifier: PathCountThen.className())
...
extension PushDataSource {
    /// 请求用户在线列表
    //: private func reqData() {
    private func pageOfModel() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineList(roomId: TalkingVoiceRoomManager.shared().partyModel.roomId) { succeed, result, _ in
        LineReqTool.theoreticalAccount(roomId: SizeEventHandler.frameShared().partyModel.roomId) { succeed, result, _ in
...
extension PushDataSource {
    ...
    func equalSize() {
        ...
        contentView.frame = CGRect(x: 0, y: kHalfText, width: kBlockRateValue, height: contentV_H)
        //: reqData()
        pageOfModel()
...
extension PushDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = PathCountThen.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingLiveRoomOnlineListCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? PathCountThen
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingLiveRoomOnlineListCell(style: .default, reuseIdentifier: identifier)
            cell = PathCountThen(style: .default, reuseIdentifier: identifier)
        }
        //: cell?.setCell(model: DataSource[indexPath.row], index: indexPath.row)
        cell?.factor(model: DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension PushDataSource {
    ...
    private func equalConstraint() {
        ...
        let style = AtAdEmptyStyle()
        //: emptyView = EmptyView(frame: CGRect(x: 0, y: -100, width: ScreenWidth, height: contentV_H+100), style: style)
        emptyView = EmptyView(frame: CGRect(x: 0, y: -100, width: kBlockRateValue, height: contentV_H + 100), style: style)
