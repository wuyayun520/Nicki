...
/*: "id" :*/
fileprivate let app_nowStyleMessage:String = "add"

/*: "num" :*/
fileprivate let kDataPath:String = "narray"
...
@objc public enum ModelViewEquatable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct BoneOfNameionMeasurable: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// ModelViewEquatable
    ...
    mutating func laughAway(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct ProductAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (kDataPath.replacingOccurrences(of: "array", with: "um"))
    }
}
