...
fileprivate let app_leadingData:[UInt8] = [0x1f,0x18,0x1f,0x2,0x5e,0x15,0x19,0x12,0x13,0x4,0x4c,0x5f,0x56,0x1e,0x17,0x5,0x56,0x18,0x19,0x2,0x56,0x14,0x13,0x13,0x18,0x56,0x1f,0x1b,0x6,0x1a,0x13,0x1b,0x13,0x18,0x2,0x13,0x12]

/*: "iosSvgaEffect" :*/
fileprivate let showFramePath:String = "normal height make busy modeliosSvg"
fileprivate let dreamLengthPath:[Character] = ["a","E","f","f","e","c","t"]

/*: "iosEffect" :*/
fileprivate let notiSizeSystemManagerIdent:String = "iosEffecinsert top run info click"
fileprivate let user_mFormat:[Character] = ["t"]

/*: "entityId" :*/
fileprivate let show_insertText:String = "nowt"
fileprivate let show_privacyKey:String = "true sizeityId"
...
class MoveThen: UIView {
    ...
    var currentEffectMsgModel: DentModelType?
    ...
    var animatedView: SpaceThen?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: TaskReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_leadingData.map{$0^118}, encoding: .utf8)!)
    }
...
extension MoveThen {
    ...
    func sizeContainer(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(showFramePath.suffix(6)) + String(dreamLengthPath))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(notiSizeSystemManagerIdent.prefix(8)) + String(user_mFormat))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(show_insertText.replacingOccurrences(of: "now", with: "en") + String(show_privacyKey.suffix(5)))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = equalMain(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func equalMain(msgModel: Any?) -> Bool {
        ...
        if msgModel is DentModelType {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? DentModelType
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is BoneOfNameionMeasurable {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! BoneOfNameionMeasurable
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.laughAway(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = DentModelType()
            ...
            if giftMsgModel.showType == ModelViewEquatable.myStery.rawValue || giftMsgModel.showType == ModelViewEquatable.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: DentModelType = self.effectMsgModelArray[i] as! DentModelType
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? DentModelType())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? DentModelType(), at: index)
                }
            ...
            destroy(effectMsgModel: effectMsgModel!)
        }
...
extension MoveThen {
    ...
    func markResume() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = replacementInfo() else { return }
                //: firstShow(effectMsgModel: removeModel)
                destroy(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.upHighlight()
        }
    ...
    func replacementInfo() -> DentModelType? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return DentModelType()
        }
        ...
        if effectMsgModel is DentModelType {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! DentModelType
        ...
        return effectMsgModel as? DentModelType
    }
}
...
extension MoveThen {
    ...
    func destroy(effectMsgModel: DentModelType) {
        ...
        self.delegate?.digitiserObj(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        PresentAnimatTool.shared.celluloid(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.currentTable(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? DonationTransformable())
            //: })
        })
    }
    ...
    func currentTable(effectMsgModel: DentModelType, success: Bool, effectModel: DonationTransformable) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.loadTitle(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    snap(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func statuteTitle(effectMsgModel: DentModelType, effectModel: DonationTransformable) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.changeAnimating()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.greetAnimating()
        ...
        className = PresentAnimatTool.shared.visual(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! SpaceThen.Type
...
extension MoveThen: FromReadThen {
    ...
    func eigenvalueOfAMatrixSuccess(effectItemView: SpaceThen, success: Bool) {
        ...
        self.delegate?.viewSuccess(effectView: self, obj: effectItemView.effectMsgModel ?? DentModelType(), success: success)
    }
    ...
    func tightView(effectItemView: SpaceThen) {
        ...
        self.delegate?.lineBy(effectView: self, obj: effectMsgModel!)
    }
}
...
extension MoveThen {
    ...
    func marginOfSafety() {
        ...
        if animatedView != nil {
            ...
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.greetAnimating() // clean animation data
    ...
    func thole(success: Bool) {
        ...
        if self.effectMsgModelArray.count > 0, self.needStopAll == false {
            ...
            let effectMsgModel = replacementInfo()
