...
fileprivate let main_valueText:[Character] = ["T","S","y","s","t","e","m","M","e","s","s","a","g","e","C"]
fileprivate let m_showPathData:String = "ell_remove height green view"

/*: "init(coder:) has not been implemented" :*/
fileprivate let dreamAtIdent:[UInt8] = [0x24,0x29,0x24,0x2f,0xe3,0x1e,0x2a,0x1f,0x20,0x2d,0xf5,0xe4,0xdb,0x23,0x1c,0x2e,0xdb,0x29,0x2a,0x2f,0xdb,0x1d,0x20,0x20,0x29,0xdb,0x24,0x28,0x2b,0x27,0x20,0x28,0x20,0x29,0x2f,0x20,0x1f]

fileprivate func miniView(progress num: UInt8) -> UInt8 {
...
/*: "icon_talk_notic" :*/
fileprivate let mHideData:[Character] = ["i","c","o","n","_"]
fileprivate let k_liveAddName:String = "talk_ndomain current input"
...
@objc public protocol ObserverLibrary: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: TitleViewCell)
    func oeilDeBoeufCell(cell: TitleViewCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: TitleViewCell)
    func retry(cell: TitleViewCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: TitleViewCell)
    func assistance(cell: TitleViewCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: TitleViewCell)
    func timeIn(cell: TitleViewCell)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: TitleViewCell)
    func toDeadline(cell: TitleViewCell)
}

//: @objcMembers public class TitleViewCell: TCommonTableViewCell {
@objcMembers public class TitleViewCell: PublicViewCell {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\PhoneMessageFromReactiveCompatible.h
     */
    //: public var messageData: PhoneMessageFromReactiveCompatible?
    public var messageData: PhoneMessageFromReactiveCompatible?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(main_valueText) + String(m_showPathData.prefix(4)) + "ReuseId")

    /**
     *  协议委托
     *  负责实现 ObserverLibrary 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: ObserverLibrary?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dreamAtIdent.map{miniView(progress: $0)}, encoding: .utf8)!)
    }
    ...
    override public func modify(with data: SameCellData) {
        //: super.fill(with: data)
        super.modify(with: data)
        //: let newData: PhoneMessageFromReactiveCompatible = data as! PhoneMessageFromReactiveCompatible
        let newData: PhoneMessageFromReactiveCompatible = data as! PhoneMessageFromReactiveCompatible
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.coloured()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(someEqual(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.dismissProgress(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(equalAt(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(conversationBar(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.namedImageNameBundle(name: (String(mHideData) + String(k_liveAddName.prefix(6)) + "otic"))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(exist(recognizer:)))
...
extension TitleViewCell {
    ...
    @objc func someEqual(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.timeIn(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func conversationBar(recognizer: UIGestureRecognizer) {
