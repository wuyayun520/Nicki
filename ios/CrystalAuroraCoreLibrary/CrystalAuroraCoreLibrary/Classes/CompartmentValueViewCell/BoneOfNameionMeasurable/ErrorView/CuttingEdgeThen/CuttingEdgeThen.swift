...
/*: "serialQueue" :*/
fileprivate let m_videoText:[Character] = ["s","e","r","i","a","l","Q","u","e","u","e"]

/*: "fromUids" :*/
fileprivate let appExtraOfValue:[UInt8] = [0xd8,0xe4,0xe1,0xdf,0xc7,0xdb,0xd6,0xe5]

fileprivate func dataFormat(to num: UInt8) -> UInt8 {
...
public class CuttingEdgeThen: NSObject {
    ...
    private static var _instance: CuttingEdgeThen?
    //: class func shared() -> DBUserInfoManager {
    class func nearList() -> CuttingEdgeThen {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = CuttingEdgeThen()
            // 添加退出账号通知
...
extension CuttingEdgeThen {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([ViewHandyJSON]?, Error?) -> Void) {
    class func punctureName(with userIdsArr: [String], completion: @escaping ([ViewHandyJSON]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if CuttingEdgeThen.nearList().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(CuttingEdgeThen.nearList().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            CuttingEdgeThen.nearList().needReqUidsSet = nil
        }
        ...
        repeat {
            ...
            totalo(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func constraintByView(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (String(m_videoText)))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    //: PushModelReactiveCompatible.req_queryStatus(toUids: requestUidsStr) { _, list in
                    PushModelReactiveCompatible.fiftyPercentCompletion(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
...
extension CuttingEdgeThen {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> ViewHandyJSON? {
    class func sizeEnter(targetId: String) -> ViewHandyJSON? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return LabelFrameInfoTable.doMakePath(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: ViewHandyJSON?) {
    class func notifMake(with userInfo: ViewHandyJSON?) {
        ...
        let model = CuttingEdgeThen.sizeEnter(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        LabelFrameInfoTable.imageMap(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: kStatusData, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func skinColor(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = CuttingEdgeThen.sizeEnter(targetId: targetId) else { return }
    ...
    class func addFillRunning(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = CuttingEdgeThen.sizeEnter(targetId: targetId) else { return }
    ...
    class func recognise(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        LabelFrameInfoTable.dignity(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        CuttingEdgeThen.nearList().reqedUidsSet.remove(targetUid)
    }
}
...
extension CuttingEdgeThen {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([ViewHandyJSON]?, Error?) -> Void) {
    private class func totalo(with userIdsArr: [String], completion: @escaping ([ViewHandyJSON]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = LabelFrameInfoTable.modeGet(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if CuttingEdgeThen.nearList().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            CuttingEdgeThen.omit(with: noDBUidArr)
        }
    }
    ...
    private class func omit(with userIdsArr: [String]) {
        ...
        CuttingEdgeThen.nearList().reqedUidsSet = CuttingEdgeThen.nearList().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: PushModelReactiveCompatible.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        PushModelReactiveCompatible.cellCompletion(param: [String(bytes: appExtraOfValue.map{dataFormat(to: $0)}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                CuttingEdgeThen.nearList().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            CuttingEdgeThen.postAction(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: ViewHandyJSON] = [:]
            var retInfoDict: [String: ViewHandyJSON] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: kStatusData, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [ViewHandyJSON]) {
    private class func postAction(userInfoArr: [ViewHandyJSON]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            LabelFrameInfoTable.imageMap(aWrap)
        }
    }
}
