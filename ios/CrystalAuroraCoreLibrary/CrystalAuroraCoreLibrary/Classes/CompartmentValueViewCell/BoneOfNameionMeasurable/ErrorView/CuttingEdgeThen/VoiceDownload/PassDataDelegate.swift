...
fileprivate let userPathValue:String = "Documegesture page index"
fileprivate let mDataValue:String = "ser/voiceattribute layer user"
fileprivate let m_cellFormat:String = "/"

/*: "Documents/User/record/" :*/
fileprivate let user_originValueMsg:String = "Docukey return self color model"
fileprivate let showMainTitle:String = "equal height/User"

/*: "cancelled" :*/
fileprivate let dreamModelTargetValue:String = "availableanavailablee"

/*: .wav" :*/
fileprivate let k_aspectTitle:String = "case video self make let.wav"
...
typealias OneKeyGreetDownloadBlock = (_ model: ShitworkStateReactiveCompatible) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol MagnitudeManagerDelegate: NSObjectProtocol {
    /// 下载完成
    ...
    func totalernalRepresentation(model: ShitworkStateReactiveCompatible)
    /// 过期语音消息，服务器缓存时间为7天
...
extension PassDataDelegate {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func warpath(_ tasksArray: [ShitworkStateReactiveCompatible]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.byPushPath()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(userPathValue.prefix(6)) + "nts/U" + String(mDataValue.prefix(9)) + m_cellFormat.capitalized)) {
                //: model.zipFilePath = "\(PrimrosePathReactiveCompatible.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(PrimrosePathReactiveCompatible.modulePath())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(user_originValueMsg.prefix(4)) + "ments" + String(showMainTitle.suffix(5)) + "/record/")) {
                //: model.zipFilePath = "\(PrimrosePathReactiveCompatible.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(PrimrosePathReactiveCompatible.nosePath())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            EvenThen.observerPath(msgId: model.taskId, status: "\(KeyCountLiteral.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func milk(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = ShitworkStateReactiveCompatible.digitiserModel(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? ShitworkStateReactiveCompatible()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(userPathValue.prefix(6)) + "nts/U" + String(mDataValue.prefix(9)) + m_cellFormat.capitalized)) {
            //: taskModel.zipFilePath = "\(PrimrosePathReactiveCompatible.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(PrimrosePathReactiveCompatible.modulePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(user_originValueMsg.prefix(4)) + "ments" + String(showMainTitle.suffix(5)) + "/record/")) {
            //: taskModel.zipFilePath = "\(PrimrosePathReactiveCompatible.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(PrimrosePathReactiveCompatible.nosePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension PassDataDelegate: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = ShitworkStateReactiveCompatible.inscribe(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.totalernalRepresentation(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (dreamModelTargetValue.replacingOccurrences(of: "available", with: "c") + "lled") {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    anyHemAndHaw(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.colorUrl(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                anyHemAndHaw(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.taskMove(url: task.taskDescription ?? "")
        }
...
extension PassDataDelegate {
    ...
    func anyHemAndHaw(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(KeyCountLiteral.pause.rawValue)" : "\(KeyCountLiteral.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            EvenThen.observerPath(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.theoryWith(model: taskModel)
            }
    ...
    func colorUrl(url: String) {
        ...
        case .chatRecord:
            ...
            EvenThen.giftView(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.upBlock(model: taskModel)
    ...
    func taskMove(url: String) {
        ...
        let size = ShitworkStateReactiveCompatible.inscribe(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                EvenThen.observerPath(msgId: taskModel.taskId, status: "\(KeyCountLiteral.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(k_aspectTitle.suffix(4)))
            ...
            self.imageSend(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.soundAll(model: taskModel)
            ...
            case .chatRecord:
                ...
                EvenThen.observerPath(msgId: taskModel.taskId, status: "\(KeyCountLiteral.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.vocalization(model: taskModel)
    ...
    func imageSend(at path: String, withKeyId keyId: String, withTypeId typeId: NameStrideable, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(k_aspectTitle.suffix(4)))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    let unzippedData = data.statusData()
                    ...
                    if typeId == .chatRecord {
                        ...
                        EvenThen.observerPath(msgId: keyId, filePath: newPath)
                    }
