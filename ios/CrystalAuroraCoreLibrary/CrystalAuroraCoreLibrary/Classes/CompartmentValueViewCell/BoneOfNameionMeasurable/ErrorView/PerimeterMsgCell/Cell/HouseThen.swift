...
fileprivate let mMakeMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "all" :*/
fileprivate let dreamPopMessage:[Character] = ["a","l","l"]
...
class HouseThen: UserViewCell {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: RowCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mMakeMsg.reversed(), encoding: .utf8)!)
    }
...
extension HouseThen {
    ...
    override func modify(with data: SameCellData) {
        ...
        self.giftData = data as? RowCellData
        ...
        if giftData.msgModel.toUser.uid == (String(dreamPopMessage)) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.namedImageNameBundle(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.placeOf(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
