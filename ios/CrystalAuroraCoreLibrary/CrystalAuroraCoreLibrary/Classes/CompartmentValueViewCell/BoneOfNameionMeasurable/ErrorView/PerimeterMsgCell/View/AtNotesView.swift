...
fileprivate let m_dataVoiceText:[UInt8] = [0x7a,0x7f,0x7a,0x85,0x39,0x74,0x80,0x75,0x76,0x83,0x4b,0x3a,0x31,0x79,0x72,0x84,0x31,0x7f,0x80,0x85,0x31,0x73,0x76,0x76,0x7f,0x31,0x7a,0x7e,0x81,0x7d,0x76,0x7e,0x76,0x7f,0x85,0x76,0x75]

fileprivate func makeVar(equal num: UInt8) -> UInt8 {
...
/*: "0/ :*/
fileprivate let app_layerFormat:[Character] = ["0","/"]

/*: "#AAAAAA" :*/
fileprivate let app_liveFormat:String = "request self index image let#AAAA"
fileprivate let dream_makeStr:String = "interactioninteraction"

/*: "btn_chatsettings_unchoice" :*/
fileprivate let m_leadingMessage:[Character] = ["b","t","n","_","c","h","a","t"]
fileprivate let m_equalContent:String = "semakemake"
fileprivate let mHiddenUrl:String = "_uncself report half self"

/*: "btn_chatsettings_choiced" :*/
fileprivate let main_labelName:String = "to giftbtn_c"
fileprivate let userEmptyPath:[Character] = ["t","t","i","n","g","s","_","c","h","o","i","c","e","d"]

/*: "Top" :*/
fileprivate let mainAddId:[Character] = ["T","o","p"]

/*: "Save" :*/
fileprivate let appMagnitudePlayerText:String = "row model by make minSave"

/*: "Cancel" :*/
fileprivate let showDataValue:String = "Cancetext row head"
fileprivate let kCommentPath:String = "request"

/*: / :*/
fileprivate let show_slideName:[Character] = ["/"]
...
class AtNotesView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: m_dataVoiceText.map{makeVar(equal: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + k_videoName
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: kHalfText - height, width: kBlockRateValue, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.attributeAt(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.subTarget()
    ...
    private lazy var textView: YYTextView = {
        ...
        tv.textColor = UIColor.startMake()
        //: tv.font = UIFont.pingfangFont(type: .Regular, fontSize: 16)
        tv.font = UIFont.fontFile(type: .Regular, fontSize: 16)
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(app_liveFormat.suffix(5)) + dream_makeStr.replacingOccurrences(of: "interaction", with: "A")))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: RowView = {
        ...
        btn.setImage(UIImage.namedImageNameBundle(name: (String(m_leadingMessage) + m_equalContent.replacingOccurrences(of: "make", with: "t") + "ings" + String(mHiddenUrl.prefix(4)) + "hoice")), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.namedImageNameBundle(name: (String(main_labelName.suffix(5)) + "hatse" + String(userEmptyPath))), for: .selected)
        ...
        btn.setTitle((String(mainAddId)).localized, for: .normal)
        //: btn.spaceBetweenTitleAndImage = 6
        btn.spaceBetweenTitleAndImage = 6
        //: btn.addTarget(self, action: #selector(topButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(gestureDownOff), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(appMagnitudePlayerText.suffix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(frameClick), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(showDataValue.prefix(5)) + kCommentPath.replacingOccurrences(of: "request", with: "l")).localized, for: .normal)
...
extension AtNotesView: YYTextViewDelegate {
...
extension AtNotesView {
    /// 保存按钮点击事件
    //: @objc private func saveButtonClick() {
    @objc private func frameClick() {
        //: saveBtn.isEnabled = false
        saveBtn.isEnabled = false
        //: let isTop = topBtn.isSelected ? 1 : 2
        let isTop = topBtn.isSelected ? 1 : 2
        //: PushModelReactiveCompatible.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        PushModelReactiveCompatible.dismissVoiceMessage(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
            ...
            self.dismissBlock?(self.textView.text, self.topBtn.isSelected ? 1 : 2)
            //: self.dismiss()
            self.metadataDismiss()
        }
    }

    /// 置顶 / 取消置顶按钮点击事件
    //: @objc private func topButtonClick() {
    @objc private func gestureDownOff() {
    ...
    func ignitionKeyUid(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
