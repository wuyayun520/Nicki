...
/*: "title" :*/
fileprivate let userIndexName:String = "TITLE"

/*: "content" :*/
fileprivate let kBottomContent:String = "make"
fileprivate let notiTitleName:[Character] = ["o","n","t","e","n","t"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let m_closePath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img" :*/
fileprivate let k_betweenContent:String = "inameg"

/*: "jumpKey" :*/
fileprivate let dreamEqualValue:String = "cell"
fileprivate let m_indexStr:String = "success lab greetumpKey"
...
// __CLOSE_PRINT__
//
//  JumpContextReactiveCompatible.swift
//  AbroadTalking
//
...
@objcMembers public class JumpContextReactiveCompatible: ChatCellData {
    // 数据
    //: var extraJson: JSON {
    var extraJson: JSON {
    ...
    public lazy var titleAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.startMake(), .font: UIFont.fontFile(type: .Medium, fontSize: 17)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["title"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(userIndexName.lowercased())].stringValue)
    ...
    public lazy var contentAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.eraseColor(), .font: UIFont.fontFile(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["content"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(kBottomContent.replacingOccurrences(of: "make", with: "c") + String(notiTitleName))].stringValue)
        //: attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        //: return attributeStr
        return attributeStr
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: m_closePath.reversed(), encoding: .utf8)!)
    }
    ...
    override public func style(_: CGFloat) -> CGFloat {
        ...
        if extraJson[(k_betweenContent.replacingOccurrences(of: "name", with: "m"))].stringValue.count > 0 {
        ...
        if extraJson[(userIndexName.lowercased())].stringValue.count > 0 {
        ...
        if extraJson[(kBottomContent.replacingOccurrences(of: "make", with: "c") + String(notiTitleName))].stringValue.count > 0 {
            //: height += 4
            height += 4
        ...
        if extraJson[(dreamEqualValue.replacingOccurrences(of: "cell", with: "j") + String(m_indexStr.suffix(6)))].stringValue.count > 0 {
            //: jumpHeight = 35
            jumpHeight = 35
            //: height += jumpHeight
            height += jumpHeight
