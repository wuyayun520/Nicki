...
fileprivate let appTextLabCellId:[UInt8] = [0x93,0x94,0x93,0x8e,0xd2,0x99,0x95,0x9e,0x9f,0x88,0xc0,0xd3,0xda,0x92,0x9b,0x89,0xda,0x94,0x95,0x8e,0xda,0x98,0x9f,0x9f,0x94,0xda,0x93,0x97,0x8a,0x96,0x9f,0x97,0x9f,0x94,0x8e,0x9f,0x9e]

/*: "icon_chat_start" :*/
fileprivate let showNameMessage:String = "icaddn"
fileprivate let mainValueData:String = "t_startmake bottom text"

/*: "icon_chat_lock" :*/
fileprivate let user_clickEqualData:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let user_ofId:String = "request scale position tot_lock"

/*: "icon_chat_burned" :*/
fileprivate let show_messageName:String = "at icon imageicon_c"
fileprivate let noti_equalFormat:[Character] = ["h","a"]
fileprivate let user_managerId:String = "t_burate field time"

/*: "icon_chat_burned_shan" :*/
fileprivate let noti_sizePath:[Character] = ["i","c","o","n","_","c"]
fileprivate let dream_tabTitle:String = "number color tablehat_bu"

/*: "Already burned" :*/
fileprivate let showAudienceAtMsg:String = "match use to mode shareAlread"
fileprivate let noti_themeLiveIdent:String = "rneview"
...
class PetitionMsgCell: UserViewCell {
    //: var videoData: TvCellData?
    var videoData: TvCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appTextLabCellId.map{$0^250}, encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.namedImageNameBundle(name: (showNameMessage.replacingOccurrences(of: "add", with: "o") + "_cha" + String(mainValueData.prefix(7))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.namedImageNameBundle(name: (String(user_clickEqualData) + String(user_ofId.suffix(6))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: RowView = {
        ...
        imgV.setBackgroundImage(UIImage.namedImageNameBundle(name: (String(show_messageName.suffix(6)) + String(noti_equalFormat) + String(user_managerId.prefix(4)) + "rned")), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.namedImageNameBundle(name: (String(noti_sizePath) + String(dream_tabTitle.suffix(6)) + "rned_shan")), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(showAudienceAtMsg.suffix(6)) + "y bu" + noti_themeLiveIdent.replacingOccurrences(of: "view", with: "d")).localized, for: .normal)
...
extension PetitionMsgCell {
    ...
    override func modify(with data: SameCellData) {
        ...
        self.dutationLab.text = NSDate.balanceTime(totalTime: duration)
        ...
        self.finishBack(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func finishBack(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.file(image, viewLevel: 90)
                self.picImgView.image = UIImage.file(image, viewLevel: 90)
                //: } else {
            } else {
            ...
            self.picImgView.withEvent(urlStr: urlStr) { [weak self] image in
