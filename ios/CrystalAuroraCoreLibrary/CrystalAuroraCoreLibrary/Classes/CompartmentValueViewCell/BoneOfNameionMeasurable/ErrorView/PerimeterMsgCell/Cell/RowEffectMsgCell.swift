...
fileprivate let kWaitFormat:[UInt8] = [0xc2,0xc7,0xc2,0xcd,0x81,0xbc,0xc8,0xbd,0xbe,0xcb,0x93,0x82,0x79,0xc1,0xba,0xcc,0x79,0xc7,0xc8,0xcd,0x79,0xbb,0xbe,0xbe,0xc7,0x79,0xc2,0xc6,0xc9,0xc5,0xbe,0xc6,0xbe,0xc7,0xcd,0xbe,0xbd]

fileprivate func viewCenter(scale num: UInt8) -> UInt8 {
...
/*: "icon_chat_miss" :*/
fileprivate let dreamSearchedMessage:[Character] = ["i","c","o","n","_","c","h","a","t","_","m"]
fileprivate let mMakePath:String = "values"

/*: "icon_chat_yes" :*/
fileprivate let app_customRequestMessage:String = "keycon"
fileprivate let user_removeKey:String = "_yesuser equal"
...
class RowEffectMsgCell: UserViewCell {
//    var transBlock: (() -> Void)?

    //: var textData: SizeCellData?
    var textData: SizeCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kWaitFormat.map{viewCenter(scale: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var quoteView: VideoItemButton = {
        //: let quoteView = TalkingMsgQuoteView()
        let quoteView = VideoItemButton()
        //: quoteView.isHidden = false
        quoteView.isHidden = false
        //: quoteView.addTarget(self, action: #selector(quoteBtnClick), for: .touchUpInside)
        quoteView.addTarget(self, action: #selector(popularRelease), for: .touchUpInside)
        //: return quoteView
        return quoteView
        //: }()
    }()
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != MacroReactiveCompatible.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.scrapingMsg() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.namedImageNameBundle(name: (String(dreamSearchedMessage) + mMakePath.replacingOccurrences(of: "value", with: "is")))
                ...
                self.transStatusImagV.image = UIImage.namedImageNameBundle(name: (app_customRequestMessage.replacingOccurrences(of: "key", with: "i") + "_chat" + String(user_removeKey.prefix(4))))
            }
    ...
    func pressMake() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(voiceTap(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == MacroReactiveCompatible.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == MacroReactiveCompatible.preemptSystemXiao())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            dogTagTable()
...
//            }
    ...
    func dogTagTable() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(appTap(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func voiceTap(tap _: UITapGestureRecognizer?) {
    ...
    @objc func appTap(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func popularRelease() {
        //: self.delegate?.onReplyMessage(cell: self)
        self.delegate?.toDeadline(cell: self)
    }
}
