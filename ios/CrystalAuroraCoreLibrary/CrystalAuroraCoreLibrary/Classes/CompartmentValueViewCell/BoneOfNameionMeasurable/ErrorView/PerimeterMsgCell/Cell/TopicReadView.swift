...
fileprivate let k_appUrl:[UInt8] = [0x4b,0x50,0x4b,0x56,0xa,0x45,0x51,0x46,0x47,0x54,0x1c,0xb,0x2,0x4a,0x43,0x55,0x2,0x50,0x51,0x56,0x2,0x44,0x47,0x47,0x50,0x2,0x4b,0x4f,0x52,0x4e,0x47,0x4f,0x47,0x50,0x56,0x47,0x46]

fileprivate func tempDataPermission(first num: UInt8) -> UInt8 {
...
fileprivate let k_deadlineIdent:String = "let title eventicon_ch"
fileprivate let notiContainerMessage:String = "at_traview allow for"
fileprivate let kStartVideoIdent:String = "reply image_nor"

/*: "icon_chat_translate_pre" :*/
fileprivate let kItemUrl:[Character] = ["i","c","o","n","_","c","h"]
fileprivate let mColorId:[Character] = ["a","t","_","t","r","a","n","s","l","a"]
fileprivate let userTimeData:String = "te_prelocal index main"

/*: "icon_chat_miss" :*/
fileprivate let main_statusPath:[Character] = ["i","c","o","n","_","c","h"]
fileprivate let mUpValue:[Character] = ["a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let appTillSendMakeName:[Character] = ["i","c","o","n","_","c"]
fileprivate let app_fromMsg:String = "hat_yesmake else a text"
...
class TopicReadView: UserViewCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: ProduceReactiveCompatible?
    var textData: ProduceReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_appUrl.map{tempDataPermission(first: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.namedImageNameBundle(name: (String(k_deadlineIdent.suffix(7)) + String(notiContainerMessage.prefix(6)) + "nslate" + String(kStartVideoIdent.suffix(4)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.namedImageNameBundle(name: (String(kItemUrl) + String(mColorId) + String(userTimeData.prefix(6)))), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(playerWith), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension TopicReadView {
    ...
    override func modify(with data: SameCellData) {
        ...
        if imgPreview.count > 0, textData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
            ...
            giftImagV.placeOf(urlStr: imgPreview)
        }
        ...
        //: let maxWidth = MacroReactiveCompatible.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = MacroReactiveCompatible.totalimateLastCurrent() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.namedImageNameBundle(name: (String(main_statusPath) + String(mUpValue)))
            ...
            self.transStatusImagV.image = UIImage.namedImageNameBundle(name: (String(appTillSendMakeName) + String(app_fromMsg.prefix(7))))
        }
    ...
    func clause() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(modelTitlePlace(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func modelTitlePlace(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
