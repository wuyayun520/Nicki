...
fileprivate let notiCellText:[UInt8] = [0x25,0x2a,0x25,0x30,0xe4,0x1f,0x2b,0x20,0x21,0x2e,0xf6,0xe5,0xdc,0x24,0x1d,0x2f,0xdc,0x2a,0x2b,0x30,0xdc,0x1e,0x21,0x21,0x2a,0xdc,0x25,0x29,0x2c,0x28,0x21,0x29,0x21,0x2a,0x30,0x21,0x20]

fileprivate func failureInfoVideo(model num: UInt8) -> UInt8 {
...
/*: "CCCCCC" :*/
fileprivate let mCellPath:String = "totototototo"

/*: "icon_lounge" :*/
fileprivate let notiRequestContent:String = "icdata"

/*: "#FFEE4B" :*/
fileprivate let notiGiftValue:[Character] = ["#","F","F","E","E","4"]
fileprivate let appMomentIdent:[Character] = ["B"]

/*: "View read receipts?" :*/
fileprivate let userProduceIdent:String = "item color name hiddenView "
fileprivate let mainLogUrl:String = "model var view will stackread "

/*: "#FBE3FE" :*/
fileprivate let showScreenData:String = "guard style add log#FBE3FE"

/*: "#D8DDFF" :*/
fileprivate let notiTargetMessage:String = "type let#D8DD"
fileprivate let m_clearValue:[Character] = ["F","F"]
...
// __CLOSE_PRINT__
//
//  TerritorialDivisionReactiveCompatible.swift
//  AbroadTalking
//
...
class TerritorialDivisionReactiveCompatible: UserViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiCellText.map{failureInfoVideo(model: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var messageLB: YYLabel = {
        ...
        label.textColor = UIColor(hex: (mCellPath.replacingOccurrences(of: "to", with: "C")))
    ...
    private lazy var vipReadReceiptBtn: RowView = {
        //: let btn = TalkingButton()
        let btn = RowView()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_lounge"), for: .normal)
        btn.setImage(UIImage.namedImageNameBundle(name: (notiRequestContent.replacingOccurrences(of: "data", with: "on") + "_lounge")), for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        btn.titleLabel?.font = UIFont.fontFile(type: .Regular, fontSize: 15)
        //: btn.setTitleColor(UIColor(hex: "#FFEE4B"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(notiGiftValue) + String(appMomentIdent))), for: .normal)
        //: btn.setTitle("View read receipts?".localized, for: .normal)
        btn.setTitle((String(userProduceIdent.suffix(5)) + String(mainLogUrl.suffix(5)) + "receipts?").localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(listEffect), for: .touchUpInside)
        ...
        let gColor = [UIColor(hex: (String(showScreenData.suffix(7))))!.withAlphaComponent(0.6).cgColor,
                      //: UIColor.init(hex: "#D8DDFF")!.withAlphaComponent(0.6).cgColor]
                      UIColor(hex: (String(notiTargetMessage.suffix(5)) + String(m_clearValue)))!.withAlphaComponent(0.6).cgColor]
        //: btn.setBackgroundImage(UIImage.imageGradientColor(colors: gColor, size: CGSize(width: btnWidth, height: btnHeight)), for: .normal)
        btn.setBackgroundImage(UIImage.versionSize(colors: gColor, size: CGSize(width: btnWidth, height: btnHeight)), for: .normal)
...
extension TerritorialDivisionReactiveCompatible {
    /// vip订阅事件
    //: @objc private func vipReadReceiptClick() {
    @objc private func listEffect() {
...
extension TerritorialDivisionReactiveCompatible {
    ...
    override func modify(with data: SameCellData) {
        ...
        guard let tipCellData = data as? TopicDetailCellData else { return }
        ...
        if dataLayout != nil {
            ...
            //: let layout = YYTextLayout.init(containerSize: CGSize.init(width: MacroReactiveCompatible.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), text: tipCellData.attributedString)
            let layout = YYTextLayout(containerSize: CGSize(width: MacroReactiveCompatible.totalimateLastCurrent() - 50, height: CGFloat(MAXFLOAT)), text: tipCellData.attributedString)
        ...
        if tipCellData.msgModel.tips.content == (String(userProduceIdent.suffix(5)) + String(mainLogUrl.suffix(5)) + "receipts?").localized {
    ...
    override func updateConstraints() {
        ...
        self.container.snp.remakeConstraints { make in
            ...
            make.width.lessThanOrEqualTo(kBlockRateValue - 40)
            //: make.top.equalTo(10)
            make.top.equalTo(10)
    ...
    func press() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(itemGift(tap:)))
    ...
    @objc func itemGift(tap _: UITapGestureRecognizer) {
        //: self.delegate?.onSelectMessage(cell: self)
        self.delegate?.assistance(cell: self)
    }
}
