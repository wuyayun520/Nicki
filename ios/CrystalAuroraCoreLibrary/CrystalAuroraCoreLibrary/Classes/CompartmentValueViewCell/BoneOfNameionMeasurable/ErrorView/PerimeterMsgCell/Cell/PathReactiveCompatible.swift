...
fileprivate let main_sizeValue:[UInt8] = [0xcb,0xd0,0xcb,0xd6,0x8a,0xc5,0xd1,0xc6,0xc7,0xd4,0x9c,0x8b,0x82,0xca,0xc3,0xd5,0x82,0xd0,0xd1,0xd6,0x82,0xc4,0xc7,0xc7,0xd0,0x82,0xcb,0xcf,0xd2,0xce,0xc7,0xcf,0xc7,0xd0,0xd6,0xc7,0xc6]

fileprivate func attractiveFeature(handle num: UInt8) -> UInt8 {
...
/*: "icon_chat_lock" :*/
fileprivate let appTableData:[Character] = ["i","c","o","n","_","c","h","a","t","_","l","o"]
fileprivate let mCanContent:[Character] = ["c","k"]

/*: "icon_chat_burned" :*/
fileprivate let k_actionValue:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let mSizeStr:String = "manager seat app throught_bu"

/*: "icon_chat_burned_shan" :*/
fileprivate let show_dataTitle:[Character] = ["i","c","o","n","_","c","h","a","t","_","b","u","r","n","e","d","_","s","h"]
fileprivate let notiGiftDownModelContent:String = "mode"

/*: "Already burned" :*/
fileprivate let user_selectName:String = "length not number device argumentAlread"
fileprivate let dream_countPath:String = "block"
...
class PathReactiveCompatible: UserViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_sizeValue.map{attractiveFeature(handle: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.namedImageNameBundle(name: (String(appTableData) + String(mCanContent)))
    ...
    lazy var burnedImagV: RowView = {
        ...
        imgV.setBackgroundImage(UIImage.namedImageNameBundle(name: (String(k_actionValue) + String(mSizeStr.suffix(4)) + "rned")), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.namedImageNameBundle(name: (String(show_dataTitle) + notiGiftDownModelContent.replacingOccurrences(of: "mode", with: "an"))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(user_selectName.suffix(6)) + "y burn" + dream_countPath.replacingOccurrences(of: "block", with: "ed")).localized, for: .normal)
...
extension PathReactiveCompatible {
    ...
    override func modify(with data: SameCellData) {
        ...
        //: guard let imageCellData = data as? WriteCellData else { return }
        guard let imageCellData = data as? WriteCellData else { return }
        ...
        let intimatePhoto: RestrictionModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            useFrom(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            useFrom(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: WriteCellData, urlStr: String?, isMosaic: Bool) {
    func useFrom(data: WriteCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.file(image, viewLevel: 90)
                        self.photoView.image = UIImage.file(image, viewLevel: 90)
                        //: } else {
                    } else {
            ...
            listShow(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.placeOf(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func listShow(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.withEvent(urlStr: urlStr) { [weak self] image in
