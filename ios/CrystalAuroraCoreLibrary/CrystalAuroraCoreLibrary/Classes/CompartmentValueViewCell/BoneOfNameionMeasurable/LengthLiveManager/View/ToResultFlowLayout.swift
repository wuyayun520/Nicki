...
fileprivate let main_viewEqualPath:[UInt8] = [0x81,0x86,0x81,0x9c,0xc0,0x8b,0x87,0x8c,0x8d,0x9a,0xd2,0xc1,0xc8,0x80,0x89,0x9b,0xc8,0x86,0x87,0x9c,0xc8,0x8a,0x8d,0x8d,0x86,0xc8,0x81,0x85,0x98,0x84,0x8d,0x85,0x8d,0x86,0x9c,0x8d,0x8c]

/*: "no data available" :*/
fileprivate let notiWithValue:String = "no daself reply make content effect"
fileprivate let dream_halfUrl:String = "ta avaadd error center"

/*: "list" :*/
fileprivate let showFileTopToKey:String = "lisdata"
...
class ToResultFlowLayout: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_viewEqualPath.map{$0^232}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(islandHop), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collect.register(WriteSpaceItemCell.self, forCellWithReuseIdentifier: WriteSpaceItemCell.className())
        //: return collect
        return collect
        //: }()
    }()
    ...
    private lazy var emptyView: UILabel = {
        ...
        lab.text = (String(notiWithValue.prefix(5)) + String(dream_halfUrl.prefix(6)) + "ilable").localized
        //: lab.textColor = .white
        lab.textColor = .white
    ...
    private lazy var dataArr: [SpaceHandyJSON] = //: return [SpaceHandyJSON]()
        .init()
    //: }()
}
...
extension ToResultFlowLayout {
    ...
    private func chemLabList() {
        ...
        InfoDetailRequestTool.modeCompletion(category: 2) { [weak self] _, result, _ in
            ...
            json[(showFileTopToKey.replacingOccurrences(of: "data", with: "t"))].arrayObject?.forEach { dict in
                //: if let model = TalkingGameListModel.deserialize(from: dict as? Dictionary) {
                if let model = SpaceHandyJSON.deserialize(from: dict as? Dictionary) {
...
extension ToResultFlowLayout {
    ...
    private func high() -> CGFloat {
        ...
        return lineNum * k_levelSucceedName + 24.0 + k_videoName
    }
}
...
extension ToResultFlowLayout: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: WriteSpaceItemCell.className(), for: indexPath) as! WriteSpaceItemCell
        //: cell.refreshView(model: dataArr[indexPath.row])
        cell.close(model: dataArr[indexPath.row])
        //: return cell
        return cell
    }
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        return CGSize(width: (kBlockRateValue - 30) / 4, height: k_levelSucceedName)
    }
}
...
extension ToResultFlowLayout {
    ...
    func viewTitle(from: FromType) {
        //: switch from {
        switch from {
        //: case .LiveRoom:
        case .LiveRoom:
            //: dataArr = TalkingLiveManager.shared().liveRoomModel.gameList
            dataArr = LengthLiveManager.loadShared().liveRoomModel.gameList
            //: showAnimation()
            dosage()
    ...
    private func dosage() {
        ...
        self.contentView.frame = CGRect(x: 0, y: kHalfText, width: kBlockRateValue, height: high())
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y -= self.contentView.height
            self.contentView.frame.origin.y -= self.contentView.height
...
extension ToResultFlowLayout {
    ...
    private func each() {
        ...
        contentView.frame = CGRect(x: 0, y: kHalfText, width: kBlockRateValue, height: high())
