...
fileprivate let kExamineKey:[UInt8] = [0xe9,0xee,0xe9,0xf4,0xa8,0xe3,0xef,0xe4,0xe5,0xf2,0xba,0xa9,0xa0,0xe8,0xe1,0xf3,0xa0,0xee,0xef,0xf4,0xa0,0xe2,0xe5,0xe5,0xee,0xa0,0xe9,0xed,0xf0,0xec,0xe5,0xed,0xe5,0xee,0xf4,0xe5,0xe4]

private func arrayComment(video num: UInt8) -> UInt8 {
    return num ^ 128
}
...
@objc protocol NumberObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func upSearch()

    //: @objc optional func endScroll()
    @objc optional func currentEnd()
}

//: class TalkingDanmuMsgListTableView: UIView {
class AtDanmuViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kExamineKey.map{arrayComment(video: $0)}, encoding: .utf8)!)
    }
...
extension AtDanmuViewDelegate {
    ...
    func atTheSameTime() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(micPlayer), userInfo: nil, repeats: true)
    }
...
extension AtDanmuViewDelegate {
    ...
    func preposeModel(msgModel: EqualDwellHandyJSON?) {
    ...
    func tableWithBottom() {
        ...
        if AllHeight > kButtonLineName {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < kButtonLineName {
                ...
                self.msgTableView.height = kButtonLineName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kButtonLineName - AllHeight
        ...
        editRow(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func editRow(animated: Bool) {
...
extension AtDanmuViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func courseOfStudy(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.currentEnd?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(k_sessionRecordAgentName)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ModelBassCell.indexEqual(tableView: tableView, msg: msgModel, indexPath: indexPath)
