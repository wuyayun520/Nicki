...
fileprivate let mCenterMessage:[UInt8] = [0x68,0x6f,0x68,0x75,0x29,0x62,0x6e,0x65,0x64,0x73,0x3b,0x28,0x21,0x69,0x60,0x72,0x21,0x6f,0x6e,0x75,0x21,0x63,0x64,0x64,0x6f,0x21,0x68,0x6c,0x71,0x6d,0x64,0x6c,0x64,0x6f,0x75,0x64,0x65]

private func textTarget(perspective num: UInt8) -> UInt8 {
    return num ^ 1
}

/*: "Select mute duration" :*/
fileprivate let userPresentationMessage:[Character] = ["S","e","l","e","c","t"," ","m","u","t","e"," ","d"]
fileprivate let main_tapSendPath:String = "uratableion"

/*: "Cirform" :*/
fileprivate let main_modelValueId:String = "type tool number mode selfCirform"

/*: "Mute success" :*/
fileprivate let dreamTitleTextIdent:[Character] = ["M","u","t","e"," "]
fileprivate let showMessageIdent:[Character] = ["s"]
fileprivate let notiEffectMessage:[Character] = ["u","c","c","e","s","s"]

/*: "day" :*/
fileprivate let mainDataMsg:String = "dacurrent"

/*: "days" :*/
fileprivate let dream_bottomValue:[Character] = ["d","a","y","s"]

/*:  " :*/
fileprivate let main_itemSizeMessage:String = " "
...
// __CLOSE_PRINT__
//
//  AddressReactiveCompatible.swift
...
class AddressReactiveCompatible: UIView {
    ...
    var viewType: HalogenViewType = .live
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mCenterMessage.map{textTarget(perspective: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(currentBegin), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.font = UIFont.dismissProgress(fontSize: 18)
        //: lb.textColor = UIColor.appTitleColor()
        lb.textColor = UIColor.startMake()
        //: lb.textAlignment = .center
        lb.textAlignment = .center
        //: lb.text = "Select mute duration".localized
        lb.text = (String(userPresentationMessage) + main_tapSendPath.replacingOccurrences(of: "table", with: "t")).localized
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
        //: return lb
        return lb
    ...
    lazy var commitBtn: UIButton = {
        ...
        btn.setTitle((String(main_modelValueId.suffix(7))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(showToImage), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension AddressReactiveCompatible {
    /// 消除视图按钮
    //: @objc private func bgDismissButtonClick() {
    @objc private func currentBegin() {
        //: dismiss()
        viaBackgroundTitle()
    }
    ...
    func birdSEyeView() {
        ...
        self.contentView.frame = CGRect(x: (kBlockRateValue - contentV_Size.width) / 2, y: kHalfText, width: contentV_Size.width, height: contentV_Size.height)

        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = (ScreenHeight-self.contentV_Size.height)/2
            self.contentView.frame.origin.y = (kHalfText - self.contentV_Size.height) / 2
        }
    }

    /// 隐藏视图
    //: func dismiss() {
    func viaBackgroundTitle() {
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = ScreenHeight
            self.contentView.frame.origin.y = kHalfText
...
extension AddressReactiveCompatible {
    ...
    @objc private func region(btn: UIButton) {
    ...
    @objc private func showToImage() {
        ...
        if self.viewType == .live {
            ...
            OfToReactiveCompatible().targetDownCompletion(targetUid: targetUid, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                AnagrammatizeView.implant()
                //: if succeed {
                if succeed {
                    //: self.func__showStatusBarSuccessMsg(showMsg: "Mute success".localized)
                    self.logArea(showMsg: (String(dreamTitleTextIdent) + String(showMessageIdent) + String(notiEffectMessage)).localized)
                }
                //: self.dismiss()
                self.viaBackgroundTitle()
            }
            //: }else{
        } else {
            ...
            LineReqTool.clout(targetUid: targetUid, roomID: SizeEventHandler.frameShared().partyModel.roomId, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                AnagrammatizeView.implant()
                //: if succeed {
                if succeed {
...
extension AddressReactiveCompatible {
    ...
    private func beforeTo() {
        ...
        for i in 0 ..< 3 {
            ...
            btn.note(color: UIColor.subject(), forState: .selected)
            ...
            let dayStr = (title == "1") ? (mainDataMsg.replacingOccurrences(of: "current", with: "y")).localized : (String(dream_bottomValue)).localized
