...
/*: "#ffffff" :*/
fileprivate let showToData:String = "height image return normal normal#ffffff"

/*: "/dist/loungePlus/index.html" :*/
fileprivate let noti_recordPhonePathName:String = "hide object/dist"
fileprivate let notiFlushIdent:String = "ngePltop false target after"
fileprivate let showViewName:String = "let truedex."

/*: "https://" :*/
fileprivate let kJoinText:[Character] = ["h","t","t","p","s",":"]
fileprivate let m_descriptionIdent:String = "makemake"

/*: "http://" :*/
fileprivate let mainSeemTitle:String = "to center view superhttp://"
...
class ProductThen: UIView {
    ...
    let Broadcast_TextColor = (String(showToData.suffix(7)))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .dismissProgress(fontSize: 12)
    ...
    convenience init(model: WorkValuePtolemaicSystemResourceTransformable?) {
        //: self.init()
        self.init()
    ...
    private func theoreticalAccountBlock() {
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            var mutableAtt = parse.aforesaid(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func ending(withUrl url: String) {
        ...
        self.backgroundImageView.giveAndTake(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.showQuality(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: WorkValuePtolemaicSystemResourceTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            theoreticalAccountBlock()
        }
    }
    ...
    var totalTime: CGFloat {
        ...
        return (kBlockRateValue + widthValue) * Broadcast_SpendTime / kBlockRateValue
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / kBlockRateValue
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / kBlockRateValue
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(createerByWith(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension ProductThen {
    ...
    @objc func createerByWith(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: SucceedInfoReactiveCompatible.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            SucceedInfoReactiveCompatible.share.duringState(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: SucceedInfoReactiveCompatible.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                SucceedInfoReactiveCompatible.share.endAdd(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(noti_recordPhonePathName.suffix(5)) + "/lou" + String(notiFlushIdent.prefix(5)) + "us/in" + String(showViewName.suffix(4)) + "html")) {
                    //: SucceedInfoReactiveCompatible.share.func__pushToSubscribePageWebVC()
                    SucceedInfoReactiveCompatible.share.dataVc()
                    //: return
                    return
                }
                //: let httpsUrl = ModeStageReactiveCompatible.share.appConfigMode.urlH5Domain+url
                let httpsUrl = ModeStageReactiveCompatible.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(kJoinText) + m_descriptionIdent.replacingOccurrences(of: "make", with: "/")), with: (String(mainSeemTitle.suffix(7))))
                //: SucceedInfoReactiveCompatible.share.func__pushToWebVC(urlStr: httpUrl)
                SucceedInfoReactiveCompatible.share.untilEqual(urlStr: httpUrl)
            }
