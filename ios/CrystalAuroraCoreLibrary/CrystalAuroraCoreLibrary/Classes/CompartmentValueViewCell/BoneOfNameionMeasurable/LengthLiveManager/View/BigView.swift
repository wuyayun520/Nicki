...
/*: "99+" :*/
fileprivate let k_actionMessage:[Character] = ["9","9","+"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let show_earnMeStr:[UInt8] = [0x7d,0x7a,0x7d,0x60,0x3c,0x77,0x7b,0x70,0x71,0x66,0x2e,0x3d,0x34,0x7c,0x75,0x67,0x34,0x7a,0x7b,0x60,0x34,0x76,0x71,0x71,0x7a,0x34,0x7d,0x79,0x64,0x78,0x71,0x79,0x71,0x7a,0x60,0x71,0x70]

/*: "#2DF2FF" :*/
fileprivate let kPositionData:String = "#2DF2FFreturn type"

/*: "Live" :*/
fileprivate let showCropData:String = "size image var video makeLive"

/*: "img_home_shadow_icon" :*/
fileprivate let m_managerModeMsg:String = "observe share equal myimg_ho"
fileprivate let app_heartPartyStatusData:String = "w_iconup self"

/*: "get json error" :*/
fileprivate let dreamSucceedName:String = "count return self varget jso"
fileprivate let show_messageTitle:String = "normal var less anyn error"

/*: "#FF2348" :*/
fileprivate let dream_makeValue:[Character] = ["#","F","F","2","3"]
fileprivate let mainToText:[Character] = ["4","8"]

/*: "#35C759" :*/
fileprivate let user_equivalentKey:String = "#35C759manager equal touch"

/*: "mfBean" :*/
fileprivate let show_netValue:String = "mfBeantitle title equal"

/*: "%.1f" :*/
fileprivate let dream_toEndIdent:String = "false finish import start%.1f"

/*: "points\n≈ $" :*/
fileprivate let mainTitleKey:String = "areaoints"
fileprivate let dream_whiteAddData:String = "\n≈ $"

/*:   :*/
fileprivate let user_layerTitle:[Character] = [" "]
...
public var k_buttonData: Int {
    //: return ModeStageReactiveCompatible.share.appConfigMode.countdown1 + ModeStageReactiveCompatible.share.appConfigMode.countdown2
    return ModeStageReactiveCompatible.share.appConfigMode.countdown1 + ModeStageReactiveCompatible.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class BigView: ModelMiniView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= ModeStageReactiveCompatible.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(k_buttonData - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                totalervalView()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(ModeStageReactiveCompatible.share.loginUserMode.mf_bean) > 0 {
        if Double(ModeStageReactiveCompatible.share.loginUserMode.mf_bean) > 0 {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_earnMeStr.map{$0^20}, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: RowView = {
        ...
        let image = UIImage.sumernalRepresentation(color: UIColor(hex: (String(kPositionData.prefix(7))))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.scaleValue(), for: .normal)
        img.setImage(image.scaleValue(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(showCropData.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.namedImageNameBundle(name: (String(m_managerModeMsg.suffix(6)) + "me_shado" + String(app_heartPartyStatusData.prefix(6))))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(dreamSucceedName.suffix(7)) + String(show_messageTitle.suffix(7))))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.font = UIFont.dismissProgress(fontSize: 20)
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.textColor = UIColor(hex: "#FF2348")
        lab.textColor = UIColor(hex: (String(dream_makeValue) + String(mainToText)))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(user_equivalentKey.prefix(7))))
        //: lab.font = UIFont.pingfangFont(type: .Medium, fontSize: 11)
        lab.font = UIFont.fontFile(type: .Medium, fontSize: 11)
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.textAlignment = .center
        lab.textAlignment = .center
...
extension BigView {
    ...
    static func declaim() -> BigView {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = BigView(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = EffectSocketDelegate.shared.liveMiniPosition
    ...
    func totalervalView() {
        ...
        liveStatusImgV.isHidden = !LengthLiveManager.loadShared().isLive
...
extension BigView {
    ...
    private func punctuate() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(valueSize(notification:)),
    ...
    @objc private func valueSize(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(show_netValue.prefix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        keyBean(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func keyBean(mfBean: Double) {
        //: let nowBean = ModeStageReactiveCompatible.share.loginUserMode.mf_bean + mfBean
        let nowBean = ModeStageReactiveCompatible.share.loginUserMode.mf_bean + mfBean
        //: ModeStageReactiveCompatible.share.loginUserMode.mf_bean = nowBean
        ModeStageReactiveCompatible.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((mainTitleKey.replacingOccurrences(of: "area", with: "p") + dream_whiteAddData).localized) \(divisor)"
