...
fileprivate let notiKeyValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Online Users" :*/
fileprivate let mainLowerMakeKey:[Character] = ["O","n","l","i","n","e"," ","U","s","e"]
fileprivate let noti_labelStr:String = "rgift"
...
class TaskViewDelegate: UIView {
    ...
    override init(frame: CGRect) {
        ...
        fromLockSwaddlingClothesObject()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: notiKeyValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(mainLowerMakeKey) + noti_labelStr.replacingOccurrences(of: "gift", with: "s")).localized
        //: return lb
        return lb
        //: }()
    }()
...
extension TaskViewDelegate {
    ...
    private func titleEnable() {
        //: TalkingAudienceManager().req_liveRoomOnlineList(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
        OfToReactiveCompatible().tillEqual(chatGroupId: self.chatGroupId) { succeed, result, _ in
            ...
            if succeed {
                ...
                if let datas = Array<LabelMeasurable>.deserialize(from: array as? Array) {
...
extension TaskViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.factor(model: self.DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension TaskViewDelegate {
    ...
    private func fromLockSwaddlingClothesObject() {
