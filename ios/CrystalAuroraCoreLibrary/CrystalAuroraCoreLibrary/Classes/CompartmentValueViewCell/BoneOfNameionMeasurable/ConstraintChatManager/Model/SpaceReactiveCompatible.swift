...
/*: ：" :*/
fileprivate let show_mostContent:String = "show"

/*: "#BAFBFF" :*/
fileprivate let mainSizeMsg:[Character] = ["#","B","A","F","B","F","F"]
...
class SpaceReactiveCompatible: NSObject {
    ...
    var model = VideoDetailTransformable()

    //: func caculateMsgHeight(model: TalkingVideoCallDanmuModel) -> TalkingVideoCallDanmuModel {
    func viewFor(model: VideoDetailTransformable) -> VideoDetailTransformable {
        //: self.model = model
        self.model = model
        /// 普通文本消息
    ...
    func cohabit(model: VideoDetailTransformable) -> VideoDetailTransformable {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func messageMini() {
        ...
        let name = sumeraction(text: firstStr, font: UIFont.dismissProgress(fontSize: 14), color: UIColor(hex: (String(mainSizeMsg)))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = sumeraction(text: self.model.content, font: UIFont.dismissProgress(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        viewSend(attribText: self.model.msgAttribText)
    }
    ...
    func output() {
        ...
        let content = sumeraction(text: self.model.transContent, font: UIFont.dismissProgress(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func wiseDismiss() -> NSMutableParagraphStyle {
    ...
    func sumeraction(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = wiseDismiss()
        //: return attribute
        return attribute
    }
    ...
    func viewSend(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = kViewTitle - 36
