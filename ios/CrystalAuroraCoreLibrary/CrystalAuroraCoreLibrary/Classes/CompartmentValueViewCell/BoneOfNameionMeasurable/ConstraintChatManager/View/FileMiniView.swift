...
fileprivate let kStatusStr:[UInt8] = [0x4,0x3,0x4,0x19,0x45,0xe,0x2,0x9,0x8,0x1f,0x57,0x44,0x4d,0x5,0xc,0x1e,0x4d,0x3,0x2,0x19,0x4d,0xf,0x8,0x8,0x3,0x4d,0x4,0x0,0x1d,0x1,0x8,0x0,0x8,0x3,0x19,0x8,0x9]

/*: "%02ld:%02ld:%02ld" :*/
fileprivate let mQuoteStr:String = "%02ldself bar end start"
fileprivate let mainClickStr:String = "ld:%play lab self var view"
fileprivate let dreamKeyMatchData:String = "02ldtype theme"

/*: "icon_mini_voice_end" :*/
fileprivate let dream_withMakeDisappearData:[UInt8] = [0x64,0x6e,0x65,0x5f,0x65,0x63,0x69,0x6f,0x76,0x5f,0x69,0x6e,0x69,0x6d,0x5f,0x6e,0x6f,0x63,0x69]

/*: "icon_mini_voice_calling" :*/
fileprivate let userImageTitle:[Character] = ["i","c","o","n","_","m","i","n","i","_","v","o","i"]
fileprivate let mPanName:[Character] = ["c","e","_","c","a"]
fileprivate let user_valuePath:[Character] = ["l","l","i","n","g"]

/*: "#F95151" :*/
fileprivate let showSexIdent:String = "block"
fileprivate let dreamPopStr:String = "image5151"

/*: "#34C759" :*/
fileprivate let mItemTitle:String = "center return background extension user#34C759"

/*: "End of call" :*/
fileprivate let k_aId:[Character] = ["E","n","d"," ","o","f"," ","c","a"]
fileprivate let mShareStr:String = "manager"
...
class FileMiniView: ModelMiniView {
    // 小窗口通话状态
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStatusStr.map{$0^109}, encoding: .utf8)!)
    }
...
extension FileMiniView {
    ...
    static func viewMessage() -> FileMiniView {
    ...
    func messageModify(_ time: Double) {
...
extension FileMiniView {
    ...
    private func subFromAbout() {
        ...
        nameBegin(status: .calling)
    }

    /// 刷新视图
    /// - Parameter status: 通话状态
    //: private func refreshVoiceView(status: MiniWindowStatus) {
    private func nameBegin(status: VideoWindowStatus) {
        //: self.currStatus = status
        self.currStatus = status
        //: let iconStr = (status == .end) ? "icon_mini_voice_end":"icon_mini_voice_calling"
        let iconStr = (status == .end) ? String(bytes: dream_withMakeDisappearData.reversed(), encoding: .utf8)! : (String(userImageTitle) + String(mPanName) + String(user_valuePath))
        //: self.statusIcon.image = UIImage.BundleImageNamed(name: iconStr)
        self.statusIcon.image = UIImage.namedImageNameBundle(name: iconStr)
        //: self.timeLab.textColor = (status == .end) ? UIColor(hex: "#F95151"):UIColor(hex: "#34C759")
        self.timeLab.textColor = (status == .end) ? UIColor(hex: (showSexIdent.replacingOccurrences(of: "block", with: "#") + dreamPopStr.replacingOccurrences(of: "image", with: "F9"))) : UIColor(hex: (String(mItemTitle.suffix(7))))
        //: if status == .end {
        if status == .end {
            //: self.timeLab.text = "End of call".localized
            self.timeLab.text = (String(k_aId) + mShareStr.replacingOccurrences(of: "manager", with: "ll")).localized
        }
    }
}
