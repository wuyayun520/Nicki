...
fileprivate let m_valueKey:[UInt8] = [0xc0,0xc7,0xc0,0xdd,0x81,0xca,0xc6,0xcd,0xcc,0xdb,0x93,0x80,0x89,0xc1,0xc8,0xda,0x89,0xc7,0xc6,0xdd,0x89,0xcb,0xcc,0xcc,0xc7,0x89,0xc0,0xc4,0xd9,0xc5,0xcc,0xc4,0xcc,0xc7,0xdd,0xcc,0xcd]

/*: "#FFD13A" :*/
fileprivate let app_tableManagerContent:String = "cell view if in object#FFD13A"

/*: "gift_gb_umnad_1" :*/
fileprivate let appDoingFormat:String = "string self hidden handlegift_"
fileprivate let user_colorName:String = "nad_1discount text"

/*: "icon_circle_pic_default" :*/
fileprivate let mainTranslateFormat:[Character] = ["i","c","o","n","_","c","i","r","c","l","e","_","p","i","c","_","d","e","f","a","u","l","t"]

/*: "Open lucky blind-box" :*/
fileprivate let notiMainId:[Character] = ["O","p","e","n"," ","l","u","c"]
fileprivate let user_pingName:String = "ky blcurrent manager publish input"

/*: "Sent " :*/
fileprivate let appNameValue:String = "file errorSent "

/*: "PingFangSC-Semibold" :*/
fileprivate let userStateName:String = "print makePingF"
fileprivate let noti_propertyMsg:String = "to title let end letC-Se"
fileprivate let mStyleStr:String = "MIBOLD"

/*: "%d" :*/
fileprivate let kBackTitle:String = "%dcount share if"

/*: "Bombard" :*/
fileprivate let dream_norId:[Character] = ["B","o","m","b","a","r","d"]
...
class MenuListThen: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = BoneOfNameionMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: m_valueKey.map{$0^169}, encoding: .utf8)!)
    }
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(app_tableManagerContent.suffix(7))))
    ...
    private lazy var pluralTip: PluralReactiveCompatible = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = PluralReactiveCompatible()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: PluralReactiveCompatible = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = PluralReactiveCompatible()
...
extension MenuListThen {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func pointModel(model: BoneOfNameionMeasurable) {
        ...
        self.bgImage.image = UIImage.namedImageNameBundle(name: (String(appDoingFormat.suffix(5)) + "gb_um" + String(user_colorName.prefix(5))))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.namedImageNameBundle(name: (String(mainTranslateFormat)))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == ModelViewEquatable.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(notiMainId) + String(user_pingName.prefix(5)) + "ind-box").localized
            //: }else {
        } else {
        ...
        self.avatarImage.giveAndTake(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.giveAndTake(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(appNameValue.suffix(5))).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(userStateName.suffix(5)) + "angS" + String(noti_propertyMsg.suffix(4)) + mStyleStr.lowercased()), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = pathBurnCrop(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(userStateName.suffix(5)) + "angS" + String(noti_propertyMsg.suffix(4)) + mStyleStr.lowercased()), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = pathBurnCrop(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.mutilpart(texts: strs, colors: colors, fonts: fonts)
        ...
        freebeeQuantity(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func freebeeQuantity(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(userStateName.suffix(5)) + "angS" + String(noti_propertyMsg.suffix(4)) + mStyleStr.lowercased()), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = pathBurnCrop(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(userStateName.suffix(5)) + "angS" + String(noti_propertyMsg.suffix(4)) + mStyleStr.lowercased()), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = pathBurnCrop(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.my(150, 77, 20, 1), UIColor.white, UIColor.my(150, 77, 20, 1)]
        ...
        attText = attText.mutilpart(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.club(attributedText: attText)
        ...
        countLb.sample(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func cellEnd(giftMsgModel: BoneOfNameionMeasurable) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(lurk), object: nil)
        ...
        self.perform(#selector(lurk), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func populationProfileManager(model _: BoneOfNameionMeasurable, animate _: Bool) {
    ...
    @objc func lurk() {
        ...
        giftMsgModel = BoneOfNameionMeasurable()
...
extension MenuListThen {
    ...
    func pathBurnCrop(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension MenuListThen {
    ...
    private func exceptFirst() {
        ...
        font = pathBurnCrop(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.mutilpart(texts: [(String(dream_norId)).localized], colors: [UIColor.my(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.sample(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.club(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.sample(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
