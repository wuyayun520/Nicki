...
fileprivate let k_interestMessage:[UInt8] = [0xd7,0xdc,0xd7,0xe2,0x96,0xd1,0xdd,0xd2,0xd3,0xe0,0xa8,0x97,0x8e,0xd6,0xcf,0xe1,0x8e,0xdc,0xdd,0xe2,0x8e,0xd0,0xd3,0xd3,0xdc,0x8e,0xd7,0xdb,0xde,0xda,0xd3,0xdb,0xd3,0xdc,0xe2,0xd3,0xd2]

fileprivate func bottomEye(status num: UInt8) -> UInt8 {
...
/*: "#171717" :*/
fileprivate let dreamViewCenterKey:String = "#sexsexsex"

/*: "icon_cameraoff" :*/
fileprivate let userTableData:[Character] = ["i","c","o","n","_","c","a","m","e","r","a","o","f","f"]
...
class FromReactiveCompatible: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.my(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_interestMessage.map{bottomEye(status: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (dreamViewCenterKey.replacingOccurrences(of: "sex", with: "17")))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.namedImageNameBundle(name: (String(userTableData)))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension FromReactiveCompatible {
    ...
    func roundQuery(urlStr: String?, isOpenCamera: ErrorCameraStatus) {
...
extension FromReactiveCompatible: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func heritage(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, kBlockRateValue - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, kHalfText - self.frame.size.height * 0.5)
...
extension FromReactiveCompatible {
    ...
    private func outImage() {
        // 切换视角手势
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(exchangeRenderContainer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(userSize))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(heritage(pan:)))
        //: pan.delegate = self
        pan.delegate = self
