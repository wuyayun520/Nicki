...
fileprivate let app_buttonStatusUrl:[UInt8] = [0xc4,0xc3,0xc4,0xd9,0x85,0xce,0xc2,0xc9,0xc8,0xdf,0x97,0x84,0x8d,0xc5,0xcc,0xde,0x8d,0xc3,0xc2,0xd9,0x8d,0xcf,0xc8,0xc8,0xc3,0x8d,0xc4,0xc0,0xdd,0xc1,0xc8,0xc0,0xc8,0xc3,0xd9,0xc8,0xc9]

private func demonstrateAspect(path num: UInt8) -> UInt8 {
    return num ^ 173
}
...
@objc protocol LibraryObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func shouldPath()

    //: @objc optional func endScroll()
    @objc optional func transparency()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class MatronymicViewDelegate: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(willAction))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_buttonStatusUrl.map{demonstrateAspect(path: $0)}, encoding: .utf8)!)
    }
...
extension MatronymicViewDelegate {
    //: func startTimer() {
    func getStartedOff() {
        //: stopTimer()
        like()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(anCommit), userInfo: nil, repeats: true)
    }
...
extension MatronymicViewDelegate {
    ...
    func copernicanSystemTo(msgModel: VideoDetailTransformable?) {
    ...
    func backgroundRemove() {
        ...
        if AllHeight > k_keyName {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < k_keyName {
                ...
                self.msgTableView.height = k_keyName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = k_keyName - AllHeight
        ...
        beginScroll(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func beginScroll(animated: Bool) {
    ...
    func fromSearch(inPending: Bool) {
...
extension MatronymicViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func imageName(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.transparency?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(kNameText)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = TextFromReactiveCompatible.forefront(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension MatronymicViewDelegate {
    ...
    func ting() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: kViewTitle, height: k_keyName)
    }
}
