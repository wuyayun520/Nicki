...
/*: "call_bgm" :*/
fileprivate let m_targetName:[Character] = ["c","a","l","l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let showNameText:[Character] = ["c","a","l","l","_","r","e"]
fileprivate let kSizeKey:String = "livepon"
...
class PushPlayListener: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = DetailSignedNumeric.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func untilPage(type: DetailSignedNumeric) -> PushPlayListener {
        //: let player = TalkingCallBGMPlayer()
        let player = PushPlayListener()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.country()
...
extension PushPlayListener {
    ...
    private func enableceMoreAgain() {
    ...
    private func searchNotify(notify: Notification) {
        ...
        if type == AVAudioSession.InterruptionType.began {
            ...
            //: self.playAgain()
            self.enableceMoreAgain()
        }
    }

    //: private func audioRouteChangeListenerCallback(notify: Notification) {
    private func coldShoulderImage(notify: Notification) {
    ...
    private func country() {
        ...
        var fileName = (String(m_targetName))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(showNameText) + kSizeKey.replacingOccurrences(of: "live", with: "s") + "se_bgm")
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = LineReactiveCompatible.default.toCancel(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension PushPlayListener: TXVodPlayListener {
