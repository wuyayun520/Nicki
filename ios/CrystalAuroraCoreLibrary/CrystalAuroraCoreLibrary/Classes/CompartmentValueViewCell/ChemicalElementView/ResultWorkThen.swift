...
fileprivate let noti_modelName:[UInt8] = [0xe7,0xe0,0xe7,0xfa,0xa6,0xed,0xe1,0xea,0xeb,0xfc,0xb4,0xa7,0xae,0xe6,0xef,0xfd,0xae,0xe0,0xe1,0xfa,0xae,0xec,0xeb,0xeb,0xe0,0xae,0xe7,0xe3,0xfe,0xe2,0xeb,0xe3,0xeb,0xe0,0xfa,0xeb,0xea]

/*: "icon_coinxxx" :*/
fileprivate let appIgnoreMessage:[Character] = ["i","c","o","n","_","c","o","i","n","x","x"]
fileprivate let show_clickName:String = "cell"

/*: "icon_gift_num_bg" :*/
fileprivate let app_matchUrl:String = "icoits"
fileprivate let kPlayerKey:String = "make user model center bind_num_bg"

/*: "x1" :*/
fileprivate let user_sendOfData:[Character] = ["x","1"]

/*: " x :*/
fileprivate let mainKitKey:[Character] = [" ","x"]
...
class ResultWorkThen: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = NnwTransformable()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_modelName.map{$0^142}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.fontFile(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: RowView = {
        //: let btn = TalkingButton.init()
        let btn = RowView()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.fontFile(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.namedImageNameBundle(name: (String(appIgnoreMessage) + show_clickName.replacingOccurrences(of: "cell", with: "x"))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.namedImageNameBundle(name: (app_matchUrl.replacingOccurrences(of: "its", with: "n") + "_gift" + String(kPlayerKey.suffix(7))))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.fontFile(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension ResultWorkThen {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func equalSelected(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.subject().cgColor
    ...
    func circle(currenmodel: NnwTransformable, giftType: PrixFixeSubscriptType) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.placeOf(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.placeOf(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.fontFile(type: .Regular, fontSize: 11),
...
extension ResultWorkThen {
    ...
    func clickText() {
        ...
        bgView.backgroundColor = UIColor.totalimateBy()
