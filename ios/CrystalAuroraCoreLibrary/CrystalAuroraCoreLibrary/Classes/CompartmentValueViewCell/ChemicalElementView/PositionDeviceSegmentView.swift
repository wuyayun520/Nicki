...
fileprivate let app_jumpFormat:String = "设置普通状态的文"
fileprivate let dream_leadingFormat:String = "字\u{989c}色时"
fileprivate let noti_sizePath:[Character] = ["R","G","B","空","\u{95f4}","的","颜","色","\u{503c}"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let main_layerText:[Character] = ["设","\u{7f6e}","选","\u{4e2d}","状","态","的","文","字","颜","色","时"," ","请","使","\u{7528}"]
fileprivate let notiShowEqualTitle:[Character] = ["R","G","B","空","间","的","\u{989c}","色","\u{503c}"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let show_arrayUpPath:[UInt8] = [0xd5,0xd2,0xd5,0xc8,0x94,0xdf,0xd3,0xd8,0xd9,0xce,0x86,0x95,0x9c,0xd4,0xdd,0xcf,0x9c,0xd2,0xd3,0xc8,0x9c,0xde,0xd9,0xd9,0xd2,0x9c,0xd5,0xd1,0xcc,0xd0,0xd9,0xd1,0xd9,0xd2,0xc8,0xd9,0xd8]

private func liveWindow(content num: UInt8) -> UInt8 {
    return num ^ 188
}

/*: "设置的下标不合法!!" :*/
fileprivate let mainResultToIdent:String = "设置的下标不合法"
fileprivate let main_borderIconMsg:[Character] = ["!","!"]

/*: "Bags" :*/
fileprivate let mainGroupName:[Character] = ["B","a","g","s"]
...
// __CLOSE_PRINT__
//
//  PositionDeviceSegmentView.swift
//  ZEScollViewPage
//
...
open class PositionDeviceSegmentView: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: PasskeySegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.moveRgb(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((app_jumpFormat.capitalized + dream_leadingFormat + " 请使用" + String(noti_sizePath)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.moveRgb(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(main_layerText) + String(notiShowEqualTitle)))
        }
    ...
    fileprivate func moveRgb(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: PasskeySegmentStyle, titles: [String]) {
    ...
    @objc func cinema(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? ReadLabel else { return }
        //: currentIndex = currentLabel.tag
        currentIndex = currentLabel.tag
        //: print(currentLabel.tag)
        //: adjustUIWhenBtnOnClickWithAnimate(true)
        nameAcrossAnimate(true)
    }

    //: required public init?(coder aDecoder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: show_arrayUpPath.map{liveWindow(content: $0)}, encoding: .utf8)!)
    }
}
...
public extension PositionDeviceSegmentView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func publication(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (mainResultToIdent.capitalized + String(main_borderIconMsg)))
        ...
        nameAcrossAnimate(animated)
    }

    // 暴露给外界来刷新标题的显示
    //: public func reloadTitlesWithNewTitles(_ titles: [String]) {
    func betweenKey(_ titles: [String]) {
        // 移除所有的scrollView子视图
...
//: extension ScrollSegmentView {
extension PositionDeviceSegmentView {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func starCapacity() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = ReadLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.cinema(_:)))
            ...
            if title == (String(mainGroupName)).localized {
    ...
    fileprivate func listEnableGuidance() {
        // 设置Label位置
    ...
    fileprivate func lowerClass() {
        ...
        if let firstLabel = labelsArray[0] as? ReadLabel {
    ...
    public func resign(_ currentIndex: Int) {
