...
fileprivate let m_fileStr:[UInt8] = [0x58,0x5f,0x58,0x45,0x19,0x52,0x5e,0x55,0x54,0x43,0xb,0x18,0x11,0x59,0x50,0x42,0x11,0x5f,0x5e,0x45,0x11,0x53,0x54,0x54,0x5f,0x11,0x58,0x5c,0x41,0x5d,0x54,0x5c,0x54,0x5f,0x45,0x54,0x55]

private func cellMenu(row num: UInt8) -> UInt8 {
    return num ^ 49
}

/*: "#30D00B" :*/
fileprivate let mainObserveText:[Character] = ["#","3","0","D","0","0","B"]

/*: "icon_receivedcalls" :*/
fileprivate let user_iconIdent:[Character] = ["i","c"]
fileprivate let dreamEqualKey:[Character] = ["o","n","_","r","e","c","e","i","v","e","d","c","a","l","l","s"]

/*: "Received calls" :*/
fileprivate let mFrameMessage:String = "Receivelse to make right view"

/*: "icon_dialedcalls" :*/
fileprivate let dream_multiData:String = "icotop"
fileprivate let k_buttonPath:[Character] = ["c","a","l","l","s"]

/*: "Dialed calls" :*/
fileprivate let dreamPointTitle:String = "var path name inDial"
fileprivate let notiImageMessage:String = "source with ined c"

/*: "%02i:%02i" :*/
fileprivate let dream_tapModelWithKey:[Character] = ["%","0","2","i"]
fileprivate let show_yearStr:[Character] = [":","%","0","2","i"]
...
class PictureTableCell: UITableViewCell {
    ...
    var currenModel = KeyModelType()
    //: var seleteIndex: IndexPath?
    var seleteIndex: IndexPath?
    //: open weak var delegate: WhoLikeDelegate?
    open weak var delegate: FrameworkLikeDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: m_fileStr.map{cellMenu(row: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(camp), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .fontFile(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .totalervertebralDisc()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(mainObserveText)))
...
extension PictureTableCell {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func baptizeTo(model: KeyModelType) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.faceTable(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.namedImageNameBundle(name: (String(user_iconIdent) + String(dreamEqualKey)))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(mFrameMessage.prefix(6)) + "ed calls").localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.namedImageNameBundle(name: (dream_multiData.replacingOccurrences(of: "top", with: "n") + "_dialed" + String(k_buttonPath)))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(dreamPointTitle.suffix(4)) + String(notiImageMessage.suffix(4)) + "alls").localized
        }
...
extension PictureTableCell {
    ...
    private func numbereractionEqual() {}
}
