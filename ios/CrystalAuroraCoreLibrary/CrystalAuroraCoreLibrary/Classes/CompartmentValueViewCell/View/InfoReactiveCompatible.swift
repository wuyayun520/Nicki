...
fileprivate let showInfoStr:String = "mode mediumNeed "
fileprivate let showValueTransitionMsg:String = "t to pop estimated path info local"
fileprivate let mainStageTransferTitle:[Character] = ["u","n","l","o","c","k"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_fileMsg:[UInt8] = [0xb1,0xb6,0xb1,0xac,0xf0,0xbb,0xb7,0xbc,0xbd,0xaa,0xe2,0xf1,0xf8,0xb0,0xb9,0xab,0xf8,0xb6,0xb7,0xac,0xf8,0xba,0xbd,0xbd,0xb6,0xf8,0xb1,0xb5,0xa8,0xb4,0xbd,0xb5,0xbd,0xb6,0xac,0xbd,0xbc]

/*: "intimatephoto_btn_no" :*/
fileprivate let show_subData:[Character] = ["i","n","t","i","m","a","t"]
fileprivate let k_actualWriteKey:[Character] = ["e","p","h","o","t","o","_","b","t","n","_","n","o"]

/*: "Burn After Reading" :*/
fileprivate let dreamFromIdent:String = "Burn Ai in add"
fileprivate let showMakeIdent:String = "eadsharen"
fileprivate let noti_aMsg:[Character] = ["g"]

/*: "Use Photo" :*/
fileprivate let userHiddenContent:[Character] = ["U","s","e"," ","P","h","o","t","o"]

/*: "Use Video" :*/
fileprivate let appSaveMakeName:String = "Use Vindex record guard"

/*: "intimatephoto_btn_chose" :*/
fileprivate let show_toolMeetData:String = "inyesterdaymatep"
fileprivate let notiThanId:String = "HOT"
fileprivate let userQuantityName:String = "o_btn_cover page view"

/*: "%@ (%@x1  %@ Gold coins)" :*/
fileprivate let notiLayTitle:String = "height to error case%@ (%@x1"
fileprivate let showPathData:String = "hiddenhidden%@"
fileprivate let dreamViewKey:String = "oins)star super list view"

/*: "#7E6AFF" :*/
fileprivate let show_sendStr:String = "number share model cell#7E6AF"
fileprivate let showUserFormat:String = "f"
...
let kImageValue = (176 + k_videoName)

//: typealias UseBlock = (_ isBurn: String, _ giftId: String) -> Void
typealias UseBlock = (_ isBurn: String, _ giftId: String) -> Void

//: class TalkingSendIntimateInfoView: UIView {
class InfoReactiveCompatible: UIView {
    //: var block: UseBlock?
    var block: UseBlock?
    //: private let normalGiftText = "Need a gift to unlock".localized
    private let normalGiftText = (String(showInfoStr.suffix(5)) + "a gif" + String(showValueTransitionMsg.prefix(5)) + String(mainStageTransferTitle)).localized
    //: private var giftId = "0"  // 解锁礼物Id
    private var giftId = "0" // 解锁礼物Id
    //: private var type = IntimateInfoViewType.intimatePhoto
    private var type = AmbagesViewType.intimatePhoto // 私密类型

    //: init(frame: CGRect, type: IntimateInfoViewType) {
    init(frame: CGRect, type: AmbagesViewType) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.frame = CGRect(x: 0, y: ScreenHeight-INTIMATEINFOVIEW_H, width: ScreenWidth, height: INTIMATEINFOVIEW_H)
        self.frame = CGRect(x: 0, y: kHalfText - kImageValue, width: kBlockRateValue, height: kImageValue)
        //: self.type = type
        self.type = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_fileMsg.map{$0^216}, encoding: .utf8)!)
    }
    ...
    private lazy var burnAfterReadingImgV: UIImageView = {
        ...
        let img = UIImageView(image: UIImage.namedImageNameBundle(name: (String(show_subData) + String(k_actualWriteKey))))
    ...
    private lazy var burnAfterReadingLab: UILabel = {
        ...
        lab.text = (String(dreamFromIdent.prefix(6)) + "fter R" + showMakeIdent.replacingOccurrences(of: "share", with: "i") + String(noti_aMsg)).localized
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var burnAfterReadingBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(swath), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var needGiftBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(getWeaving), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var useBtn: UIButton = {
        ...
        btn.setTitle(self.type == .intimatePhoto ? (String(userHiddenContent)).localized : (String(appSaveMakeName.prefix(5)) + "ideo").localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(fromOpen), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var giftView: ChatDatabaseThen = {
...
extension InfoReactiveCompatible {
    ...
    @objc private func swath() {
        ...
        let imageName = burnAfterReadingBtn.isSelected ? (show_toolMeetData.replacingOccurrences(of: "yesterday", with: "ti") + notiThanId.lowercased() + String(userQuantityName.prefix(6)) + "chose") : (String(show_subData) + String(k_actualWriteKey))
        //: burnAfterReadingImgV.image = UIImage.BundleImageNamed(name: imageName)
        burnAfterReadingImgV.image = UIImage.namedImageNameBundle(name: imageName)
    }
}
...
extension InfoReactiveCompatible {
    ...
    @objc private func getWeaving() {
        ...
        guard needGiftBtn.isSelected == false else { // 还原状态
            ...
            needGiftImgeV.image = UIImage.namedImageNameBundle(name: (String(show_subData) + String(k_actualWriteKey)))
        ...
        let type = (self.type == .intimatePhoto ? ConstraintValueConvertible.intimatePhoto : ConstraintValueConvertible.intimateVideo)
        //: TalkingChatGiftManager.share.func__sendGiftEvent(type: type) {
        TrivialityThen.share.soapResume(type: type) {
    ...
    private func soliloquise() {
        ...
        giftView.okActionBlock = { [weak self] giftModel in
            ...
            self.needGiftImgeV.image = UIImage.namedImageNameBundle(name: (show_toolMeetData.replacingOccurrences(of: "yesterday", with: "ti") + notiThanId.lowercased() + String(userQuantityName.prefix(6)) + "chose"))
            ...
            let giftAttrStr = NSMutableAttributedString(string: (String(notiLayTitle.suffix(8)) + showPathData.replacingOccurrences(of: "hidden", with: " ") + " Gold c" + String(dreamViewKey.prefix(5))).localizedDataArguments(self.normalGiftText, giftModel.name, giftModel.price))
            ...
            let attributes = [.foregroundColor: UIColor(hex: (String(show_sendStr.suffix(6)) + showUserFormat.uppercased()))!, .font: UIFont.fontFile(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
...
extension InfoReactiveCompatible {
    ...
    private func textFor() {
        ...
        whiteBgV.addSubview(burnAfterReadingBtn)
        //: if ModeStageReactiveCompatible.share.loginUserMode.status != 1 &&
        if ModeStageReactiveCompatible.share.loginUserMode.status != 1,
    ...
    private func takePlace() {
        ...
        useBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().offset(-20 - k_videoName)
