...
fileprivate let dream_blockActualIdent:[UInt8] = [0x6a,0x6d,0x6a,0x77,0x2b,0x60,0x6c,0x67,0x66,0x71,0x39,0x2a,0x23,0x6b,0x62,0x70,0x23,0x6d,0x6c,0x77,0x23,0x61,0x66,0x66,0x6d,0x23,0x6a,0x6e,0x73,0x6f,0x66,0x6e,0x66,0x6d,0x77,0x66,0x67]

/*: "8075F5" :*/
fileprivate let appDataTitle:String = "8075F5import with of self var"

/*: "9A9B9C" :*/
fileprivate let mainMonthKey:[Character] = ["9","A","9","B","9","C"]

/*: "kOpacityAnimation" :*/
fileprivate let main_fillGestureMessage:[Character] = ["k","O","p","a","c","i","t","y","A","n"]
fileprivate let m_pushData:String = "imatiavailablen"

/*: "opacity" :*/
fileprivate let user_titleData:String = "opaciimage"
fileprivate let k_marginData:[Character] = ["y"]

/*: "forwards" :*/
fileprivate let kShowTitle:String = "forwarprice"

/*: "easeIn" :*/
fileprivate let notiSizePath:String = "of vieweaseI"
fileprivate let userPathMessage:[Character] = ["n"]

/*: "8C7AF8" :*/
fileprivate let main_topTitle:[Character] = ["8","C","7","A","F","8"]
...
// __CLOSE_PRINT__
//
//  WrittenCommunicationInputView.swift
//  AbroadTalking
//
...
class WrittenCommunicationInputView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dream_blockActualIdent.map{$0^3}, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(pic(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension WrittenCommunicationInputView {
    ...
    func alongActionVanguard() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(appDataTitle.prefix(6))))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(mainMonthKey)))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func appear(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(main_fillGestureMessage) + m_pushData.replacingOccurrences(of: "available", with: "o")))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(animationFull(), forKey: (String(main_fillGestureMessage) + m_pushData.replacingOccurrences(of: "available", with: "o")))
        }
    ...
    fileprivate func animationFull() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (user_titleData.replacingOccurrences(of: "image", with: "t") + String(k_marginData)))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (kShowTitle.replacingOccurrences(of: "price", with: "ds")))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(notiSizePath.suffix(5)) + String(userPathMessage))))
        //: return animation
        return animation
    }
}
...
extension WrittenCommunicationInputView {
    ...
    private func notInfo() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(main_topTitle)))?.cgColor
