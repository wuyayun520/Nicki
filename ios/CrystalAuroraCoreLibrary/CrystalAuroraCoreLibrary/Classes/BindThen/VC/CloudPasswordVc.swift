...
class CloudPasswordVc: StateViewController {
    //: var type = LoginType.PhoneLogin
    var type = VideoClusterLiteral.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    override func viewDidLoad() {
        ...
        socialRemove()
        //: bindInteraction()
        numbereractionQueryed()
    }

    // MARK: - Lazy load

    //: private lazy var pwdView: TalkingSetNewPwdView = {
    private lazy var pwdView: ResultQuantityView = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = ResultQuantityView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension CloudPasswordVc {
    /// 修改密码
    //: private func req_modifyPwd(_ pwd: String, _ vcode: String?) {
    private func writeUp(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        ...
        LibraryThen.make(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
    ...
    private func location() {
        ...
        LibraryThen.button { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            AnagrammatizeView.implant()
            //: guard let self = self else { return }
            guard let self = self else { return }
