...
#import "UIImage+Place.h"

//: @implementation UIImage (Compress)
@implementation UIImage (Place)

//: - (NSData *)compressedImageData {
- (NSData *)count {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.to;
...
- (UIImage *)last {
    ...
    return [UIImage imageWithData:[self count]];
}


//: - (UIImage *)wcSessionCompress {
- (UIImage *)effectCompress {
    //: return [self wcCompres:YES];
    return [self ptolemaicSystemCompres:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)to {
    //: return [self wcCompres:NO];
    return [self ptolemaicSystemCompres:NO];
}
...
- (UIImage *)ptolemaicSystemCompres:(Boolean)isSession {
...
+ (NSData *)localSize:(UIImage *)image count:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG content:scalesize lock:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG device:(fileSize*1024)];
    }
...
- (UIImage*)bond:(CGSize)targetSize{
...
- (UIImage *)content:(CGSize)targetSize lock:(BOOL)isScale{
...
- (NSData *)device:(NSInteger)fileSize{
