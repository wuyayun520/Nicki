...
/*: "recharge" :*/
fileprivate let m_layerPopMessage:[Character] = ["r","e","c","h","a","r","g","e"]

/*: "recharge_ :*/
fileprivate let mainEqualTitle:String = "rechsucceedrge"
fileprivate let kMagnitudeMessage:String = "_"

/*: ".png" :*/
fileprivate let mainImageUrl:[Character] = [".","p","n","g"]

/*: "_ :*/
fileprivate let dream_modelMessage:String = "user"

/*: .png" :*/
fileprivate let m_wayUrl:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let noti_stopMsg:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let mainSelectStr:[Character] = [".","j","p","g"]
...
enum ListImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class RowView: UIButton {
    ...
    var imageAlignment: ListImageAlignment = .left
...
extension UIButton {
    ...
    func colorEventTitle(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        startDoingset(interval: interval, topOffset: 0)
    }
    ...
    func startDoingset(interval: CGFloat, topOffset: CGFloat) {
    ...
    func managerByEnableset(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func note(color: UIColor, forState: UIControl.State) {
    ...
    func frank(urlStr: String,
               //: placeImg: UIImage? = UIImage.placeImgSquare(),
               placeImg: UIImage? = UIImage.betise(),
               ...
               loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: kVoiceName.format(), with: k_screenRecordData)
    ...
    func faceTable(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((String(m_layerPopMessage))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (String(m_layerPopMessage)), with: (mainEqualTitle.replacingOccurrences(of: "succeed", with: "a") + kMagnitudeMessage.capitalized) + "\(k_messageFlushValue)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.frank(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(mainImageUrl))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(mainImageUrl)), with: "_\(k_messageFlushValue)" + (String(m_wayUrl)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(noti_stopMsg))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(noti_stopMsg)), with: "_\(k_messageFlushValue)" + (String(mainSelectStr)))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.frank(urlStr: newUrl, placeImg: nil)
    }
