...
/*: "type" :*/
fileprivate let userItemContent:[Character] = ["t","y","p","e"]

/*: "You are currently on a call, please try again later" :*/
fileprivate let mContrastKey:[UInt8] = [0x3f,0x55,0x5b,0x6,0x47,0x58,0x4b,0x6,0x49,0x5b,0x58,0x58,0x4b,0x54,0x5a,0x52,0x5f,0x6,0x55,0x54,0x6,0x47,0x6,0x49,0x47,0x52,0x52,0x12,0x6,0x56,0x52,0x4b,0x47,0x59,0x4b,0x6,0x5a,0x58,0x5f,0x6,0x47,0x4d,0x47,0x4f,0x54,0x6,0x52,0x47,0x5a,0x4b,0x58]

fileprivate func labelLayer(temp num: UInt8) -> UInt8 {
...
/*: "onCallResume" :*/
fileprivate let notiJumpValue:[Character] = ["o","n","C","a","l","l","R"]
fileprivate let m_labName:String = "erawme"
...
// __CLOSE_PRINT__
//
//  ModeStageReactiveCompatible+VideoChat.swift
//  AbroadTalking
//
...
extension ModeStageReactiveCompatible {
    ...
    func logLocation(info: [String: Any], situation: TelevisionChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(String(userItemContent))].intValue
        ...
        if type == 0 { // 视频
            ...
            (vc as! ModeViewController).chatModel = LibraryChatModel.fishBowl(info: info, situation: situation)
        }

        //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: false, resumeData: [:])
        self.underBurnData(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: false, resumeData: [:])
    }

    /// 接收到视频通话
    /// - Parameter info: 数据
    //: func recieve1v1TalkRequest(info: [String: Any]) {
    func bindInfoName(info: [String: Any]) {
        //: handle1v1AudioRequest(info: info, resumeTalk: false)
        makeUp(info: info, resumeTalk: false)
    }

    /// 恢复视频通话
    /// - Parameter info: 数据
    //: func recieve1v1TalkResumeRequest(info: [String: Any]) {
    func theWhoCell(info: [String: Any]) {
        //: handle1v1AudioRequest(info: info, resumeTalk: true)
        makeUp(info: info, resumeTalk: true)
    }
}
...
extension ModeStageReactiveCompatible {
    ...
    private func makeUp(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isWork {
        if !resumeTalk && EffectSocketDelegate.shared.isWork {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.actionKey(showMsg: String(bytes: mContrastKey.map{labelLayer(temp: $0)}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if startScreen()!.isKind(of: LightErrorDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            startScreen()!.isKind(of: ModeViewController.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            startScreen()!.isKind(of: WorkObjectProtocol.self)
        {
            //: return
            return
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! LightErrorDelegate).chatModel = LibraryChatModel.fishBowl(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = ModeViewController()
            ...
            (vc as! ModeViewController).chatModel = LibraryChatModel.fishBowl(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(notiJumpValue) + m_labName.replacingOccurrences(of: "raw", with: "su"))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.underBurnData(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func underBurnData(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = startScreen() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = MacroReactiveCompatible.getWindow().rootViewController as! UITabBarController
            let rootVc = MacroReactiveCompatible.colorForDestroy().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(ModeViewController.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(LightErrorDelegate.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: LightErrorDelegate?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! LightErrorDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? LightErrorDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.requestVideo(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: ToNavigationController.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! ToNavigationController
            ...
            if topVc != nil, topVc!.isKind(of: AdPushMatchDelegate.self) {
            ...
            var currentChatVc: ModeViewController?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! ModeViewController

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? ModeViewController
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.valueDataFormatting(resumeData: resumeData)
                }
                //: return
                return
            }
