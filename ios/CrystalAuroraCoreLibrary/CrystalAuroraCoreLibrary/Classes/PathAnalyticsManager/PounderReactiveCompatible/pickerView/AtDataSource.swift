...
/*: "Select a date" :*/
fileprivate let showSizeMessage:String = "Selectwait any color"
fileprivate let notiCenterVoiceValue:String = "file"

/*: M" :*/
fileprivate let notiImageName:String = "name"

/*: D" :*/
fileprivate let show_textValue:[Character] = ["D"]

/*: Y" :*/
fileprivate let userAreaTitleStatusText:[Character] = ["Y"]
...
protocol LabelViewDelegate: NSObjectProtocol {
...
class AtDataSource: UndressPickView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func deal() {
        ...
        titleString = (String(showSizeMessage.prefix(6)) + " a dat" + notiCenterVoiceValue.replacingOccurrences(of: "file", with: "e")).localized
        ...
        halfMastFirst(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func halfMastFirst(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func corroborate(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.pickAdd(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.pickAdd(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return halogenLimited(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func halogenLimited(year: Int, month: Int) -> Int {
