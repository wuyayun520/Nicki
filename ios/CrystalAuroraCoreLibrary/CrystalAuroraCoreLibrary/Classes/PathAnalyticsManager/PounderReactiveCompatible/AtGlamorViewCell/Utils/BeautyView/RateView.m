...
//: STDisplayShader
@property (nonatomic, copy) NSString *showSkipStr;
...
- (Byte *)ActiveDataToCache:(Byte *)data {
    int dimensionWith = data[0];
    int propertyLimit = data[1];
    for (int i = 0; i < dimensionWith / 2; i++) {
        int begin = propertyLimit + i;
        int end = propertyLimit + dimensionWith - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[propertyLimit + dimensionWith] = 0;
    return data + propertyLimit;
}
...
//: STDisplayShader
- (NSString *)showSkipStr {
    if (!_showSkipStr) {
        Byte value[] = {15, 4, 129, 107, 114, 101, 100, 97, 104, 83, 121, 97, 108, 112, 115, 105, 68, 84, 83, 7};
        _showSkipStr = [self StringFromActiveData:value];
    }
    return _showSkipStr;
}
...
- (instancetype)initWithTotal:(CGRect)frame screen:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self data:[ActiveData sharedInstance].showSkipStr
                //: fragmentShader:@"STDisplayShader"
                absorption:[ActiveData sharedInstance].showSkipStr
                    //: forProgram:&stDisplayProgram];
                    context:&stDisplayProgram];
...
- (BOOL)data:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          absorption:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              context:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self offline:&vertexShader strength:0x8B31 menuHandle:kRangeName]) {
    ...
    if (![self offline:&fragShader strength:0x8B30 menuHandle:kStatusTapName]) {
    ...
    if (![self position:*programPointer]) {
...
- (BOOL)offline:(GLuint *)shader strength:(GLenum)type menuHandle:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
