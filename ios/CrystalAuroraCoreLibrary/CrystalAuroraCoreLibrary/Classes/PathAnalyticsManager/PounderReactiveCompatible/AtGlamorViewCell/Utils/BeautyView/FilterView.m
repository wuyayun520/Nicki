...
@property (nonatomic, copy) NSString *app_canvasEventValue;

//: hasFace
@property (nonatomic, copy) NSString *mPopPath;

//: model
@property (nonatomic, copy) NSString *dream_descriptionPath;

//: key.Notification.Tap.To.Modify.Exposure.Point
@property (nonatomic, copy) NSString *dreamRowVocalMsg;

//: FaceActionEvent_Interval_12_frame
@property (nonatomic, copy) NSString *noti_guidanceFormat;

//: camera_focus_red
@property (nonatomic, copy) NSString *showMicKey;

//: M_SenseME_Face_Video_5.3.4
@property (nonatomic, copy) NSString *userRetainDataTaskMsg;
...
- (Byte *)StateDataToCache:(Byte *)data {
    int tapLast = data[0];
    int guidanceAppear = data[1];
    for (int i = 0; i < tapLast / 2; i++) {
        int begin = guidanceAppear + i;
        int end = guidanceAppear + tapLast - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[guidanceAppear + tapLast] = 0;
    return data + guidanceAppear;
}
...
- (NSString *)mPopPath {
    if (!_mPopPath) {
        Byte value[] = {7, 12, 122, 223, 187, 109, 244, 126, 172, 59, 224, 238, 101, 99, 97, 70, 115, 97, 104, 84};
        _mPopPath = [self StringFromStateData:value];
    }
    return _mPopPath;
}

//: key.Notification.Tap.To.Modify.Exposure.Point
- (NSString *)dreamRowVocalMsg {
    if (!_dreamRowVocalMsg) {
        Byte value[] = {45, 9, 39, 243, 201, 42, 26, 141, 174, 116, 110, 105, 111, 80, 46, 101, 114, 117, 115, 111, 112, 120, 69, 46, 121, 102, 105, 100, 111, 77, 46, 111, 84, 46, 112, 97, 84, 46, 110, 111, 105, 116, 97, 99, 105, 102, 105, 116, 111, 78, 46, 121, 101, 107, 99};
        _dreamRowVocalMsg = [self StringFromStateData:value];
    }
    return _dreamRowVocalMsg;
}

//: M_SenseME_Face_Video_5.3.4
- (NSString *)userRetainDataTaskMsg {
    if (!_userRetainDataTaskMsg) {
        Byte value[] = {26, 2, 52, 46, 51, 46, 53, 95, 111, 101, 100, 105, 86, 95, 101, 99, 97, 70, 95, 69, 77, 101, 115, 110, 101, 83, 95, 77, 91};
        _userRetainDataTaskMsg = [self StringFromStateData:value];
    }
    return _userRetainDataTaskMsg;
}

//: camera_focus_red
- (NSString *)showMicKey {
    if (!_showMicKey) {
        Byte value[] = {16, 12, 118, 232, 208, 162, 86, 200, 72, 165, 58, 81, 100, 101, 114, 95, 115, 117, 99, 111, 102, 95, 97, 114, 101, 109, 97, 99, 192};
        _showMicKey = [self StringFromStateData:value];
    }
    return _showMicKey;
}

//: model
- (NSString *)dream_descriptionPath {
    if (!_dream_descriptionPath) {
        Byte value[] = {5, 5, 71, 186, 49, 108, 101, 100, 111, 109, 6};
        _dream_descriptionPath = [self StringFromStateData:value];
    }
    return _dream_descriptionPath;
}

//: FaceActionEvent_Interval_12_frame
- (NSString *)noti_guidanceFormat {
    if (!_noti_guidanceFormat) {
        Byte value[] = {33, 2, 101, 109, 97, 114, 102, 95, 50, 49, 95, 108, 97, 118, 114, 101, 116, 110, 73, 95, 116, 110, 101, 118, 69, 110, 111, 105, 116, 99, 65, 101, 99, 97, 70, 25};
        _noti_guidanceFormat = [self StringFromStateData:value];
    }
    return _noti_guidanceFormat;
}

//: FaceActionEvent_Interval_1_second
- (NSString *)app_canvasEventValue {
    if (!_app_canvasEventValue) {
        Byte value[] = {33, 5, 106, 153, 6, 100, 110, 111, 99, 101, 115, 95, 49, 95, 108, 97, 118, 114, 101, 116, 110, 73, 95, 116, 110, 101, 118, 69, 110, 111, 105, 116, 99, 65, 101, 99, 97, 70, 66};
        _app_canvasEventValue = [self StringFromStateData:value];
    }
    return _app_canvasEventValue;
}
...
//
//#import "DetailCacheCost.h"
//#import "ProductLooksModel.h"
//ST_MOBILE
...
@interface FilterView ()<JumpOpenMessage,UIGestureRecognizerDelegate>
{
...
//: @property (nonatomic, strong) ConstraintEraseReactiveCompatible * lichelper;
@property (nonatomic, strong) ConstraintEraseReactiveCompatible * lichelper;
//: @property (nonatomic, strong) TimingReactiveCompatible * ParamUtil;
@property (nonatomic, strong) TimingReactiveCompatible * ParamUtil;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //: self.lichelper = [ConstraintEraseReactiveCompatible share];
        self.lichelper = [ConstraintEraseReactiveCompatible share];
        //: self.ParamUtil = [[TimingReactiveCompatible alloc]init];
        self.ParamUtil = [[TimingReactiveCompatible alloc]init];
        //: _currentDropCount = 5;
        ...
        if ([self.lichelper momentLicense]) {
            ...
            //: [[DetailCacheCost share] getLocalModel];
            [[DetailCacheCost share] rangeGift];
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   ...
                                                   selector:@selector(deviceErase:)
                                                       //: name:@"key.Notification.Tap.To.Modify.Exposure.Point"
                                                       name:[StateData sharedInstance].dreamRowVocalMsg
                                                     //: object:nil];
                                                     object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getDown:) name:self.ParamUtil.kNotificationSelectBeautyFilterStr object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(writerChanged:) name:self.ParamUtil.kNotificationBeautyFilterValueChangeStr object:nil];
...
- (instancetype)initIdentity:(NSString*)sessionPreset {
    ...
    if (self = [super init]) {
        ...
        if ([self.lichelper momentLicense]) {
            ...
            [self initScale:sessionPreset];

        //: } else {
        } else {
            //: [ConstraintEraseReactiveCompatible.share checkRemoteLicInfoWithCompletion:^(BOOL x) {
            [ConstraintEraseReactiveCompatible.share sizeMakeWithCompletion:^(BOOL x) {
                //: if (x) {
                if (x) {
                    //: [self initFilterView:sessionPreset];
                    [self initScale:sessionPreset];
...
- (void)initScale:(NSString*)sessionPreset {
    //: self.layer.masksToBounds = YES;
    self.layer.masksToBounds = YES;
    //: self.clipsToBounds = YES;
    self.clipsToBounds = YES;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getDown:)
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(writerChanged:)
...
- (void)local {
    ...
    //: if ([DetailCacheCost share].filterSuite.filterModelPath.length > 0) {
    if ([DetailCacheCost share].filterSuite.filterModelPath.length > 0) {
        //: ProductLooksModel* model = [[DetailCacheCost share] getFilterDataModel];
        ProductLooksModel* model = [[DetailCacheCost share] hideConversation];
        //: if (model) {
        if (model) {
            //: [self handleFilterChanged:model];
            [self mnemonics:model];
        }
    }
}
...
- (void)setCurrentSessionPreset:(NSString *)currentSessionPreset {
    ...
    [self.stCamera tillObserver:self.previewCenter user:self.previewFrame];
}
...
- (void)device {
    ...
    _stCamera = [[PremiumCamera alloc] initWithKeyAndProduction:AVCaptureDevicePositionFront
                                          //: sessionPresset:self.currentSessionPreset
                                          with:self.currentSessionPreset
                                                     //: fps:fps
                                                     menu:fps
                                           //: needYuvOutput:NO];
                                           assemblage:NO];
...
- (void)kind:(CGFloat)width custom:(CGFloat)height {
    //: CGRect previewRect = [self.stCamera getScaleRect:CGRectMake(0 , 0 , width, height)
    CGRect previewRect = [self.stCamera photoLoad:CGRectMake(0 , 0 , width, height)
                                                   //: scaleToFit:NO];
                                                   rect:NO];
    ...
    if (self.glPreview.superview == nil) {
        ...
        @synchronized (self) {
           ...
           _glPreview = [[RateView alloc] initWithTotal:previewRect screen:previewContext];
        }
...
- (void)sumimateAcross {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(drawApp) name:UIApplicationWillResignActiveNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(dateView) name:UIApplicationDidEnterBackgroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appWillEnterForeground) name:UIApplicationWillEnterForegroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(willEnterSendOutput) name:UIApplicationWillEnterForegroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidBecomeActive) name:UIApplicationDidBecomeActiveNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(cellMain) name:UIApplicationDidBecomeActiveNotification object:nil];
}
...
- (void)sittingOrientation:(CMAccelerometerData *)accelerometerData {
...
- (st_rotate_type)variant {
    ...
    [self sittingOrientation:self.motionManager.accelerometerData];
...
- (void)keyEnable {
    ...
    //初始化检测模块句柄
    //: NSString *strModelPath = [MacroReactiveCompatible.getModelFileName stringByAppendingPathComponent:@"M_SenseME_Face_Video_5.3.4"];
    NSString *strModelPath = [MacroReactiveCompatible.sendTo stringByAppendingPathComponent:[StateData sharedInstance].userRetainDataTaskMsg];
    //: strModelPath = [strModelPath stringByAppendingPathExtension:@"model"];
    strModelPath = [strModelPath stringByAppendingPathExtension:[StateData sharedInstance].dream_descriptionPath];
...
- (BOOL)observer:(CVPixelBufferRef)pixelBuffer {
...
- (void)deviceErase:(NSNotification *)sender {
    ...
    if ([tapGesture isKindOfClass:UITapGestureRecognizer.class]) {
        ...
        [self filter:tapGesture];
    }

}

//: - (void)tapScreen:(UITapGestureRecognizer *)tapGesture {
- (void)filter:(UITapGestureRecognizer *)tapGesture {
    ...
    [self.stCamera tillObserver:point user:self.frame];
...
- (UIImageView *)focusImageView {
    ...
    if (!_focusImageView) {
        ...
        _focusImageView.image = [UIImage imageNamed:[StateData sharedInstance].showMicKey];
...
- (void)getDown:(NSNotification*) notice {
    //: if ([notice.object isKindOfClass:[ProductLooksModel class]]) {
    if ([notice.object isKindOfClass:[ProductLooksModel class]]) {
        //: ProductLooksModel* model = (ProductLooksModel*)notice.object;
        ProductLooksModel* model = (ProductLooksModel*)notice.object;
        //: [self handleFilterChanged:model];
        [self mnemonics:model];
    }
...
//: - (void)handleFilterChanged:(ProductLooksModel *)model {
- (void)mnemonics:(ProductLooksModel *)model {
...
- (void)writerChanged:(NSNotification*) notice {
    //: ProductLooksModel* model = (ProductLooksModel*)notice.object;
    ProductLooksModel* model = (ProductLooksModel*)notice.object;
    //: if (_hFilter) {
    if (_hFilter) {
...
- (void)duringLength {
    ...
    //: BorderReactiveCompatible *beautyValueCache = DetailCacheCost.share.currentBeautySuite;
    BorderReactiveCompatible *beautyValueCache = DetailCacheCost.share.flatClick;
    ...
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self place:beautyValueCache.fMouthStrength]];
    //缩人中
    ...
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self place:beautyValueCache.fPhiltrumStrength]];
    ...
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self place:beautyValueCache.fContrastStrength]];
    ...
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self place:beautyValueCache.fSaturationStrength]];
    ...
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self place:beautyValueCache.fSharpenStrength]];
    ...
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self place:beautyValueCache.fAppleMusleStrength]];
...
void spaceStyle(st_mobile_animal_face_t *src, int faceCount, st_mobile_animal_face_t *dst) {
...
void programMake(st_mobile_animal_face_t *src, int faceCount) {
    //: if (faceCount > 0) {
    if (faceCount > 0) {
        //: for (int i = 0; i < faceCount; ++i) {
        for (int i = 0; i < faceCount; ++i) {
...
- (void)output:(CMSampleBufferRef)originalSampleBuffer
                       //: resultPixelBuffer:(CVPixelBufferRef)resultPixelBufffer {
                       object:(CVPixelBufferRef)resultPixelBufffer {
    ...
    if ( originalSampleBuffer && resultPixelBufffer ) {
        ...
        if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(duringPic:mishandle:)]) {
            ...
            if (sampleBuffer) {
                ...
                [tmpDelegate duringPic:self mishandle:sampleBuffer];
            }
...
- (CVPixelBufferRef)turn:(CVPixelBufferRef)pixelBuffer {
...
- (void)session:(AVCaptureOutput *)captureOutput add:(CMSampleBufferRef)sampleBuffer runDescription:(AVCaptureConnection *)connection {
    ...
    st_rotate_type stMobileRotate = [self variant];
    ...
    if (_hDetector) {
        ...
        if (self.iCurrentAction > 0) {
            ...
            if (_faceFrameCount == 12) { // 第12帧时
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[StateData sharedInstance].noti_guidanceFormat object:nil userInfo: @{[StateData sharedInstance].mPopPath: @(hasFace)}];
            }
            ...
            if (_faceFrameCount%18 == 0) { // 每秒发送一次
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[StateData sharedInstance].app_canvasEventValue object:nil userInfo: @{[StateData sharedInstance].mPopPath: @(hasFace)}];
            }
        }
    }
    ...
    if (faceCount > 0) {
        ...
        spaceStyle(_detectResult1, faceCount, newDetectResult1);
    }
    ...
    dispatch_async(self.renderQueue, ^{
       ...
       BOOL isTextureOriginReady = [self observer:pixelBuffer];
       ...
       // ST_MOBILE 以下为滤镜部分
        ...
        if (callBackFlag) [self output:sampleBuffer object:resultPixelBufffer];
        ...
        if (faceCount > 0) {
            ...
            programMake(newDetectResult1, faceCount);
        }
        //: if (callBackFlag) {
        if (callBackFlag) {
            //: @synchronized (self) {
            @synchronized (self) {
                ...
                [self.glPreview supply:textureResult];
            }
        }
