...
/*: "SENSEME" :*/
fileprivate let notiSharedGameData:String = "SEtoSEME"

/*: "lic" :*/
fileprivate let kAllMessage:String = "lieffect"

/*: "mf/Index/senseTime" :*/
fileprivate let notiInfoUrl:[Character] = ["m","f","/","I","n","d","e","x","/","s","e","n","s","e","T","i","m","e"]

/*: "url" :*/
fileprivate let m_thirdName:[Character] = ["u","r","l"]

/*: "md5" :*/
fileprivate let mainAskId:String = "hidden5"

/*: "SENSEMELIC" :*/
fileprivate let m_positionLabelData:String = "modelENmodel"
fileprivate let userMeIdent:[Character] = ["E","M","E","L","I","C"]

/*: "App/ST/" :*/
fileprivate let app_ofTitle:[Character] = ["A","p","p","/","S","T"]
fileprivate let appWhiteValue:String = "up"

/*: "GET" :*/
fileprivate let notiGiftPath:String = "titleET"
...
// __CLOSE_PRINT__
//
//  ConstraintEraseReactiveCompatible.swift
//  AbroadTalking
//
...
//: @objcMembers public class ConstraintEraseReactiveCompatible: NSObject {
@objcMembers public class ConstraintEraseReactiveCompatible: NSObject {
    //: static public let share = ConstraintEraseReactiveCompatible()
    public static let share = ConstraintEraseReactiveCompatible()
    //: private override init() {}
    override private init() {}

    // 校验license有效性 先本地->后网络
    //: public func checkLicense() -> Bool {
    public func momentLicense() -> Bool {
    ...
    func license() -> Bool {
        ...
        if let path = ViewPodspecBundle.bundle.path(forResource: (notiSharedGameData.replacingOccurrences(of: "to", with: "N")), ofType: (kAllMessage.replacingOccurrences(of: "effect", with: "c"))) {
            //: let licenseData = NSData(contentsOfFile: path)
            let licenseData = NSData(contentsOfFile: path)
            //: if licenseData != nil {
            if licenseData != nil {
                //: return checkActiveCodeWithData(dataLicense: licenseData!)
                return withDataQueryLiquorLicenseActiveAgentOmertaAssemblage(dataLicense: licenseData!)
            }
        }
    ...
    func targetOf() -> Bool {
        ...
        if FileManager.default.fileExists(atPath: path as String) {
            ...
            if licData != nil {
                ...
                return withDataQueryLiquorLicenseActiveAgentOmertaAssemblage(dataLicense: licData!)
            }
        }
    ...
    func withDataQueryLiquorLicenseActiveAgentOmertaAssemblage(dataLicense: NSData) -> Bool {
...
extension ConstraintEraseReactiveCompatible {
    ...
    public func sizeMake(completion: @escaping Completionblock) {
        ...
        kUseSenseName.underway(model: reqModel) { _, result, _ in
            ...
            let licLink = json[(String(m_thirdName))].stringValue
            //: let md5 = json["md5"].stringValue
            let md5 = json[(mainAskId.replacingOccurrences(of: "hidden", with: "md"))].stringValue
            //: if self.checkLicFileMd5With(md5: md5 as NSString) == false {
            if self.userModel(md5: md5 as NSString) == false {
                //: if  licLink.count > 0 {
                if licLink.count > 0 {
                    //: self.downloadLicsenWith(licUrl: licLink as NSString, completion: completion)
                    self.nameCompletion(licUrl: licLink as NSString, completion: completion)
                    //: return
                    return
                }
            }
    ...
    func userModel(md5: NSString) -> Bool {
        //: if md5.length <= 0 {
        if md5.length <= 0 {
            //: return false
            return false
        ...
        let fileMd5: NSString = PresentAnimatTool.shared.socialIcon(filePath: filePath as String) as NSString
    ...
    func comeOn() -> NSString {
        ...
        return stDirectory.appendingPathComponent((m_positionLabelData.replacingOccurrences(of: "model", with: "S") + String(userMeIdent))) as NSString
    }
    ...
    func directory() -> NSString {
        ...
        let stDirectory = docDirectory.appendingPathComponent((String(app_ofTitle) + appWhiteValue.replacingOccurrences(of: "up", with: "/")))
    ...
    func nameCompletion(licUrl: NSString, completion: @escaping Completionblock) {
        //: var urlOk = true
        var urlOk = true
        ...
        request.httpMethod = (notiGiftPath.replacingOccurrences(of: "title", with: "G"))
        ...
        URLSession.shared.dataTask(with: request as URLRequest) { [weak self] data, _, _ in
            ...
            DispatchQueue.main.async {
                ...
                if data != nil {
                    ...
                    self.acceptTable(licData: data! as NSData)
                    //: licValid = self.checkDownloadedLicense()
                    licValid = self.targetOf()
                }
    ...
    func acceptTable(licData: NSData) {
        //: let path = licDiskPath()
        let path = comeOn()
        //: let fm = FileManager.default
        let fm = FileManager.default
