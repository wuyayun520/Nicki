...
protocol DelimitationNavigationDelegate: AnyObject {
...
class StageViewController: UIViewController {
    ...
    weak var delegate: DelimitationNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = MacroReactiveCompatible.getScreenHeight() - (topDistance + kTitle_H)
        var height = MacroReactiveCompatible.acquire() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= k_senseBottomAutoData
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: MacroReactiveCompatible.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: MacroReactiveCompatible.totalimateLastCurrent(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(MacroReactiveCompatible.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(MacroReactiveCompatible.totalimateLastCurrent()), height: 0)
        ...
        scrollView.semanticContentAttribute = ManagerThen.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: MacroReactiveCompatible.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: MacroReactiveCompatible.totalimateLastCurrent()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: MacroReactiveCompatible.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: MacroReactiveCompatible.totalimateLastCurrent()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = ManagerThen.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(snapDown), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = MacroReactiveCompatible.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = MacroReactiveCompatible.totalimateLastCurrent() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(MacroReactiveCompatible.getScreenWidth()-originX-titleWidth)
            let x = ManagerThen.shared.direction == .leftToRight ? originX:(MacroReactiveCompatible.totalimateLastCurrent()-originX-titleWidth)
...
extension StageViewController {
    ...
    @objc private func snapDown(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        word(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * MacroReactiveCompatible.getScreenWidth()
        let offset_x = CGFloat(index) * MacroReactiveCompatible.totalimateLastCurrent()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        singingVoice(withIndex: index)
        //: seletedViewController(withIndex: index)
        currentAfter(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func singingVoice(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * MacroReactiveCompatible.getScreenWidth()
        let origin_x = CGFloat(index) * MacroReactiveCompatible.totalimateLastCurrent()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: MacroReactiveCompatible.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: MacroReactiveCompatible.totalimateLastCurrent(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func word(_ button: UIButton) {
    ...
    func topTable(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= MacroReactiveCompatible.getScreenWidth() {
        if titleScrollView.contentSize.width <= MacroReactiveCompatible.totalimateLastCurrent() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - MacroReactiveCompatible.getScreenWidth() * 0.5
        var offset_x = sender.center.x - MacroReactiveCompatible.totalimateLastCurrent() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - MacroReactiveCompatible.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - MacroReactiveCompatible.totalimateLastCurrent()
...
extension StageViewController: UIScrollViewDelegate {
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / MacroReactiveCompatible.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / MacroReactiveCompatible.totalimateLastCurrent())
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / MacroReactiveCompatible.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / MacroReactiveCompatible.totalimateLastCurrent())
        ...
        //: let scaleR = scrollView.contentOffset.x / MacroReactiveCompatible.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / MacroReactiveCompatible.totalimateLastCurrent() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
