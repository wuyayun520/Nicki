...
fileprivate let dreamNameViewMessage:[UInt8] = [0x41,0x46,0x41,0x5c,0x0,0x4b,0x47,0x4c,0x4d,0x5a,0x12,0x1,0x8,0x40,0x49,0x5b,0x8,0x46,0x47,0x5c,0x8,0x4a,0x4d,0x4d,0x46,0x8,0x41,0x45,0x58,0x44,0x4d,0x45,0x4d,0x46,0x5c,0x4d,0x4c]

/*: "FFD855" :*/
fileprivate let mainRawTitle:String = "tableD855"
...
class CopernicanSystemThen: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - k_recordText
                ...
                self.sizeEnable(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dreamNameViewMessage.map{$0^40}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (mainRawTitle.replacingOccurrences(of: "table", with: "FF")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: kIntervalervalText, y: 0, width: k_recordText, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - k_recordText, y: 0, width: k_recordText, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.my(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: kIntervalervalText, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.my(0, 0, 0, 0.6)
...
extension CopernicanSystemThen {
    ...
    func untilState() {
        ...
        self.leftSilder.frame = CGRect(x: kIntervalervalText, y: 0, width: k_recordText, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - k_recordText, y: 0, width: k_recordText, height: self.frame.size.height)
        //: resetIndicator()
        netIndicator()
    }

    //: func needShowIndicator(isShow: Bool) {
    func currentReport(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func sizeEnable(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - k_recordText,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - k_recordText)
    }
}
...
extension CopernicanSystemThen {
    ...
    @objc func its(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.sizeEnable(startX: videoStartX)
    ...
    @objc func actionBond(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension CopernicanSystemThen {
    ...
    private func valueMinute() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(its(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(actionBond(gesture:)))
    ...
    func sexualCharacteristic(_ xposition: CGFloat) {
