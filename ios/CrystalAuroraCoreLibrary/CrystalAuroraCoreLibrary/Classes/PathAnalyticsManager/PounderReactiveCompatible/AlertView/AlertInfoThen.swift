...
fileprivate let user_shareData:String = "user title tip inputbtn_sh"
fileprivate let m_topValue:String = "rtBackplain up equal data"
fileprivate let noti_thenName:String = "fill"

/*: "btn_alertBackground_false" :*/
fileprivate let m_infoMessage:String = "size disappear make afterbtn_al"
fileprivate let k_stageTagTitle:[Character] = ["c","k","g","r","o","u","n","d","_","f","a","l","s","e"]
...
public class AlertInfoThen: NSObject {
    /// 通用回调
...
public extension AlertInfoThen {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: SpaceAlertConfig) -> Void
    ...
    internal class func nameConfig(title: String? = nil,
                                   //: message: String?  = nil,
                                   message: String? = nil,
    ...
    @objc internal class func dataDoing(title: String? = nil,
    ...
    class func mug(title: String? = nil,
                   ...
                   config: SpaceAlertConfig? = nil)
    {
        ...
        let alertView = FrameLabelView(title: title,
        ...
        //: MacroReactiveCompatible.getWindow().addSubview(alertView)
        MacroReactiveCompatible.colorForDestroy().addSubview(alertView)
    ...
    class func showAlert() {
        //: MacroReactiveCompatible.getWindow().subviews.forEach { (view) in
        MacroReactiveCompatible.colorForDestroy().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: FrameLabelView.self) {
    ...
    class func cellSizeConversation(title: String, isTouch: Bool) {
        //: MacroReactiveCompatible.getWindow().subviews.forEach { (view) in
        MacroReactiveCompatible.colorForDestroy().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: FrameLabelView.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: FrameLabelView = view as! FrameLabelView
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.reApp(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class SpaceAlertConfig {
    ...
    public var maskType: ValueMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .fontFile(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.startMake()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .fontFile(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.startMake()
    ...
    public var buttonFont: UIFont = .fontFile(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.subject()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(user_shareData.suffix(6)) + "ow_ale" + String(m_topValue.prefix(6)) + "groun" + noti_thenName.replacingOccurrences(of: "fill", with: "d"))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(m_infoMessage.suffix(6)) + "ertBa" + String(k_stageTagTitle))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .fontFile(type: .Medium, fontSize: 16)
