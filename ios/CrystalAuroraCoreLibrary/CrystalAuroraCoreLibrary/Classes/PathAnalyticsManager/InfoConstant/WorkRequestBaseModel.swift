...
/*: "http://stat. :*/
fileprivate let k_modelContent:String = "number makehttp:/"
fileprivate let noti_amNameKey:[Character] = ["/","s","t","a","t","."]

/*: .com" :*/
fileprivate let k_spaceName:String = ".comerror if"

/*: "http://" :*/
fileprivate let m_equalKey:String = "http://count name"

/*: "https://" :*/
fileprivate let k_sessionUrl:[Character] = ["h","t","t"]
fileprivate let m_dataModelContent:[Character] = ["p","s",":","/","/"]

/*: "http://m. :*/
fileprivate let dreamVoiceTitleKey:[Character] = ["h","t","t","p",":","/","/","m"]
fileprivate let notiRefMessage:String = "label"

/*: "http:// :*/
fileprivate let show_fromIdent:String = "file info viewhttp://"

/*: "-1" :*/
fileprivate let noti_valueTitle:String = "-1"

/*: "3,90" :*/
fileprivate let appDismissTitle:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let dreamUserName:[Character] = ["1","9","9","0","-","0","1","-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let userErrMsg:[Character] = ["M","M","-","d","d","-","y"]
fileprivate let mNameSkinIdent:String = "sizesizesize"

/*: "id" :*/
fileprivate let userNoStr:String = "mark"
...
class SmallnessThen: NSObject {
    ...
    var requestType: BlotRequestType?
    ...
    override init() {
        ...
        self.requestServer = NorAddrTool.share.serverUrlStr
...
struct PresentMeasurable: HandyJSON {
    //: var errno: Int!
...
public struct DetailTopicErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct ContentMeasurable: HandyJSON {
    ...
    var CSConfig = FromConfigModel() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(k_modelContent.suffix(6)) + String(noti_amNameKey)) + "\(k_screenRecordData)" + (String(k_spaceName.prefix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(m_equalKey.prefix(7)))) && !statDomain.hasPrefix((String(k_sessionUrl) + String(m_dataModelContent))) {
                //: return "http://".appending(statDomain)
                return (String(m_equalKey.prefix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(dreamVoiceTitleKey) + notiRefMessage.replacingOccurrences(of: "label", with: ".")) + "\(k_screenRecordData)" + (String(k_spaceName.prefix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(show_fromIdent.suffix(7))) + "\(urlDomain)"
        }
    }
...
public class PansexualHandyJSON: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [DatabaseMeasurable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [DatabaseMeasurable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [DatabaseMeasurable] = [] // 语音收费设置
    ...
    var vipSetting = [TrackTaskModelType]() // vip皮肤
    ...
    var tabPageBanner = [DigitizerMeasurable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = RowTransformable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class ModeThen: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [SumTagModel]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [SumTagModel]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var callPopupSetting = (appDismissTitle.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.dayImageWith(self.birthday ?? (String(dreamUserName)), dateFormat: (String(userErrMsg) + mNameSkinIdent.replacingOccurrences(of: "size", with: "y")))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.meanSolarDayComponentsGetCurrent().year else { return "0" }
...
struct UserMeasurable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct SumTagModel: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func pathCurTo(compareTo: SumTagModel) -> Bool {
        //: return
        return
...
struct CloudTagModel: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [TitleModelType]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [TitleModelType]!
}
...
struct TitleModelType: HandyJSON {
    ...
    var list: [SumTagModel]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct FromConfigModel: HandyJSON {
...
struct TrackTaskModelType: HandyJSON {
