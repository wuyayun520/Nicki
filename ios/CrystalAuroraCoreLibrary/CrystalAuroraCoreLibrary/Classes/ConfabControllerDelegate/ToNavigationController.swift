...
class ToNavigationController: UINavigationController {
    ...
    var tabBarType: TotalKeyRepresentable?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.averageTime()
            }
        }
    ...
    func toRange() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.sumColor(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.subTarget()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.startMake(), NSAttributedString.Key.font: UIFont.fontFile(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.sumColor(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.startMake(), NSAttributedString.Key.font: UIFont.fontFile(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.startMake()
...
extension ToNavigationController: UINavigationControllerDelegate {
...
extension ToNavigationController: UIGestureRecognizerDelegate {
